...
small_object_rand = pregenerated_asset_rand(
...
def floating_object_asset_rand(rng: pf.RNG) -> pf.MeshObject:
    obj_func = pf.control.choice(
        ...
        [
            ...
            (table.coffee_table_rand, 1.0),
            (table.cocktail_table_rand, 1.0),
            (table.dining_table_rand, 1.0),
            (vase.vase_rand, 1.0),
            # (plate_rack.plate_rack_rand, 1.0),
            # (plate_rack.plate_rand, 1.0),
            # (plate_rack.plate_on_rack_rand, 1.0),
            (flower.flower_rand, 1.0),
            (triangle_shelf.triangle_shelf_rand, 1.0),
    ...
    res_func = pf.control.choice(
        ...
        [
            ...
            (obj_func, 3.0),
            # (small_object_rand, 1.5),
        ],
    )

    result = res_func(rng)
    if result is None:
        result = random_primitives.primitives_rand(rng)
...
def floating_lights_rand(
    ...
    if n_lights > 0:
        ...
        for i, frac in enumerate(fractions):
            light = point_lamp_colored_rand(rng, energy=float(frac * total_wattage))
