...
def _bezier_zeros(x, q0, q1, q2, q3):
    c0 = q0 - x
    c1 = 3.0 * (q1 - q0)
    c2 = 3.0 * (q0 - 2.0 * q1 + q2)
    c3 = q3 - q0 + 3.0 * (q1 - q2)

    return _solve_cubic(c0, c1, c2, c3)


def _y_of_bezier(t, cy):
    """
...
def _data_from_keyframes(keyframe_points, frame_start, frame_end, for_acceleration):
    """
    ...
    for i in range(len(keyframe_points) - 1):
        ...
        if keyframe_points[i].interpolation == "CONSTANT":
            ...
            for j in range(f_to_i(kfs, True), f_to_i(kfe, False) + 1):
                roots = _bezier_zeros(i_to_f(j), cx[0], cx[1], cx[2], cx[3])
                if len(roots) == 0:
                    raise Exception(
                        "Bezier interpolation failed at frame {}".format(i_to_f(j))
                    )
                data[j] = (
                    _a_of_bezier(roots[0], cx, cy)
                    if for_acceleration
                    else _v_of_bezier(roots[0], cx, cy)
                )
                locs[j] = _y_of_bezier(roots[0], cy)
...
def _get_imu_tum_data(obj: bpy.types.Object, start: int, end: int) -> tuple[str, str]:
    """
    ...
    for curve in obj.animation_data.action.fcurves:
        if curve.data_path == "location":
            if curve.array_index == 0:
                ax, x = _data_from_keyframes(curve.keyframe_points, start, end, True)
            elif curve.array_index == 1:
                ay, y = _data_from_keyframes(curve.keyframe_points, start, end, True)
            elif curve.array_index == 2:
                az, z = _data_from_keyframes(curve.keyframe_points, start, end, True)

        elif curve.data_path == "rotation_euler":
            if curve.array_index == 0:
                rvx, rx = _data_from_keyframes(curve.keyframe_points, start, end, False)
            elif curve.array_index == 1:
                rvy, ry = _data_from_keyframes(curve.keyframe_points, start, end, False)
            elif curve.array_index == 2:
                rvz, rz = _data_from_keyframes(curve.keyframe_points, start, end, False)
...
def save_imu_tum_files(
    ...
    for i in range(len(objects)):
        try:
            imu_text, tum_text = _get_imu_tum_data(
