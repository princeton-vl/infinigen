...
# Authors:
# - Beining Han: original Infinigen v1 nodegroup (https://github.com/princeton-vl/infinigen/blob/05a09759fe9478595a3323ec2d6e26ce3513223f/infinigen/assets/objects/shelves/triangle_shelf.py)
...
class _LegStraightResult(NamedTuple):
    mesh: pf.ProcNode
...
def _leg_straight(
    ...
    table_profile_result = _table_profile(
        profile_n_gon=n_gon,
...
def _curve_board(
    ...
    translation = pf.nodes.math.combine_xyz(z=thickness)
...
def _shelf_boards(
    ...
    curve_board_result = _curve_board(
        thickness=thickness,
...
def triangle_shelf_rand(
    ...
    if board_material is None:
        board_material = table_top_material_rand(rng, vec)
