...
def _flower_color_rand(rng: pf.RNG) -> pf.Color:
...
def _petal_material_rand(rng: pf.RNG) -> pf.Material:
    rngs = rng.spawn(4)

    petal_color = _flower_color_rand(rngs[0])
...
def _flower_material_rand(rng: pf.RNG) -> FlowerMaterials:
    rngs = rng.spawn(2)

    petal_mat = _petal_material_rand(rngs[0])
...
def _follow_curve(
    ...
    position_length = pf.nodes.math.map_range(
    ...
    sample_curve_length = pf.nodes.geo.sample_curve_length(
        curves=curve,
        length=position_length * curve_length,
        value=0.0,
    )

    offset_vector = pf.nodes.math.vector_cross_product(
        a=sample_curve_length.tangent,
        b=sample_curve_length.normal,
    )
    offset_1 = pf.nodes.math.vector_scale(
        vector=offset_vector,
        scale=capture_attribute.attribute.x,
    )
    offset_0 = pf.nodes.math.vector_scale(
...
def _polar_to_cart(
    ...
    b_y = pf.nodes.math.cos(value)
    b_z = pf.nodes.math.sin(value)
    b = pf.nodes.math.combine_xyz(y=b_y, z=b_z)

    vector_multiply_add = pf.nodes.math.vector_multiply_add(
        a=vector, b=b, addend=addend
    )
...
def _flower_petal(
    ...
    selection = pf.nodes.math.constant(1.0)
...
def flower_rand(
    ...
    if materials is None:
        materials = _flower_material_rand(rngs[0])
