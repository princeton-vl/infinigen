...
def art_frame_swept(
    ...
    panel_material: t.SocketOrVal[pf.Material],
    panel_depth_pct: t.SocketOrVal[float],
    frame_profile: pf.ProcNode[pf.CurveObject],
) -> pf.ProcNode:
    """Like `art_frame`, but sweeps a moulding profile around the border centerline.

    `frame_profile` follows the trim convention (symmetric about y=0 with height
    matching frame_width, relief in -X); with the centerline normal set to -Z the
    relief protrudes out the panel front.
    """
    grid_size_x = pf.nodes.math.multiply_add(a=frame_width, b=-2.0, addend=dimensions.y)
    grid_size_y = pf.nodes.math.multiply_add(a=frame_width, b=-2.0, addend=dimensions.z)
    ...
    centerline = pf.nodes.geo.curve_quadrilateral(
        width=grid_size_x + frame_width, height=grid_size_y + frame_width
    )
    ...
    join = pf.nodes.geo.join_geometry([panel, frame])
    set_shade_smooth = pf.nodes.geo.set_shade_smooth(geometry=join, shade_smooth=False)
...
def wall_art_rand(
    ...
    def _moulded_frame() -> pf.ProcNode:
        profile = trim_profile_rand(r_profile, width=dimensions.x, height=frame_width)
        return art_frame_swept(
            dimensions=dimensions,
            frame_width=frame_width,
            frame_material=frame_material,
            panel_material=panel_material,
            panel_depth_pct=panel_depth_pct,
            frame_profile=pf.nodes.geo.object_info(profile).geometry,
        )

    frame_fn = pf.control.choice(
        r_frame_style, [(_plain_frame, 1.0), (_moulded_frame, 2.0)]
    )
    geo = frame_fn()
