...
def _flip_index(
    ...
    a_0 = input_index.astype(dtype=float) % v_resolution.astype(dtype=float)
    b = pf.nodes.math.floor(
        input_index.astype(dtype=float) / v_resolution.astype(dtype=float)
    )
    add = a_0 * u_resolution.astype(dtype=float) + b
    return add
...
class _LoftingResult(NamedTuple):
    geometry: pf.ProcNode
    top: pf.ProcNode[bool]
    side: pf.ProcNode[bool]
    bottom: pf.ProcNode[bool]
...
def vase_material_rand(rng: pf.RNG, vec) -> pf.Material:
    def vase_tile(rng, vector: pf.Vector) -> pf.Material:
        ...
        return tiles.tile_indoor_wall_rand(rng_tile, vector, grout=grout, scale=scale)

    def vase_hammered(rng, _vector: pf.Vector) -> pf.Material:
        return metal_hammered.metal_hammered_rand(
    ...
    material_func = pf.control.choice(
        ...
        [
            ...
            (vase_hammered, 1.0),
            (vase_tile, 2.0),
            (terrazzo.terrazzo_multicolor_rand, 2.0),
...
def vase_rand(rng: pf.RNG) -> VaseResult:
    ...
    mat_result = vase_material_rand(rng_mat, uv)
