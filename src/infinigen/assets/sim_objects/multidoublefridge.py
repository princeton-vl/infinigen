...
__all__ = ["MultiDoublefridgeFactory"]
...
class MultiDoublefridgeFactory(AssetFactory):
    ...
    def sample_heights(self, level_num):
        if level_num == 1:
            ...
            if uniform() < 1:  # 1 drawer + 1 fridge
                heights = [uniform(0.7, 1.5), uniform(0.3, 0.5)]
            ...
            if uniform() < 1:  # 2 drawers + 1 fridge
                heights = [uniform(0.7, 1), uniform(0.3, 0.5), uniform(0.3, 0.5)]
                # random.shuffle(heights)
                return heights
    ...
    def sample_parameters(self):
        ...
        level_num = 2  # np.random.choice([1, 2, 3], p=[0.65, 0.25, 0.1])
        ...
        for index in range(level_num):
            ...
            if is_fridge:
                ...
                door_shelf_num = 2  # min(4, math.floor((size[2] - door_shelf_margin) * uniform(0.85, 0.9) / door_shelf_height))
                door_shelf_size = (
                    door_shelf_height,
                ...
                door_margin = max(
                    uniform(0.02, 0.05),
                    (size[1] / 2 - door_shelf_size[1]) / 2 + door_shelf_thickness * 2,
                )
    ...
    def create_asset(self, export=True, exporter="mjcf", asset_params=None, **kwargs):
        ...
        tmp_buleprint_path = blueprint_path_completion(
            f"multidoublefridge_tmp_{random_string}.json"
        )
