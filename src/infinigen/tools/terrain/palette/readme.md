...
This palette generating tool is enabled using `GoogleImagesSearch` (set the `GCS_DEVELOPER_KEY` and `GCS_CX` environment variables to your own API credentials). 
...
This downloads 10 images from Google by default, and runs Mixed Gaussian Model (MGM) on HSV space to get an initial palette.
...
Under folder `json`, there is a `mountain rock.json`. By installing Add-on `Flutter Color` and `JSON5 syntax` in VSCode, you can see the color icon. Manually comment out the background colors that do not belong to your keyword.
