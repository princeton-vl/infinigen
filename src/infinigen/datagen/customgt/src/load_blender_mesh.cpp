...
auto parse_json(const fs::path json_path)
{
    ...
    {
        if (instance_item.contains("filename") && (instance_item["object_type"] == "MESH"))
...
std::shared_ptr<BaseBlenderObject> load_blender_mesh(const fs::path src_json_path,
                                                     const fs::path dst_json_path)
{
    ...
    {
        // while ((it != info_lookup.end()) && match_regex(".*errain.*",
        // it->second.name).empty()) it++;
        ...
        std::cout << std::to_string(object_idx++) << "/" << std::to_string(info_lookup.size())
                  << std::endl;

        if (next_info_lookup.count(current_mesh_id) > 0)
        {
            next_obj = next_info_lookup.at(current_mesh_id);
            const fs::path next_npz_path =
                fs::path(dst_json_path.parent_path()) / next_obj.npz_filename;
    ...
    if (current_obj.type == "MESH")
    {
        ...
        new_obj = std::shared_ptr<BaseBlenderObject>(
            new MeshBlenderObject(current_buf, next_buf, instance_ids, current_obj));
    ...
    else if (current_obj.type == "CURVES")
    {
        ...
        new_obj = std::shared_ptr<BaseBlenderObject>(
            new CurvesBlenderObject(current_buf, next_buf, all_instance_ids, current_obj));
