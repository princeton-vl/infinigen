...
AOV_CONFIGS = {
    ExportType.OBJECT_INDEX: AOVConfig(
        name="object_index", type="VALUE", export_type=ExportType.OBJECT_INDEX
    ),
    ExportType.MATERIAL_INDEX: AOVConfig(
...
def setup_object_attributes_for_aovs(
...
def configure_materials_for_aovs(
    ...
    if object_index_aov:
        setup_object_attributes_for_aovs(aov_mapping)

    for mat in bpy.data.materials:
        if mat.use_nodes and mat.node_tree:
            configure_material_aov_outputs(
                mat,
                object_index_aov,
                material_index_aov,
            )
...
def configure_material_aov_outputs(
