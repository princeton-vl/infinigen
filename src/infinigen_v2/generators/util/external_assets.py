...
from infinigen_v2.util.import_utils import module_path
...
def import_asset_file(file_path: Path) -> pf.MeshObject:
...
def distribution_from_asset_glob(folder: Path):
    ...
    if not paths:
        ...
        def missing_distribution(_rng: pf.RNG) -> pf.MeshObject | None:
            return None

        return missing_distribution

    def new_distribution(rng: pf.RNG) -> pf.MeshObject:
        asset_path = _sample_asset_path(paths, rng)
        # with Suppress(): # unfortunately doesnt work
        obj = import_asset_file(asset_path)
...
def pregenerated_asset_distribution(
    relative_glob: str,
