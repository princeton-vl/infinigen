...
def table_top_material_distribution(rng: pf.RNG, vec) -> pf.Material:
    vec = uv_maybe_rotate_90(rng, vec)
...
def glass_material_distribution(rng: pf.RNG, vec, glass_height=None) -> pf.Material:
    ...
    mask = pf.control.choice(rng, options)(rng=rng, vector=splats_vector).mask

    roughness = pf.random.clip_gaussian(rng, 0.02, 0.03, 0.0, 0.3)
    glass = glass_no_refraction.glass_no_refraction_distribution(
        rng, vec, roughness=roughness
    )
    grime = splats_base_material_distribution(rng, splats_vector)
...
def furniture_fabric(
    ...
    color = fabric.fabric_color_distribution(rng, value=value)
    ...
    return wear_func(rng, vec, material)
...
def _layer_distribution(
    ...
    return layer(rng, vector, material)
