...
def test_conf_emits_assets_images_layout() -> None:
    ...
    and mirrors a bundle into <slug>/assets/images/<name>/<i>.<ext> (png stills or mp4
    trajectories). If conf.py stops emitting that exact layout, published images 404
    (the #806 class of bug). Update collect.py's dest_subpath in lockstep if this fails."""
    src = CONF.read_text(encoding="utf-8")
    assert 'f"images/{name}/{i}.{ext}"' in src
