...
Self-contained example projects: each renders a multi-pass video dataset with `infinigen2`, locally or as a SLURM array. The intended workflow is to **copy these scripts into your own project and edit them**, not to clone the whole repo.
...
All commands below assume you are inside this `examples/` folder, with `infinigen2` and `procfunc` installed (`uv run` resolves them).
