...
def nodegroup_dragonfly_wing(nw: NodeWrangler):
    ...
    resample_curve = nw.new_node(Nodes.ResampleCurve, input_kwargs={"Curve": bezier_segment, "Count": resolution})
    ...
    curve_to_mesh_1 = nw.new_node(Nodes.CurveToMesh, input_kwargs={"Curve": quadratic_bezier_2})

    geometry_proximity = nw.new_node(
        Nodes.Proximity, input_kwargs={"Target": curve_to_mesh_1}, attrs={"target_element": "EDGES"}
    )
    ...
    capture_attribute = nw.new_node(
        Nodes.CaptureAttribute, input_kwargs={"Geometry": quadratic_bezier_1, 2: spline_parameter.outputs["Factor"]}
    )

    curve_to_mesh_2 = nw.new_node(Nodes.CurveToMesh, input_kwargs={"Curve": capture_attribute.outputs["Geometry"]})

    less_than = nw.new_node(
        Nodes.Compare, input_kwargs={0: capture_attribute.outputs[2], 1: 0.65}, attrs={"operation": "LESS_THAN"}
    )

    greater_than = nw.new_node(Nodes.Compare, input_kwargs={0: capture_attribute.outputs[2], 1: 0.84})

    op_or = nw.new_node(Nodes.BooleanMath, input_kwargs={0: less_than, 1: greater_than}, attrs={"operation": "OR"})

    delete_geometry = nw.new_node(Nodes.DeleteGeometry, input_kwargs={"Geometry": curve_to_mesh_2, "Selection": op_or})

    geometry_proximity_1 = nw.new_node(
        Nodes.Proximity, input_kwargs={"Target": delete_geometry}, attrs={"target_element": "EDGES"}
    )
...
def shader_wing_shader(nw: NodeWrangler):
    ...
    attribute_2 = nw.new_node(Nodes.Attribute, attrs={"attribute_name": "stripes coordinate"})

    map_range_1 = nw.new_node(Nodes.MapRange, input_kwargs={"Value": attribute_2.outputs["Fac"], 1: 0.04, 2: 0.54})

    attribute_1 = nw.new_node(Nodes.Attribute, attrs={"attribute_name": "pos"})

    vector_rotate = nw.new_node(
        Nodes.VectorRotate, input_kwargs={"Vector": attribute_1.outputs["Vector"], "Angle": 0.1047}
    )

    value = nw.new_node(Nodes.Value)
    value.outputs[0].default_value = 0.08

    group = nw.new_node(nodegroup_add_noise().name, input_kwargs={"Vector": vector_rotate, "amount": value})
    ...
    multiply = nw.new_node(
        Nodes.Math, input_kwargs={0: voronoi_texture_2.outputs["Distance"], 1: 2.34}, attrs={"operation": "MULTIPLY"}
    )
    ...
    less_than = nw.new_node(
        Nodes.Math, input_kwargs={0: separate_xyz.outputs["Y"], 1: value_2}, attrs={"operation": "LESS_THAN"}
    )

    maximum = nw.new_node(
        Nodes.Math, input_kwargs={0: voronoi_texture.outputs["Distance"], 1: less_than}, attrs={"operation": "MAXIMUM"}
    )

    multiply_1 = nw.new_node(Nodes.Math, input_kwargs={0: maximum, 1: 0.56}, attrs={"operation": "MULTIPLY"})

    vector_rotate_1 = nw.new_node(
        Nodes.VectorRotate, input_kwargs={"Vector": attribute_1.outputs["Vector"], "Angle": 0.2485}
    )

    value_1 = nw.new_node(Nodes.Value)
    value_1.outputs[0].default_value = 0.08

    group_1 = nw.new_node(nodegroup_add_noise().name, input_kwargs={"Vector": vector_rotate_1, "amount": value_1})

    separate_xyz_1 = nw.new_node(Nodes.SeparateXYZ, input_kwargs={"Vector": group_1})

    subtract = nw.new_node(
        Nodes.Math, input_kwargs={0: separate_xyz.outputs["Y"], 1: value_2}, attrs={"operation": "SUBTRACT"}
    )

    multiply_2 = nw.new_node(Nodes.Math, input_kwargs={0: subtract, 1: -0.74}, attrs={"operation": "MULTIPLY"})

    power = nw.new_node(Nodes.Math, input_kwargs={0: multiply_2, 1: 2.22}, attrs={"operation": "POWER"})

    add = nw.new_node(Nodes.Math, input_kwargs={0: separate_xyz_1.outputs["Y"], 1: power})
    ...
    greater_than = nw.new_node(
        Nodes.Math, input_kwargs={0: separate_xyz.outputs["Y"], 1: value_2}, attrs={"operation": "GREATER_THAN"}
    )
    ...
    less_than_1 = nw.new_node(Nodes.Math, input_kwargs={0: add, 1: -0.48}, attrs={"operation": "LESS_THAN"})

    maximum_2 = nw.new_node(Nodes.Math, input_kwargs={0: maximum_1, 1: less_than_1}, attrs={"operation": "MAXIMUM"})

    multiply_3 = nw.new_node(Nodes.Math, input_kwargs={0: maximum_2, 1: 3.0}, attrs={"operation": "MULTIPLY"})

    minimum = nw.new_node(Nodes.Math, input_kwargs={0: multiply_1, 1: multiply_3}, attrs={"operation": "MINIMUM"})

    minimum_1 = nw.new_node(Nodes.Math, input_kwargs={0: multiply, 1: minimum}, attrs={"operation": "MINIMUM"})

    attribute = nw.new_node(Nodes.Attribute, attrs={"attribute_name": "distance to edge"})

    map_range = nw.new_node(Nodes.MapRange, input_kwargs={"Value": attribute.outputs["Color"], 3: 0.1, 4: 0.0})

    maximum_3 = nw.new_node(
        Nodes.Math, input_kwargs={0: minimum_1, 1: map_range.outputs["Result"]}, attrs={"operation": "MAXIMUM"}
    )

    minimum_2 = nw.new_node(
        Nodes.Math, input_kwargs={0: map_range_1.outputs["Result"], 1: maximum_3}, attrs={"operation": "MINIMUM"}
    )
    ...
    transparent_bsdf_1 = nw.new_node(Nodes.TransparentBSDF, input_kwargs={"Color": reroute})

    principled_bsdf = nw.new_node(Nodes.PrincipledBSDF, input_kwargs={"Base Color": reroute})

    mix_shader = nw.new_node(Nodes.MixShader, input_kwargs={"Fac": 0.1, 1: transparent_bsdf_1, 2: principled_bsdf})
