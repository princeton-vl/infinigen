...
def shader_dragonfly_head_shader(nw: NodeWrangler, base_color, v):
    ...
    combine_xyz = nw.new_node(
        Nodes.CombineXYZ, input_kwargs={"X": separate_xyz.outputs["X"], "Y": absolute, "Z": separate_xyz.outputs["Z"]}
    )
    ...
    map_range = nw.new_node(Nodes.MapRange, input_kwargs={"Value": musgrave_texture, 1: -0.28, 2: 0.48})
    ...
    mix = nw.new_node(
        Nodes.MixRGB, input_kwargs={"Fac": map_range.outputs["Result"], "Color1": hue_saturation_value, "Color2": group}
    )

    principled_bsdf = nw.new_node(
        Nodes.PrincipledBSDF, input_kwargs={"Base Color": mix, "Specular": 0.7545, "Roughness": 0.0636}
    )
...
def nodegroup_dragon_fly_head(
    nw: NodeWrangler, base_color=(0.2789, 0.3864, 0.0319, 1.0), eye_color=(0.2789, 0.3864, 0.0319, 1.0), v=0.3
):
    # Code generated using version 2.4.3 of the node_transpiler

    curve_line = nw.new_node(Nodes.CurveLine, input_kwargs={"End": (1.8, 0.0, 0.0)})

    resample_curve = nw.new_node(Nodes.ResampleCurve, input_kwargs={"Curve": curve_line, "Count": 32})
    ...
    capture_attribute = nw.new_node(
        Nodes.CaptureAttribute, input_kwargs={"Geometry": set_position, 2: spline_parameter_1.outputs["Factor"]}
    )

    float_curve_1 = nw.new_node(Nodes.FloatCurve, input_kwargs={"Value": capture_attribute.outputs[2]})
    node_utils.assign_curve(
        float_curve_1.mapping.curves[0], [(0.0, 0.14), (0.3055, 0.93), (0.7018, 0.79), (0.9236, 0.455), (1.0, 0.0)]
    )

    set_curve_radius = nw.new_node(
        Nodes.SetCurveRadius, input_kwargs={"Curve": capture_attribute.outputs["Geometry"], "Radius": float_curve_1}
    )

    curve_circle = nw.new_node(Nodes.CurveCircle, input_kwargs={"Resolution": 200, "Radius": 1.1})
    ...
    set_material_1 = nw.new_node(
        ...
        input_kwargs={
            ...
            "Material": surface.shaderfunc_to_material(shader_dragonfly_head_shader, base_color, v),
        },
    )
    ...
    dragonflyeye = nw.new_node(nodegroup_dragonfly_eye(base_color=eye_color, v=0.0).name, input_kwargs={"Rings": 128})

    value_1 = nw.new_node(Nodes.Value)
    value_1.outputs[0].default_value = 0.6

    transform_1 = nw.new_node(Nodes.Transform, input_kwargs={"Geometry": dragonflyeye, "Scale": value_1})
    ...
    symmetric_clone = nw.new_node(
        nodegroup_symmetric_clone().name, input_kwargs={"Geometry": attach_part.outputs["Geometry"]}
    )
    ...
    surfacebump = nw.new_node(
        nodegroup_surface_bump().name, input_kwargs={"Geometry": addhair_1, "Displacement": 0.05, "Scale": 5.0}
    )

    value = nw.new_node(Nodes.Value)
    value.outputs[0].default_value = 0.07

    transform = nw.new_node(Nodes.Transform, input_kwargs={"Geometry": surfacebump, "Scale": value})
    ...
    surfacebump_1 = nw.new_node(
        nodegroup_surface_bump().name, input_kwargs={"Geometry": antenna.outputs["Geometry"], "Scale": 5.0}
    )
    ...
    transform_2 = nw.new_node(
        Nodes.Transform, input_kwargs={"Geometry": set_material, "Translation": (-0.02, 0.0, 0.0), "Scale": value_2}
    )
    ...
    symmetric_clone_1 = nw.new_node(
        nodegroup_symmetric_clone().name, input_kwargs={"Geometry": attach_part_2.outputs["Geometry"]}
    )
    ...
    join_geometry = nw.new_node(Nodes.JoinGeometry, input_kwargs={"Geometry": store_named_attribute})
