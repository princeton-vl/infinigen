...
def shader_dragonfly_eye_shader(nw: NodeWrangler, base_color, v):
    ...
    musgrave_texture = nw.new_node(Nodes.MusgraveTexture, input_kwargs={"Scale": 2.0, "Detail": 1.0})

    map_range = nw.new_node(Nodes.MapRange, input_kwargs={"Value": musgrave_texture, 1: -1.0, 2: 0.2})
    ...
    voronoi_texture = nw.new_node(
        Nodes.VoronoiTexture, input_kwargs={"Scale": 1000.0}, attrs={"feature": "DISTANCE_TO_EDGE"}
    )

    map_range_1 = nw.new_node(
        Nodes.MapRange, input_kwargs={"Value": voronoi_texture.outputs["Distance"], 1: 0.03, 2: 0.2, 3: 1.0, 4: -0.78}
    )
...
def nodegroup_dragonfly_eye(
    ...
    group_input = nw.new_node(Nodes.GroupInput, expose_input=[("NodeSocketInt", "Rings", 16)])

    multiply = nw.new_node(
        Nodes.Math, input_kwargs={0: group_input.outputs["Rings"], 1: 2.0}, attrs={"operation": "MULTIPLY"}
    )

    uv_sphere = nw.new_node(
        Nodes.MeshUVSphere, input_kwargs={"Segments": multiply, "Rings": group_input.outputs["Rings"]}
    )

    transform = nw.new_node(Nodes.Transform, input_kwargs={"Geometry": uv_sphere, "Scale": (1.0, 1.0, 1.3)})

    set_material = nw.new_node(
        Nodes.SetMaterial,
        input_kwargs={
            "Geometry": transform,
            "Material": surface.shaderfunc_to_material(shader_dragonfly_eye_shader, base_color, v),
        },
    )
