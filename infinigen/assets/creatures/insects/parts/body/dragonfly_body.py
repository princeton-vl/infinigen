...
def shader_dragonfly_body_shader(nw: NodeWrangler, base_color, v):
    ...
    multiply = nw.new_node(
        Nodes.Math, input_kwargs={0: separate_xyz.outputs["Z"], 1: 3.0}, attrs={"operation": "MULTIPLY"}
    )
    ...
    map_range = nw.new_node(Nodes.MapRange, input_kwargs={"Value": musgrave_texture, 1: -0.26, 2: 0.06})

    attribute_2 = nw.new_node(Nodes.Attribute, attrs={"attribute_name": "spline parameter"})

    combine_xyz_1 = nw.new_node(Nodes.CombineXYZ, input_kwargs={"X": attribute_2.outputs["Fac"]})
    ...
    combine_xyz_2 = nw.new_node(
        Nodes.CombineXYZ, input_kwargs={"X": separate_xyz_1.outputs["X"], "Y": attribute_1.outputs["Fac"]}
    )
    ...
    map_range_2 = nw.new_node(Nodes.MapRange, input_kwargs={"Value": add, 1: 0.7, 3: 1.0, 4: 0.0})
    ...
    hue_saturation_value = nw.new_node("ShaderNodeHueSaturation", input_kwargs={"Value": v, "Color": rgb_1})
...
def nodegroup_dragonfly_body(
    ...
    combine_xyz = nw.new_node(Nodes.CombineXYZ, input_kwargs={"Z": group_input.outputs["Body Length"]})

    curve_line = nw.new_node(Nodes.CurveLine, input_kwargs={"End": combine_xyz})

    resample_curve = nw.new_node(Nodes.ResampleCurve, input_kwargs={"Curve": curve_line, "Count": 128})

    spline_parameter = nw.new_node(Nodes.SplineParameter)

    capture_attribute = nw.new_node(
        Nodes.CaptureAttribute, input_kwargs={"Geometry": resample_curve, 2: spline_parameter.outputs["Factor"]}
    )
    ...
    curve_to_mesh = nw.new_node(
        Nodes.CurveToMesh, input_kwargs={"Curve": set_curve_radius, "Profile Curve": transform, "Fill Caps": True}
    )
    ...
    multiply = nw.new_node(
        Nodes.VectorMath, input_kwargs={0: position_2, 1: (1.0, 0.2, 0.8)}, attrs={"operation": "MULTIPLY"}
    )
    ...
    map_range = nw.new_node(
        Nodes.MapRange, input_kwargs={"Value": voronoi_texture.outputs["Distance"], 4: 0.4}, attrs={"clamp": False}
    )

    multiply_1 = nw.new_node(
        Nodes.Math, input_kwargs={0: map_range.outputs["Result"], 1: -1.0}, attrs={"operation": "MULTIPLY"}
    )
    ...
    set_position = nw.new_node(
        Nodes.SetPosition, input_kwargs={"Geometry": curve_to_mesh, "Offset": add.outputs["Vector"]}
    )
    ...
    set_material = nw.new_node(
        ...
        input_kwargs={
            ...
            "Material": surface.shaderfunc_to_material(shader_dragonfly_body_shader, base_color, v),
        },
    )
    ...
    greater_than = nw.new_node(Nodes.Compare, input_kwargs={0: separate_xyz.outputs["Y"], 1: 0.5})

    reroute = nw.new_node(Nodes.Reroute, input_kwargs={"Input": capture_attribute.outputs[2]})

    less_than = nw.new_node(Nodes.Compare, input_kwargs={0: reroute, 1: 0.4}, attrs={"operation": "LESS_THAN"})

    op_and = nw.new_node(Nodes.BooleanMath, input_kwargs={0: greater_than, 1: less_than})

    reroute_1 = nw.new_node(Nodes.Reroute, input_kwargs={"Input": group_input.outputs["Hair Density"]})
    ...
    leghair = nw.new_node(nodegroup_principled_hair().name, input_kwargs={"Resolution": 2})

    transform_3 = nw.new_node(Nodes.Transform, input_kwargs={"Geometry": leghair, "Scale": (1.0, 1.0, 5.0)})
    ...
    multiply_2 = nw.new_node(Nodes.Math, input_kwargs={0: reroute_1, 1: 0.3}, attrs={"operation": "MULTIPLY"})

    distribute_points_on_faces_1 = nw.new_node(
        Nodes.DistributePointsOnFaces, input_kwargs={"Mesh": store_named_attribute, "Density": multiply_2, "Seed": 1}
    )
    ...
    join_geometry = nw.new_node(
        Nodes.JoinGeometry, input_kwargs={"Geometry": [store_named_attribute, instanceonpoints, instanceonpoints_1]}
    )
