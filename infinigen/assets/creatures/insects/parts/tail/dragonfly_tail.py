...
from infinigen.assets.creatures.insects.utils.shader_utils import nodegroup_add_noise, nodegroup_color_noise
...
def shader_dragonfly_tail_shader(nw: NodeWrangler, base_color, v, ring_length):
    ...
    attribute = nw.new_node(Nodes.Attribute, attrs={"attribute_name": "cross section parameter"})
    ...
    map_range = nw.new_node(Nodes.MapRange, input_kwargs={"Value": colorramp.outputs["Color"], 1: 0.02, 2: 0.38})

    attribute_1 = nw.new_node(Nodes.Attribute, attrs={"attribute_name": "spline parameter"})

    map_range_1 = nw.new_node(Nodes.MapRange, input_kwargs={"Value": attribute_1.outputs["Fac"], 1: 0.18, 2: 0.42})

    combine_xyz = nw.new_node(
        Nodes.CombineXYZ, input_kwargs={"X": map_range.outputs["Result"], "Y": map_range_1.outputs["Result"]}
    )
    ...
    add = nw.new_node(Nodes.Math, input_kwargs={0: separate_xyz.outputs["X"], 1: separate_xyz.outputs["Y"]})
    ...
    map_range_2 = nw.new_node(
        Nodes.MapRange, input_kwargs={"Value": voronoi_texture.outputs["Distance"], 2: 0.1, 3: 1.0, 4: 0.0}
    )

    maximum = nw.new_node(
        Nodes.Math, input_kwargs={0: add, 1: map_range_2.outputs["Result"]}, attrs={"operation": "MAXIMUM"}
    )
    ...
    map_range_4 = nw.new_node(Nodes.MapRange, input_kwargs={"Value": attribute_1.outputs["Fac"], 1: value, 2: add_1})

    minimum = nw.new_node(
        Nodes.Math, input_kwargs={0: maximum, 1: map_range_4.outputs["Result"]}, attrs={"operation": "MINIMUM"}
    )

    rgb = nw.new_node(Nodes.RGB)
    rgb.outputs[0].default_value = base_color

    hue_saturation_value = nw.new_node("ShaderNodeHueSaturation", input_kwargs={"Value": v, "Color": rgb})
    ...
    mix_1 = nw.new_node(Nodes.MixRGB, input_kwargs={"Fac": minimum, "Color1": hue_saturation_value, "Color2": group_2})
...
def nodegroup_dragonfly_tail(nw: NodeWrangler, base_color=(0.2789, 0.3864, 0.0319, 1.0), v=0.3, ring_length=0.3):
    # Code generated using version 2.4.3 of the node_transpiler
    ...
    reroute_1 = nw.new_node(Nodes.Reroute, input_kwargs={"Input": group_input.outputs["Segment Scale"]})
    ...
    map_range = nw.new_node(Nodes.MapRange, input_kwargs={"Value": capture_attribute.outputs[2], 3: 1.0, 4: 0.8})
    ...
    droplast = nw.new_node(nodegroup_droplast().name, input_kwargs={"Geometry": curve_to_points.outputs["Points"]})
    ...
    transform = nw.new_node(
        Nodes.Transform, input_kwargs={"Geometry": circlecrosssection, "Rotation": (0.0, 0.0, 1.5708)}
    )

    spline_parameter_1 = nw.new_node(Nodes.SplineParameter)

    subtract = nw.new_node(
        Nodes.Math, input_kwargs={0: spline_parameter_1.outputs["Factor"]}, attrs={"operation": "SUBTRACT"}
    )

    absolute = nw.new_node(Nodes.Math, input_kwargs={0: subtract}, attrs={"operation": "ABSOLUTE"})

    multiply_1 = nw.new_node(Nodes.Math, input_kwargs={0: absolute, 1: 2.0}, attrs={"operation": "MULTIPLY"})
    ...
    reroute = nw.new_node(Nodes.Reroute, input_kwargs={"Input": store_named_attribute_2})

    shapequadratic_001 = nw.new_node(
        nodegroup_shape_quadratic(
            radius_control_points=[(0.0, 0.3906), (0.1795, 0.4656), (0.5, 0.4563), (0.8795, 0.45), (1.0, 0.4344)]
        ).name,
        input_kwargs={
    ...
    join_geometry = nw.new_node(Nodes.JoinGeometry, input_kwargs={"Geometry": [instanceonpoints, instance_on_points]})

    set_material = nw.new_node(
        Nodes.SetMaterial,
        input_kwargs={
            "Geometry": join_geometry,
            "Material": surface.shaderfunc_to_material(shader_dragonfly_tail_shader, base_color, v, ring_length),
        },
    )
...
def nodegroup_droplast(nw: NodeWrangler):
    ...
    domain_size = nw.new_node(
        Nodes.DomainSize, input_kwargs={"Geometry": group_input.outputs["Geometry"]}, attrs={"component": "POINTCLOUD"}
    )

    subtract = nw.new_node(
        Nodes.Math, input_kwargs={0: domain_size.outputs["Point Count"], 1: 1.0}, attrs={"operation": "SUBTRACT"}
    )

    equal = nw.new_node(
        Nodes.Compare, input_kwargs={2: index, 3: subtract}, attrs={"data_type": "INT", "operation": "EQUAL"}
    )

    op_not = nw.new_node(Nodes.BooleanMath, input_kwargs={0: equal}, attrs={"operation": "NOT"})

    delete_geometry_1 = nw.new_node(
        Nodes.DeleteGeometry, input_kwargs={"Geometry": group_input.outputs["Geometry"], "Selection": op_not}
    )

    delete_geometry = nw.new_node(
        Nodes.DeleteGeometry, input_kwargs={"Geometry": group_input.outputs["Geometry"], "Selection": equal}
    )

    group_output = nw.new_node(Nodes.GroupOutput, input_kwargs={"Last": delete_geometry_1, "Others": delete_geometry})
...
def nodegroup_add_vertical_stripes(nw: NodeWrangler):
    ...
    multiply = nw.new_node(
        Nodes.Math, input_kwargs={0: separate_xyz.outputs["Z"], 1: 0.05}, attrs={"operation": "MULTIPLY"}
    )

    combine_xyz = nw.new_node(
        Nodes.CombineXYZ, input_kwargs={"X": absolute, "Y": separate_xyz.outputs["Y"], "Z": multiply}
    )
    ...
    reroute_1 = nw.new_node(Nodes.Reroute, input_kwargs={"Input": voronoi_texture.outputs["Distance"]})
    ...
    multiply_1 = nw.new_node(Nodes.Math, input_kwargs={0: reroute_1, 1: 0.1}, attrs={"operation": "MULTIPLY"})

    scale = nw.new_node(Nodes.VectorMath, input_kwargs={0: normal, "Scale": multiply_1}, attrs={"operation": "SCALE"})

    set_position = nw.new_node(
        Nodes.SetPosition, input_kwargs={"Geometry": store_named_attribute_3, "Offset": scale.outputs["Vector"]}
    )
