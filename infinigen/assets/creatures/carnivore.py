...
from infinigen.assets.creatures.util import cloth_sim, creature, genome
...
def tiger_genome():
    ...
    if U() < 0.5:
        ...
        genome.attach(
            jaw, head, coord=(0.2 * N(1, 0.1), 0, 0.35 * N(1, 0.1)), joint=Joint(rest=(0, U(10, 35), 0), pose=(0, 0, 0))
        )
    ...
    for side in [-1, 1]:
        back_leg = genome.attach(
            genome.part(foot_fac), genome.part(backleg_fac), coord=(0.9, 0, 0), joint=Joint(rest=(0, 0, 0))
        )
        genome.attach(
            back_leg,
            body,
    ...
    for side in [-1, 1]:
        front_leg = genome.attach(
            genome.part(foot_fac), genome.part(frontleg_fac), coord=(0.9, 0, 0), joint=Joint(rest=(0, 0, 0))
        )
        genome.attach(
            front_leg,
            body,
    ...
    genome.attach(
        head, body, coord=(N(0.97, 0.01), 0, 0), joint=Joint(rest=(0, N(20, 5), 0)), rotation_basis="global"
    )  # , bridge_rad=0.1)
...
class CarnivoreFactory(AssetFactory):
    ...
    def create_asset(self, i, placeholder, **kwargs):
        ...
        joined, extras, arma, ik_targets = joining.join_and_rig_parts(
            root, parts, genome, rigging=dynamic, postprocess_func=tiger_postprocessing, **kwargs
        )

        butil.parent_to(root, placeholder, no_inverse=True)

        if self.hair:
            creature_hair.configure_hair(joined, root, genome.postprocess_params["hair"], is_dynamic=dynamic)
