...
def nodegroup_carnivore_jaw(nw: NodeWrangler):
    ...
    node_utils.assign_curve(
        ...
        [(-1.0, -1.0), (0.0035999999999999999, 0.0), (0.24360000000000001, 0.20999999999999999), (1.0, 1.0)],
    )
    node_utils.assign_curve(
    ...
    greater_than = nw.new_node(Nodes.Compare, input_kwargs={0: group_input.outputs["Canine Length"]})
    ...
    switch_1 = nw.new_node(Nodes.Switch, input_kwargs={1: greater_than, 15: symmetric_clone.outputs["Both"]})

    greater_than_1 = nw.new_node(Nodes.Compare, input_kwargs={0: group_input.outputs["Incisor Size"]})
    ...
    switch = nw.new_node(Nodes.Switch, input_kwargs={1: greater_than_1, 15: realize_instances})

    join_geometry_1 = nw.new_node(
        Nodes.JoinGeometry, input_kwargs={"Geometry": [switch_1.outputs[6], switch.outputs[6]]}
    )
    ...
    quadratic_bezier_1 = nw.new_node(
        Nodes.QuadraticBezier, input_kwargs={"Resolution": 3, "Middle": (0.0, 0.69999999999999996, 0.0)}
    )
...
def nodegroup_carnivore_head(nw: NodeWrangler):
    ...
    combine_xyz = nw.new_node(
        Nodes.CombineXYZ, input_kwargs={"X": 1.0, "Y": group_input.outputs["snout_y_scale"], "Z": 1.0}
    )
    ...
    mammaleye = nw.new_node(nodegroup_mammal_eye().name, input_kwargs={"Radius": group_input.outputs["EyeRad"]})

    reroute_4 = nw.new_node(Nodes.Reroute, input_kwargs={"Input": group_input.outputs["length_rad1_rad2"]})

    separate_xyz = nw.new_node(Nodes.SeparateXYZ, input_kwargs={"Vector": reroute_4})

    reroute_3 = nw.new_node(Nodes.Reroute, input_kwargs={"Input": simple_tube_v2.outputs["Endpoint"]})
    ...
    symmetric_clone = nw.new_node(nodegroup_symmetric_clone().name, input_kwargs={"Geometry": transform_4})

    difference = nw.new_node(
        Nodes.MeshBoolean,
        input_kwargs={
            "Mesh 1": union.outputs["Mesh"],
            "Mesh 2": [attach_part.outputs["Geometry"], symmetric_clone.outputs["Both"]],
    ...
    symmetric_clone_1 = nw.new_node(nodegroup_symmetric_clone().name, input_kwargs={"Geometry": join_geometry_1})

    join_geometry_2 = nw.new_node(
        Nodes.JoinGeometry, input_kwargs={"Geometry": [difference.outputs["Mesh"], symmetric_clone_1.outputs["Both"]]}
    )
    ...
    symmetric_clone_2 = nw.new_node(nodegroup_symmetric_clone().name, input_kwargs={"Geometry": transform_3})
    ...
    symmetric_clone_3 = nw.new_node(nodegroup_symmetric_clone().name, input_kwargs={"Geometry": transform_5})
...
def nodegroup_neck(nw: NodeWrangler):
    ...
    rear_top = nw.new_node(
        ...
        input_kwargs={
            ...
            "StartRad, EndRad, Fullness": group_input.outputs["Muscle StartRad, EndRad, Fullness"],
            "ProfileHeight, StartTilt, EndTilt": group_input.outputs["ProfileHeight, StartTilt, EndTilt"],
        },
        label="Rear Top",
    )

    join_geometry_1 = nw.new_node(Nodes.JoinGeometry, input_kwargs={"Geometry": rear_top})
...
def nodegroup_flying_bird_head(nw: NodeWrangler):
    ...
    group_input = nw.new_node(
        Nodes.GroupInput,
        expose_input=[
            ("NodeSocketVector", "length_rad1_rad2", (0.34999999999999998, 0.11, 0.17000000000000001)),
