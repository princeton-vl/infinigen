...
class CrabClawFactory(CrabLegFactory):
    ...
    def make_claw(self, params):
        x_length, y_length, z_length, x_mid, y_mid = map(
            params.get, ["x_length", "y_length", "z_length", "x_mid_second", "y_mid_second"]
        )
        xs = x_mid, (x_mid + 1) / 2, (x_mid + 3) / 4, 1
        ys = y_mid, y_mid * params["claw_y_first"], y_mid * params["claw_y_second"], 0.01
        obj = spin(
        ...
        displace_vertices(
            ...
            lambda x, y, z: (
                ...
                -np.clip(
                    ...
                    + y_length * (y_mid - bottom_cutoff) * (x / x_length - x_mid) / claw_x_depth,
                    None,
                    0,
                )
        ...
        width_scale = interp1d(
            [x_mid, x_mid + claw_x_depth, x_mid + claw_x_depth + params["claw_x_turn"] * (1 - x_mid - claw_x_depth), 1],
        ...
        displace_vertices(
            ...
            lambda x, y, z: (
                0,
                0,
                np.where(z > 0, np.clip(params["top_cutoff"] * y_length - np.abs(y), 0, None) * params["top_shift"], 0),
            ),
        )
        z = read_co(obj).T[-1]
        write_attr_data(obj, "ratio", 1 + np.where(z > 0, 0, uniform(0.5, 1.0) * z / params["y_length"]))
        ...
        def selection(nw: NodeWrangler):
            ...
            center = nw.compare("LESS_THAN", nw.math("ABSOLUTE", y), params["y_length"] * 0.5)
            return nw.boolean_math("AND", nw.boolean_math("AND", lower, x_range), center)

        temp = butil.spawn_vert("temp")
        surface.add_geomod(
            temp, geo_base_selection, apply=True, input_args=[obj, selection, params["claw_spike_distance"]]
        )
