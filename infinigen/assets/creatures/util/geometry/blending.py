...
from infinigen.core.util.math import (
    FixedSeed,
    affine_from_new_domain,
    new_domain_from_affine,
    normalize,
...
class CurveND:
    ...
    def evaluate_points_and_derivatives_at_t(
        ...
        if points.shape != t.shape + (self.dim,):
            raise ValueError(f"points {points.shape} has wrong shape, {t.shape}")
...
class Curve2DFactory:
    ...
    def circle(center, start_pos, arc_angle=2 * np.pi) -> CurveND:
        ...
        def eval_fn(t):
            uv = np.stack(
                [
                    np.cos(t * arc_angle + start_angle) * rad + center[0],
                    np.sin(t * arc_angle + start_angle) * rad + center[1],
                ],
                axis=-1,
            )
            duvdt = (
    ...
    def nurbs(
        ...
        if ctrlpts.shape[-1] < 2:
            raise ValueError(f"control points have wrong shape {ctrlpts.shape}")
...
class UVMesh:
    def __init__(
        ...
        self._rtree_idx = rtree.index.Index(
            (
                (i, x.bounds, None)
                for ps in [self._polygons, self._polygons_rshift, self._polygons_lshift]
    ...
    def export_uvmesh(self):
        ...
        edges = [
            (new_ids[pt1], new_ids[pt2])
            for pt1 in range(len(self._uvpoints))
    ...
    def _enclosing_faces_polys_edges_verts_of_point(self, coords):
        ...
        e_verts = [
            ...
            for pt in self._faces[f]
            if (np.array(coords) == np.array(self._uvpoints[pt])).all()
        ]
    ...
    def _print_all(self):
        ...
        print("polly lshift")
        print([list(p.exterior.coords) for p in self._polygons_lshift if p is not None])

        print("polly rshift")
        print([list(p.exterior.coords) for p in self._polygons_rshift if p is not None])
    ...
    def _add_edge(self, pt1, pt2, cross_dir=0):
        ...
        if self._cyclic_v and cross_dir != 0:
            self._pos_cross_edges.add((pt1, pt2) if cross_dir > 0 else (pt2, pt1))
    ...
    def _delete_edge_and_merge_faces(self, pt1, pt2):
        ...
        if len(set(f1_pts).intersection(set(f2_pts))) > 2:
            raise ValueError("cannot merge faces that share more than one edge")

        new_f_pts = (
            list(np.roll(f1_pts, -np.argmax(f1_pts == pt2)))[:-1]
            + list(np.roll(f2_pts, -np.argmax(f2_pts == pt1)))[:-1]
        )
    ...
    def _triangulate_face_from_pt(self, face_id, pt):
        ...
        for pt2 in r_pts[2:-1]:
            ...
            if poly.covers(line) and not line.covers(Point(pt1_coords)):
                new_f0, new_f = self._split_face_with_new_edge(face_id, pt, pt2)
    ...
    def _split_edge_with_new_point(
        ...
        cross_dir1 = self._get_cross_dir_from_pt(pt1, enclosing_face, enclosing_polygon)
        cross_dir2 = self._get_cross_dir_from_pt(pt2, enclosing_face, enclosing_polygon)
        self._add_edge(pt1, new_pt, cross_dir1)
        self._add_edge(pt2, new_pt, cross_dir2)
        ...
        f1_pts.insert(f1_pts.index(pt1) + 1, new_pt) if f1 is not None else None
        f2_pts.insert(f2_pts.index(pt2) + 1, new_pt) if f2 is not None else None
        if f1 is not None:
    ...
    def _within_domain(self, uv):
        uv = np.array(uv)
        return (
            (self._domain[0][0] <= uv[..., 0]).all()
            and (uv[..., 0] < self._domain[0][1]).all()
            and (self._domain[1][0] <= uv[..., 1]).all()
            and (uv[..., 1] < self._domain[1][1]).all()
        )
    ...
    def add_edge_and_remesh(self, pt1, pt2, cross_dir=0):
        ...
        if len(shared_f) > 0:
            ...
            f1, f2 = self._split_face_with_new_edge(list(shared_f)[0], pt1, pt2)
            self._triangulate_face(f1)
            self._triangulate_face(f2)
            return
        ...
        for f in self._faces_of_point(pt1):
            ...
            line = LineString([poly.exterior.coords[f_pts.index(pt1)], pt2_coords])
...
class Surface:
    ...
    def affine_transform_domain(self, ua=1, ub=0, va=1, vb=0) -> Surface:
        ...
        def new_eval_fn(uv):
            fuv = np.stack([ua * uv[..., 0] + ub, va * uv[..., 1] + vb], axis=-1)
    ...
    def evaluate_points_and_derivatives_at_uv(
        ...
        def check_domain(u, domain, cyclic, name):
            ...
            if (u < domain[0]).any() or (u > domain[1]).any():
                raise ValueError(f"{name} out of domain, {domain}, {self.domain}")
        ...
        eval_uv = uv.copy()
        if self.cyclic_u:
            eval_uv[..., 0] = wrap_around_cyclic_coord(uv[..., 0], *self.domain[0])
        if self.cyclic_v:
            eval_uv[..., 1] = wrap_around_cyclic_coord(uv[..., 1], *self.domain[1])
...
class SurfaceFactory:
    ...
    def plane(center, normal, domain=((-1, 1), (-1, 1))) -> Surface:
        ...
        def eval_fn(uv):
            points = np.concatenate([uv, np.zeros(uv.shape[:-1] + (1,))], axis=-1)
            dsdu = np.zeros_like(points)
            dsdu[..., 0] = 1
            dsdv = np.zeros_like(points)
            dsdv[..., 1] = 1
            upward = np.array([0, 0, 1])
            rot_mat = np.squeeze(rotate_match_directions(upward[None], normal[None]))
            points = np.einsum("...ij,...j->...i", rot_mat, points) + center[None]
    ...
    def blending(
        ...
        def eval_fn(st):
            ...
            k = c2 - c1
            T1 = _compute_blending_tangent(dc1dt, k, z1, alpha1, w1, sweep_left[0])
            T2 = _compute_blending_tangent(dc2dt, k, z2, alpha2, w2, sweep_left[1])
...
def example_use3():
    ...
    resolution_t = 50
    with FixedSeed(1103):
        uv_mesh = UVMesh.from_meshgrid(resolution_s, resolution_t, cyclic_v=True)
        ...
        curve2 = Curve2DFactory.nurbs(np.array([[0.5, 0.1], [0.5, 1.1]]), degree=1)
