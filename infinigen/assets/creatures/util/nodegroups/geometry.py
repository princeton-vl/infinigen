...
def nodegroup_symmetric_instance(nw: NodeWrangler):
    ...
    instance_on_points = nw.new_node(
        Nodes.InstanceOnPoints, input_kwargs={"Points": mesh_line, "Instance": group_input.outputs["Geometry"]}
    )

    index = nw.new_node(Nodes.Index)

    equal = nw.new_node(Nodes.Compare, input_kwargs={2: index}, attrs={"data_type": "INT", "operation": "EQUAL"})

    scale_instances = nw.new_node(
        Nodes.ScaleInstances, input_kwargs={"Instances": instance_on_points, "Selection": equal}
    )

    flip_faces = nw.new_node(Nodes.FlipFaces, input_kwargs={"Mesh": scale_instances, "Selection": equal})

    group_output = nw.new_node(Nodes.GroupOutput, input_kwargs={"Instances": flip_faces})
