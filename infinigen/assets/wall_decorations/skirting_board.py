...
from infinigen.assets.utils.shapes import obj2polygon, polygon2obj
...
def nodegroup_make_skirting_board(nw: NodeWrangler, control_points):
    ...
    multiply_3 = nw.new_node(
        Nodes.Math, input_kwargs={0: float_curve, 1: group_input.outputs["Thickness"]}, attrs={"operation": "MULTIPLY"}
    )
...
def apply_skirtingboard(nw: NodeWrangler, contour, is_ceiling=False, seed=None, thickness=0.02):
    # Code generated using version 2.6.5 of the node_transpiler
...
def make_skirtingboard_contour(objs: list[bpy.types.Object], tag: t.Subpart):
    ...
    objs = [tagging.extract_tagged_faces(o, {tag, t.Subpart.Visible}, nonempty=True) for o in list(objs)]
    ...
    boundary = unary_union(all_polys).buffer(0.05, join_style="mitre").buffer(-0.05, join_style="mitre")
...
def make_skirting_board(objs, tag, joined=True):
    ...
    for s in seqs:
        ...
        kwargs = {"contour": col, "seed": np.random.randint(1e7), "is_ceiling": tag == t.Subpart.Ceiling}
        ...
        for p in portal_cutters:
            if p.name.startswith("entrance") and int(p.location[-1] / WALL_HEIGHT - 1 / 2) == 0:
                ...
                butil.modify_mesh(
                    obj, "BOOLEAN", object=p, operation="DIFFERENCE", use_self=True, use_hole_tolerant=True
                )
                p.location[-1] += WALL_HEIGHT / 2
...
def linear_ring2curve(ring, reversed=False):
    ...
    invalid = np.sort(np.nonzero((np.abs(lengths - WALL_THICKNESS) < 0.02) | (np.abs(lengths - DOOR_WIDTH) < 0.02))[0])
