...
from infinigen.assets.utils.misc import assign_material, sample_direction, toggle_hide, toggle_show
...
def build_spikes(base_radius=0.002, **kwargs):
    ...
    branch_config = {
        "n": n_branch,
        "path_kargs": lambda idx: {"n_pts": n_major, "std": 0.5, "momentum": 0.85, "sz": uniform(0.005, 0.01)},
        "spawn_kargs": lambda idx: {"init_vec": sample_direction(0.8)},
    }

    radius_fn = lambda base_radius, size, resolution: base_radius * 0.5 ** (
...
def make_default_selections(spike_distance, cap_percentage, density):
    def selection(nw: NodeWrangler, selected, geometry):
        ...
        is_cap = nw.bernoulli(nw.build_float_curve(percentage, [(0, 1), (cap_percentage, 0.5), (1, 0)]))
        cap = nw.new_node(Nodes.SeparateGeometry, [geometry, is_cap])
        cap = nw.new_node(Nodes.MergeByDistance, [cap, None, spike_distance / 2])

        points = nw.new_node(
            Nodes.DistributePointsOnFaces, input_kwargs={"Mesh": geometry, "Selection": selected, "Density": density}
        ).outputs["Points"]
...
def geo_spikes(nw: NodeWrangler, spikes, points_fn=None, realize=True):
    geometry, selection = nw.new_node(
        Nodes.GroupInput,
        expose_input=[("NodeSocketGeometry", "Geometry", None), ("NodeSocketFloat", "Selection", None)],
    ).outputs[:2]

    capture = nw.new_node(
        Nodes.CaptureAttribute, input_kwargs={"Geometry": geometry, "Value": nw.new_node(Nodes.InputNormal)}
    )
    ...
    rotation = nw.new_node(
        Nodes.AlignEulerToVector, input_kwargs={"Vector": (capture, "Attribute")}, attrs={"axis": "Z"}
    )
    rotation = nw.new_node(
        Nodes.RotateEuler,
    ...
    rotation = nw.new_node(Nodes.AlignEulerToVector, [rotation, nw.uniform(0.2, 0.5)], attrs={"axis": "Z"})
...
def shader_spikes(nw: NodeWrangler):
    ...
    principled_bsdf = nw.new_node(
        ...
        input_kwargs={"Base Color": color, "Roughness": roughness, "Specular": specular, "Subsurface": 0.1},
    )
