...
class GlobularBaseCactusFactory(BaseCactusFactory):
    ...
    def geo_globular(nw: NodeWrangler):
        ...
        circle = nw.new_node(
            Nodes.SetPosition, [circle, selection, nw.scale(nw.new_node(Nodes.InputPosition), uniform(1.1, 1.2))]
        )
        ...
        radius = nw.scalar_multiply(
            nw.build_float_curve(nw.new_node(Nodes.SplineParameter), anchors, "AUTO"), log_uniform(0.5, 1.0)
        )
