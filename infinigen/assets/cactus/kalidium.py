...
class KalidiumBaseCactusFactory(BaseCactusFactory):
    ...
    def build_twig(i):
        branch_config = {
            "n": 1,
            "path_kargs": lambda idx: {"n_pts": 5, "std": 0.5, "momentum": 0.85, "sz": 0.01},
    ...
    def create_asset(self, face_size=0.01, **params) -> bpy.types.Object:
        ...
        butil.modify_mesh(obj, "ARRAY", count=resolution, relative_offset_displace=(1, 0, 0), use_merge_vertices=True)
        butil.modify_mesh(obj, "ARRAY", count=resolution, relative_offset_displace=(0, 1, 0), use_merge_vertices=True)
        butil.modify_mesh(obj, "ARRAY", count=resolution, relative_offset_displace=(0, 0, 1), use_merge_vertices=True)
        obj.scale = [1 / resolution] * 3
        obj.location = -1, -1, -0.1
        butil.apply_transform(obj, loc=True)
        remove_vertices(obj, lambda x, y, z: (x**2 + y**2 + (z - 1) ** 2 > 1.1) | (uniform(0, 1, len(x)) < 0.05))
        ...
        displace_vertices(obj, lambda x, y, z: uniform(-0.8 / resolution, 0.8 / resolution, (3, len(x))))
        with butil.ViewportMode(obj, "EDIT"):
            bpy.ops.mesh.quads_convert_to_tris(quad_method="BEAUTY", ngon_method="BEAUTY")
        surface.add_geomod(obj, geo_extension, apply=True)

        weight = lambda nw: nw.scalar_multiply(
            nw.vector_math("DISTANCE", *nw.new_node(Nodes.InputEdgeVertices).outputs[2:]), nw.uniform(0.8, 1)
        )
        surface.add_geomod(obj, geo_shortest_path, apply=True, input_args=[end_index, weight, 0.05])
    ...
    def geo_twigs(nw: NodeWrangler, instances):
        ...
        rotation = nw.new_node(
            ...
            [nw.add(rotation, nw.uniform([-perturb] * 3, [perturb] * 3)), nw.uniform(0.2, 0.5)],
            attrs={"axis": "Z"},
        )
        ...
        twigs = nw.new_node(
            ...
            [
                nw.new_node(
                    ...
                    [points, None, instances, True, None, rotation, nw.combine(1, 1, nw.uniform(1.0, 1.5))],
                )
            ],
        )
