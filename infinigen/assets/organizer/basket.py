...
def nodegroup_holes(nw: NodeWrangler):
    ...
    subtract = nw.new_node(
        Nodes.Math, input_kwargs={0: group_input.outputs["Value1"], 1: add}, attrs={"operation": "SUBTRACT"}
    )
    ...
    transform_1 = nw.new_node(
        Nodes.Transform, input_kwargs={"Geometry": store_named_attribute, "Rotation": (0.0000, 1.5708, 0.0000)}
    )
    ...
    instance_on_points = nw.new_node(
        Nodes.InstanceOnPoints, input_kwargs={"Points": transform_1, "Instance": store_named_attribute_1}
    )
    ...
    transform_2 = nw.new_node(
        Nodes.Transform, input_kwargs={"Geometry": store_named_attribute_2, "Rotation": (1.5708, 0.0000, 0.0000)}
    )
    ...
    instance_on_points_1 = nw.new_node(
        Nodes.InstanceOnPoints, input_kwargs={"Points": transform_2, "Instance": store_named_attribute_3}
    )
...
def nodegroup_handle_hole(nw: NodeWrangler):
    ...
    combine_xyz_3 = nw.new_node(
        Nodes.CombineXYZ, input_kwargs={"X": group_input.outputs["X"], "Y": 1.0000, "Z": group_input.outputs["Z"]}
    )
    ...
    subdivision_surface_2 = nw.new_node(
        Nodes.SubdivisionSurface, input_kwargs={"Mesh": subdivide_mesh_2, "Level": group_input.outputs["Level"]}
    )

    multiply = nw.new_node(Nodes.Math, input_kwargs={0: group_input.outputs["Value"]}, attrs={"operation": "MULTIPLY"})

    subtract = nw.new_node(
        Nodes.Math, input_kwargs={0: multiply, 1: group_input.outputs["Value2"]}, attrs={"operation": "SUBTRACT"}
    )

    combine_xyz_4 = nw.new_node(Nodes.CombineXYZ, input_kwargs={"Z": subtract})

    transform_1 = nw.new_node(
        Nodes.Transform, input_kwargs={"Geometry": subdivision_surface_2, "Translation": combine_xyz_4}
    )
...
def geometry_nodes(nw: NodeWrangler, **kwargs):
    ...
    subdivision_surface = nw.new_node(
        Nodes.SubdivisionSurface, input_kwargs={"Mesh": subdivide_mesh, "Level": sub_level}
    )
    ...
    subdivision_surface_1 = nw.new_node(
        Nodes.SubdivisionSurface, input_kwargs={"Mesh": subdivide_mesh_1, "Level": sub_level}
    )
    ...
    transform = nw.new_node(
        Nodes.Transform, input_kwargs={"Geometry": subdivision_surface_1, "Translation": combine_xyz_2}
    )
    ...
    difference = nw.new_node(
        Nodes.MeshBoolean, input_kwargs={"Mesh 1": subdivision_surface, "Mesh 2": [transform] + differences}
    )
    ...
    transform_geometry = nw.new_node(
        Nodes.Transform, input_kwargs={"Geometry": realize_instances, "Translation": combine_xyz_3}
    )

    set_material = nw.new_node(
        Nodes.SetMaterial,
        input_kwargs={"Geometry": transform_geometry, "Material": surface.shaderfunc_to_material(shader_rough_plastic)},
    )
...
class BasketBaseFactory(AssetFactory):
    ...
    def get_asset_params(self, i=0):
        ...
        if params.get("handle_dist_to_top", None) is None:
            params["handle_dist_to_top"] = params["handle_height"] * 0.5 + params["height"] * uniform(0.08, 0.15)
