...
def nodegroup_polar_to_cart_old(nw):
    ...
    cosine = nw.new_node(Nodes.Math, input_kwargs={0: group_input.outputs["Value"]}, attrs={"operation": "COSINE"})

    sine = nw.new_node(Nodes.Math, input_kwargs={0: group_input.outputs["Value"]}, attrs={"operation": "SINE"})
    ...
    group_output = nw.new_node(Nodes.GroupOutput, input_kwargs={"Vector": multiply_add.outputs["Vector"]})
...
def nodegroup_follow_curve(nw):
    ...
    curve_length = nw.new_node(Nodes.CurveLength, input_kwargs={"Curve": group_input.outputs["Curve"]})

    multiply = nw.new_node(
        Nodes.Math, input_kwargs={0: map_range.outputs["Result"], 1: curve_length}, attrs={"operation": "MULTIPLY"}
    )
...
def nodegroup_norm_index(nw):
    ...
    group_input = nw.new_node(Nodes.GroupInput, expose_input=[("NodeSocketInt", "Count", 0)])

    divide = nw.new_node(
        Nodes.Math, input_kwargs={0: index, 1: group_input.outputs["Count"]}, attrs={"operation": "DIVIDE"}
    )
...
def nodegroup_flower_petal(nw):
    ...
    grid = nw.new_node(
        Nodes.MeshGrid, input_kwargs={"Vertices X": group_input.outputs["Resolution V"], "Vertices Y": multiply_add}
    )

    position = nw.new_node(Nodes.InputPosition)

    capture_attribute = nw.new_node(
        Nodes.CaptureAttribute, input_kwargs={"Geometry": grid, 1: position}, attrs={"data_type": "FLOAT_VECTOR"}
    )

    separate_xyz = nw.new_node(Nodes.SeparateXYZ, input_kwargs={"Vector": capture_attribute.outputs["Attribute"]})

    multiply = nw.new_node(
        Nodes.Math, input_kwargs={0: separate_xyz.outputs["X"], 1: 0.05}, attrs={"operation": "MULTIPLY"}
    )

    combine_xyz = nw.new_node(Nodes.CombineXYZ, input_kwargs={"X": multiply, "Y": separate_xyz.outputs["Y"]})
    ...
    multiply_1 = nw.new_node(
        Nodes.Math, input_kwargs={0: add, 1: group_input.outputs["Wrinkle"]}, attrs={"operation": "MULTIPLY"}
    )

    separate_xyz_1 = nw.new_node(Nodes.SeparateXYZ, input_kwargs={"Vector": capture_attribute.outputs["Attribute"]})

    add_1 = nw.new_node(Nodes.Math, input_kwargs={0: separate_xyz_1.outputs["X"]})

    absolute = nw.new_node(Nodes.Math, input_kwargs={0: separate_xyz_1.outputs["Y"]}, attrs={"operation": "ABSOLUTE"})

    multiply_2 = nw.new_node(Nodes.Math, input_kwargs={0: absolute, 1: 2.0}, attrs={"operation": "MULTIPLY"})

    power = nw.new_node(
        Nodes.Math, input_kwargs={0: multiply_2, 1: group_input.outputs["Bevel"]}, attrs={"operation": "POWER"}
    )

    multiply_add_1 = nw.new_node(
        Nodes.Math, input_kwargs={0: power, 1: -1.0, 2: 1.0}, attrs={"operation": "MULTIPLY_ADD"}
    )

    multiply_3 = nw.new_node(Nodes.Math, input_kwargs={0: add_1, 1: multiply_add_1}, attrs={"operation": "MULTIPLY"})
    ...
    multiply_4 = nw.new_node(
        Nodes.Math, input_kwargs={0: separate_xyz_1.outputs["Y"], 1: multiply_add_2}, attrs={"operation": "MULTIPLY"}
    )

    power_1 = nw.new_node(
        Nodes.Math, input_kwargs={0: absolute, 1: group_input.outputs["Point"]}, attrs={"operation": "POWER"}
    )

    multiply_add_3 = nw.new_node(
        Nodes.Math, input_kwargs={0: power_1, 1: -1.0, 2: 1.0}, attrs={"operation": "MULTIPLY_ADD"}
    )
    ...
    multiply_6 = nw.new_node(Nodes.Math, input_kwargs={0: add_2, 1: multiply_add_1}, attrs={"operation": "MULTIPLY"})

    multiply_7 = nw.new_node(Nodes.Math, input_kwargs={0: add_1, 1: multiply_6}, attrs={"operation": "MULTIPLY"})

    combine_xyz_1 = nw.new_node(Nodes.CombineXYZ, input_kwargs={"X": multiply_1, "Y": multiply_4, "Z": multiply_7})
    ...
    reroute = nw.new_node(Nodes.Reroute, input_kwargs={"Input": group_input.outputs["Curl"]})
    ...
    group_output = nw.new_node(Nodes.GroupOutput, input_kwargs={"Geometry": tag_nodegroup(nw, group, "petal")})
...
def nodegroup_phyllo_points(nw):
    ...
    cosine = nw.new_node(Nodes.Math, input_kwargs={0: index}, attrs={"operation": "COSINE"})
    ...
    power = nw.new_node(
        Nodes.Math, input_kwargs={0: divide, 1: group_input.outputs["Radius exp"]}, attrs={"operation": "POWER"}
    )
    ...
    multiply = nw.new_node(
        Nodes.VectorMath, input_kwargs={0: combine_xyz, 1: map_range.outputs["Result"]}, attrs={"operation": "MULTIPLY"}
    )
    ...
    add = nw.new_node(Nodes.Math, input_kwargs={0: index, 1: group_input.outputs["Yaw offset"]})

    combine_xyz_2 = nw.new_node(
        Nodes.CombineXYZ, input_kwargs={"X": map_range_3.outputs["Result"], "Y": random_value.outputs[1], "Z": add}
    )

    group_output = nw.new_node(Nodes.GroupOutput, input_kwargs={"Points": set_position, "Rotation": combine_xyz_2})
...
def nodegroup_plant_seed(nw):
    ...
    separate_xyz = nw.new_node(Nodes.SeparateXYZ, input_kwargs={"Vector": group_input.outputs["Dimensions"]})

    combine_xyz = nw.new_node(Nodes.CombineXYZ, input_kwargs={"X": separate_xyz.outputs["X"]})

    multiply_add = nw.new_node(
        Nodes.VectorMath, input_kwargs={0: combine_xyz, 1: (0.5, 0.5, 0.5)}, attrs={"operation": "MULTIPLY_ADD"}
    )
    ...
    group = nw.new_node(nodegroup_norm_index().name, input_kwargs={"Count": group_input.outputs["U"]})
    ...
    set_curve_radius = nw.new_node(
        Nodes.SetCurveRadius, input_kwargs={"Curve": quadratic_bezier_1, "Radius": map_range.outputs["Result"]}
    )

    curve_circle = nw.new_node(
        Nodes.CurveCircle, input_kwargs={"Resolution": group_input.outputs["V"], "Radius": separate_xyz.outputs["Y"]}
    )
    ...
    group_output = nw.new_node(Nodes.GroupOutput, input_kwargs={"Mesh": tag_nodegroup(nw, curve_to_mesh, "seed")})
...
def shader_petal(nw, petal_color_name):
    ...
    translucent_color = nw.new_node(
        Nodes.MixRGB, [translucent_color_change, petal_color, color.color_category(petal_color_name)]
    )

    translucent_bsdf = nw.new_node(Nodes.TranslucentBSDF, input_kwargs={"Color": translucent_color})

    principled_bsdf = nw.new_node(
        Nodes.PrincipledBSDF, input_kwargs={"Base Color": petal_color, "Specular": specular, "Roughness": roughness}
    )

    mix_shader = nw.new_node(
        Nodes.MixShader, input_kwargs={"Fac": translucent_amt, 1: principled_bsdf, 2: translucent_bsdf}
    )
...
def geo_flower(nw, petal_material, center_material):
    ...
    uv_sphere = nw.new_node(
        Nodes.MeshUVSphere, input_kwargs={"Segments": 8, "Rings": 8, "Radius": group_input.outputs["Center Rad"]}
    )

    transform = nw.new_node(Nodes.Transform, input_kwargs={"Geometry": uv_sphere, "Scale": (1.0, 1.0, 0.05)})

    multiply = nw.new_node(
        Nodes.Math, input_kwargs={0: group_input.outputs["Seed Size"], 1: 1.5}, attrs={"operation": "MULTIPLY"}
    )
    ...
    multiply_1 = nw.new_node(
        Nodes.Math, input_kwargs={0: group_input.outputs["Seed Size"], 1: 10.0}, attrs={"operation": "MULTIPLY"}
    )

    combine_xyz = nw.new_node(Nodes.CombineXYZ, input_kwargs={"X": multiply_1, "Y": group_input.outputs["Seed Size"]})

    group_3 = nw.new_node(nodegroup_plant_seed().name, input_kwargs={"Dimensions": combine_xyz, "U": 6, "V": 6})

    musgrave_texture = nw.new_node(
        Nodes.MusgraveTexture, input_kwargs={"W": 13.8, "Scale": 2.41}, attrs={"musgrave_dimensions": "4D"}
    )

    map_range = nw.new_node(Nodes.MapRange, input_kwargs={"Value": musgrave_texture, 3: 0.34, 4: 1.21})

    combine_xyz_1 = nw.new_node(Nodes.CombineXYZ, input_kwargs={"X": map_range.outputs["Result"], "Y": 1.0, "Z": 1.0})
    ...
    join_geometry_1 = nw.new_node(Nodes.JoinGeometry, input_kwargs={"Geometry": [realize_instances, transform]})

    set_material_1 = nw.new_node(
        Nodes.SetMaterial, input_kwargs={"Geometry": join_geometry_1, "Material": center_material}
    )

    multiply_2 = nw.new_node(
        Nodes.Math, input_kwargs={0: group_input.outputs["Center Rad"], 1: 6.2832}, attrs={"operation": "MULTIPLY"}
    )

    separate_xyz = nw.new_node(Nodes.SeparateXYZ, input_kwargs={"Vector": group_input.outputs["Petal Dims"]})

    divide = nw.new_node(
        Nodes.Math, input_kwargs={0: multiply_2, 1: separate_xyz.outputs["Y"]}, attrs={"operation": "DIVIDE"}
    )

    multiply_3 = nw.new_node(Nodes.Math, input_kwargs={0: divide, 1: 1.2}, attrs={"operation": "MULTIPLY"})

    reroute_3 = nw.new_node(Nodes.Reroute, input_kwargs={"Input": group_input.outputs["Center Rad"]})

    reroute_1 = nw.new_node(Nodes.Reroute, input_kwargs={"Input": group_input.outputs["Min Petal Angle"]})

    reroute = nw.new_node(Nodes.Reroute, input_kwargs={"Input": group_input.outputs["Max Petal Angle"]})
    ...
    reroute_2 = nw.new_node(Nodes.Reroute, input_kwargs={"Input": group_input.outputs["Wrinkle"]})

    reroute_4 = nw.new_node(Nodes.Reroute, input_kwargs={"Input": group_input.outputs["Curl"]})
    ...
    realize_instances_1 = nw.new_node(Nodes.RealizeInstances, input_kwargs={"Geometry": instance_on_points})

    noise_texture = nw.new_node(Nodes.NoiseTexture, input_kwargs={"Scale": 3.73, "Detail": 5.41, "Distortion": -1.0})
    ...
    multiply_4 = nw.new_node(
        Nodes.VectorMath, input_kwargs={0: subtract_1.outputs["Vector"], 1: value}, attrs={"operation": "MULTIPLY"}
    )

    set_position = nw.new_node(
        Nodes.SetPosition, input_kwargs={"Geometry": realize_instances_1, "Offset": multiply_4.outputs["Vector"]}
    )

    set_material = nw.new_node(Nodes.SetMaterial, input_kwargs={"Geometry": set_position, "Material": petal_material})
