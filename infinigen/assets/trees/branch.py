...
def nodegroup_generate_anchor(nw: NodeWrangler):
    ...
    divide = nw.new_node(
        Nodes.Math, input_kwargs={0: 1.0000, 1: group_input.outputs["density"]}, attrs={"operation": "DIVIDE"}
    )

    multiply = nw.new_node(
        Nodes.Math, input_kwargs={0: divide, 1: group_input.outputs["keep probablity"]}, attrs={"operation": "MULTIPLY"}
    )
    ...
    greater_than = nw.new_node(
        Nodes.Compare, input_kwargs={0: group_input.outputs["curve parameter"], 1: group_input.outputs["trim_bottom"]}
    )
    ...
    delete_geometry = nw.new_node(
        Nodes.DeleteGeometry, input_kwargs={"Geometry": curve_to_points_1.outputs["Points"], "Selection": op_not}
    )

    group_output = nw.new_node(
        Nodes.GroupOutput, input_kwargs={"Points": delete_geometry}, attrs={"is_active_output": True}
    )
...
def nodegroup_create_instance(nw: NodeWrangler):
    ...
    radians = nw.new_node(
        Nodes.Math, input_kwargs={0: group_input.outputs["Rot x deg"]}, attrs={"operation": "RADIANS"}
    )

    subtract = nw.new_node(
        Nodes.Math, input_kwargs={0: 1.0000, 1: group_input.outputs["Rot x range"]}, attrs={"operation": "SUBTRACT"}
    )
    ...
    random_value_2 = nw.new_node(
        Nodes.RandomValue, input_kwargs={2: multiply, 3: multiply_1, "Seed": group_input.outputs["Seed"]}
    )
    ...
    group_output = nw.new_node(
        Nodes.GroupOutput, input_kwargs={"Instances": rotate_instances}, attrs={"is_active_output": True}
    )
...
def generate_branch(nw: NodeWrangler, **kwargs):
    ...
    combine_xyz = nw.new_node(
        Nodes.CombineXYZ, input_kwargs={"X": spline_parameter.outputs["Factor"], "Y": kwargs["seed"]}
    )
    ...
    set_position = nw.new_node(
        Nodes.SetPosition, input_kwargs={"Geometry": resample_curve, "Offset": scale_1.outputs["Vector"]}
    )
    ...
    divide = nw.new_node(
        Nodes.Math, input_kwargs={0: kwargs["twig density"], 1: keep_probablity}, attrs={"operation": "DIVIDE"}
    )
    ...
    divide_1 = nw.new_node(Nodes.Math, input_kwargs={0: kwargs["resolution"], 1: 2.0000}, attrs={"operation": "DIVIDE"})
    ...
    capture_attribute_1 = nw.new_node(
        Nodes.CaptureAttribute, input_kwargs={"Geometry": resample_curve_2, 2: spline_parameter_1.outputs["Factor"]}
    )

    add_1 = nw.new_node(Nodes.Math, input_kwargs={0: kwargs["seed"], 1: 37.0000})

    random_value = nw.new_node(
        Nodes.RandomValue, input_kwargs={"Probability": keep_probablity, "Seed": add_1}, attrs={"data_type": "BOOLEAN"}
    )
    ...
    greater_equal = nw.new_node(
        Nodes.Compare, input_kwargs={2: index, 3: multiply}, attrs={"data_type": "INT", "operation": "GREATER_EQUAL"}
    )

    multiply_1 = nw.new_node(Nodes.Math, input_kwargs={0: divide, 1: 0.9000}, attrs={"operation": "MULTIPLY"})

    less_equal = nw.new_node(
        Nodes.Compare, input_kwargs={2: index, 3: multiply_1}, attrs={"data_type": "INT", "operation": "LESS_EQUAL"}
    )
    ...
    multiply_2 = nw.new_node(
        Nodes.Math, input_kwargs={0: kwargs["twig rotation"], 1: -1.0000}, attrs={"operation": "MULTIPLY"}
    )

    map_range_2 = nw.new_node(
        Nodes.MapRange, input_kwargs={"Value": capture_attribute.outputs[2], 3: 1.0000, 4: 0.1000}
    )
    ...
    set_position_1 = nw.new_node(
        Nodes.SetPosition, input_kwargs={"Geometry": realize_instances, "Offset": scale_3.outputs["Vector"]}
    )
    ...
    map_range_1 = nw.new_node(
        Nodes.MapRange, input_kwargs={"Value": capture_attribute.outputs[2], 3: 1.0000, 4: 0.4000}
    )
    ...
    set_curve_radius = nw.new_node(
        Nodes.SetCurveRadius, input_kwargs={"Curve": capture_attribute, "Radius": multiply_4}
    )

    multiply_5 = nw.new_node(
        Nodes.Math, input_kwargs={0: kwargs["resolution"], 1: kwargs["overall radius"]}, attrs={"operation": "MULTIPLY"}
    )
    ...
    map_range_4 = nw.new_node(
        Nodes.MapRange, input_kwargs={"Value": capture_attribute_1.outputs[2], 3: 0.8000, 4: 0.1000}
    )
    ...
    multiply_8 = nw.new_node(
        Nodes.Math, input_kwargs={0: multiply_7, 1: kwargs["overall radius"]}, attrs={"operation": "MULTIPLY"}
    )

    set_curve_radius_1 = nw.new_node(
        Nodes.SetCurveRadius, input_kwargs={"Curve": capture_attribute_2.outputs["Geometry"], "Radius": multiply_8}
    )
    ...
    set_material = nw.new_node(
        Nodes.SetMaterial, input_kwargs={"Geometry": join_geometry, "Material": kwargs["material"]}
    )

    surfacebump = nw.new_node(
        nodegroup_surface_bump().name, input_kwargs={"Geometry": set_material, "Displacement": 0.0050}
    )

    join_geometry_1 = nw.new_node(
        Nodes.JoinGeometry, input_kwargs={"Geometry": [createinstance, createinstance_2, surfacebump]}
    )

    transform = nw.new_node(
        Nodes.Transform, input_kwargs={"Geometry": join_geometry_1, "Rotation": (-1.5708, 0.0000, 0.0000)}
    )
