...
class RottenTreeFactory(BaseDeformedTreeFactory):
    ...
    def geo_cutter(nw: NodeWrangler, strength, scale, metric_fn):
        ...
        offset = nw.scalar_multiply(
            nw.new_node(
                ...
                [
                    nw.new_node(
                        Nodes.NoiseTexture,
                        input_kwargs={"Vector": nw.new_node(Nodes.InputPosition), "Scale": scale},
        ...
        offset = nw.scalar_multiply(
            offset, nw.switch(nw.compare("GREATER_THAN", nw.separate(nw.new_node(Nodes.InputNormal))[-1], 0), 1, -1)
        )
    ...
    def build_cutter(self, radius, height):
        ...
        cutter_scale = np.array([radius * uniform(0.8, 1.2), radius * uniform(0.8, 1.2), log_uniform(1.0, 1.2)])
        ...
        metric_fn = lambda nw: nw.vector_math(
            "LENGTH", nw.divide(nw.sub(nw.new_node(Nodes.InputPosition), cutter_location), cutter_scale)
        )
        return cutter, fn, inverse_fn, metric_fn

    def create_asset(self, i, distance=0, **params):
        outer = self.build_tree(i, distance, **params)
        radius = max([np.sqrt(v.co[0] ** 2 + v.co[1] ** 2) for v in outer.data.vertices if v.co[-1] < 0.1])
        ...
        material_indices[np.any(material_indices[:, np.newaxis] == null_indices[np.newaxis, :], -1)] = (
            ring_material_index
        )
        ...
        surface.add_geomod(obj, self.geo_cutter, apply=True, input_args=[noise_strength, noise_scale, metric_fn])
