...
def nodegroup_chair_wheel(nw: NodeWrangler):
    ...
    multiply = nw.new_node(
        Nodes.Math, input_kwargs={0: group_input.outputs["Wheel Width"], 1: -1.0000}, attrs={"operation": "MULTIPLY"}
    )
    ...
    arctop = nw.new_node(
        nodegroup_arc_top().name, input_kwargs={"Diameter": add, "Sweep Angle": group_input.outputs["Arc Sweep Angle"]}
    )

    multiply_2 = nw.new_node(
        Nodes.Math, input_kwargs={0: group_input.outputs["Wheel Width"], 1: 2.0000}, attrs={"operation": "MULTIPLY"}
    )

    quadrilateral = nw.new_node(
        "GeometryNodeCurvePrimitiveQuadrilateral", input_kwargs={"Width": multiply_2, "Height": 0.0200}
    )
    ...
    curve_to_mesh = nw.new_node(
        Nodes.CurveToMesh, input_kwargs={"Curve": arctop, "Profile Curve": fillet_curve, "Fill Caps": True}
    )
    ...
    join_geometry = nw.new_node(
        Nodes.JoinGeometry, input_kwargs={"Geometry": [instance_on_points, curve_to_mesh, transform_2]}
    )
    ...
    subtract = nw.new_node(
        Nodes.Math, input_kwargs={0: group_input.outputs["Pole Length"], 1: 0.1500}, attrs={"operation": "SUBTRACT"}
    )
    ...
    radians = nw.new_node(
        Nodes.Math, input_kwargs={0: group_input.outputs["Wheel Rotation"]}, attrs={"operation": "RADIANS"}
    )

    combine_xyz_6 = nw.new_node(Nodes.CombineXYZ, input_kwargs={"Z": radians})

    transform_4 = nw.new_node(
        Nodes.Transform, input_kwargs={"Geometry": transform_6, "Translation": combine_xyz_5, "Rotation": combine_xyz_6}
    )
    ...
    transform_3 = nw.new_node(
        Nodes.Transform, input_kwargs={"Geometry": ngoncylinder.outputs["Mesh"], "Rotation": (0.0000, -1.5708, 0.0000)}
    )
...
def nodegroup_wheeled_leg(nw: NodeWrangler, **kwargs):
    ...
    transform_geometry = nw.new_node(
        Nodes.Transform, input_kwargs={"Geometry": chair_wheel, "Rotation": (0.0000, 1.5708, 0.0000)}
    )
    ...
    alignbottomtofloor = nw.new_node(
        nodegroup_align_bottom_to_floor().name, input_kwargs={"Geometry": createlegsandstrechers}
    )

    multiply = nw.new_node(
        Nodes.Math, input_kwargs={0: group_input.outputs["Leg Diameter"]}, attrs={"operation": "MULTIPLY"}
    )
    ...
    cylinder = nw.new_node(
        "GeometryNodeMeshCylinder", input_kwargs={"Vertices": 64, "Radius": multiply, "Depth": subtract}
    )
    ...
    cylinder_1 = nw.new_node(
        "GeometryNodeMeshCylinder", input_kwargs={"Vertices": 64, "Radius": subtract_1, "Depth": subtract_2}
    )

    multiply_2 = nw.new_node(Nodes.Math, input_kwargs={1: subtract_2}, attrs={"operation": "MULTIPLY"})

    subtract_3 = nw.new_node(
        Nodes.Math, input_kwargs={0: group_input.outputs["Top Height"], 1: multiply_2}, attrs={"operation": "SUBTRACT"}
    )

    combine_xyz_2 = nw.new_node(Nodes.CombineXYZ, input_kwargs={"Z": subtract_3})

    transform_geometry_3 = nw.new_node(
        Nodes.Transform, input_kwargs={"Geometry": cylinder_1.outputs["Mesh"], "Translation": combine_xyz_2}
    )
