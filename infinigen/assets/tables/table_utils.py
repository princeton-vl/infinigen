...
def nodegroup_n_gon_cylinder(nw: NodeWrangler):
    ...
    resample_curve = nw.new_node(
        Nodes.ResampleCurve, input_kwargs={"Curve": set_curve_tilt, "Count": group_input.outputs["Resolution"]}
    )
    ...
    resample_curve_1 = nw.new_node(
        Nodes.ResampleCurve, input_kwargs={"Curve": ngonprofile, "Count": group_input.outputs["Profile Resolution"]}
    )
    ...
    subtract = nw.new_node(
        Nodes.Math, input_kwargs={0: domain_size.outputs["Face Count"], 1: 2.0000}, attrs={"operation": "SUBTRACT"}
    )

    less_than = nw.new_node(
        Nodes.Compare, input_kwargs={2: index, 3: subtract}, attrs={"operation": "LESS_THAN", "data_type": "INT"}
    )

    delete_geometry = nw.new_node(
        Nodes.DeleteGeometry, input_kwargs={"Geometry": curve_to_mesh, "Selection": less_than}, attrs={"domain": "FACE"}
    )
...
def nodegroup_generate_radius_curve(nw: NodeWrangler, curve_control_points):
    ...
    multiply = nw.new_node(
        Nodes.VectorMath, input_kwargs={0: position, 1: combine_xyz_1}, attrs={"operation": "MULTIPLY"}
    )

    set_position = nw.new_node(
        Nodes.SetPosition, input_kwargs={"Geometry": resample_curve, "Position": multiply.outputs["Vector"]}
    )
...
def nodegroup_create_anchors(nw: NodeWrangler):
    ...
    multiply = nw.new_node(
        Nodes.Math, input_kwargs={0: group_input.outputs["Profile Width"], 1: 0.3535}, attrs={"operation": "MULTIPLY"}
    )
    ...
    curve_to_points_1 = nw.new_node(
        Nodes.CurveToPoints, input_kwargs={"Curve": curve_line}, attrs={"mode": "EVALUATED"}
    )
...
def nodegroup_create_legs_and_strechers(nw: NodeWrangler):
    ...
    transform = nw.new_node(
        Nodes.Transform, input_kwargs={"Geometry": group_input.outputs["Anchors"], "Translation": combine_xyz}
    )
    ...
    subtract = nw.new_node(
        Nodes.VectorMath, input_kwargs={0: combine_xyz, 1: combine_xyz_3}, attrs={"operation": "SUBTRACT"}
    )

    subtract_1 = nw.new_node(
        Nodes.VectorMath, input_kwargs={0: position, 1: subtract.outputs["Vector"]}, attrs={"operation": "SUBTRACT"}
    )
    ...
    multiply = nw.new_node(
        Nodes.VectorMath, input_kwargs={0: vector_rotate, 1: combine_xyz_4}, attrs={"operation": "MULTIPLY"}
    )

    subtract_2 = nw.new_node(
        Nodes.VectorMath, input_kwargs={0: position, 1: multiply.outputs["Vector"]}, attrs={"operation": "SUBTRACT"}
    )

    align_euler_to_vector = nw.new_node(
        Nodes.AlignEulerToVector, input_kwargs={"Vector": subtract_2}, attrs={"axis": "Z"}
    )
    ...
    scale = nw.new_node(
        Nodes.VectorMath, input_kwargs={0: subtract_2, "Scale": multiply_1}, attrs={"operation": "SCALE"}
    )
    ...
    set_position = nw.new_node(
        Nodes.SetPosition, input_kwargs={"Geometry": transform, "Position": add.outputs["Vector"]}
    )
    ...
    divide_1 = nw.new_node(
        Nodes.Math, input_kwargs={0: domain_size.outputs["Point Count"], 1: 2.0000}, attrs={"operation": "DIVIDE"}
    )

    less_than = nw.new_node(
        Nodes.Compare, input_kwargs={2: index_1, 3: divide_1}, attrs={"operation": "LESS_THAN", "data_type": "INT"}
    )

    switch_2 = nw.new_node(
        Nodes.Switch, input_kwargs={0: equal, 6: boolean, 7: less_than}, attrs={"input_type": "BOOLEAN"}
    )
    ...
    modulo_1 = nw.new_node(
        Nodes.Math, input_kwargs={0: add_1, 1: domain_size.outputs["Point Count"]}, attrs={"operation": "MODULO"}
    )

    field_at_index = nw.new_node(
        Nodes.FieldAtIndex, input_kwargs={"Index": modulo_1, 3: position_1}, attrs={"data_type": "FLOAT_VECTOR"}
    )

    subtract_3 = nw.new_node(
        Nodes.VectorMath, input_kwargs={0: position_1, 1: field_at_index.outputs[2]}, attrs={"operation": "SUBTRACT"}
    )

    align_euler_to_vector_1 = nw.new_node(
        Nodes.AlignEulerToVector, input_kwargs={"Vector": subtract_3.outputs["Vector"]}, attrs={"axis": "Z"}
    )

    align_euler_to_vector_2 = nw.new_node(
        Nodes.AlignEulerToVector, input_kwargs={"Rotation": align_euler_to_vector_1}, attrs={"pivot_axis": "Z"}
    )

    length_1 = nw.new_node(
        Nodes.VectorMath, input_kwargs={0: subtract_3.outputs["Vector"]}, attrs={"operation": "LENGTH"}
    )

    combine_xyz_1 = nw.new_node(
        Nodes.CombineXYZ, input_kwargs={"X": 1.0000, "Y": 1.0000, "Z": length_1.outputs["Value"]}
    )
    ...
    join_geometry = nw.new_node(
        Nodes.JoinGeometry, input_kwargs={"Geometry": [switch_1.outputs[6], realize_instances_1]}
    )
...
def nodegroup_create_cap(nw: NodeWrangler):
    ...
    group_input = nw.new_node(
        Nodes.GroupInput,
        expose_input=[("NodeSocketFloatDistance", "Radius", 1.0000), ("NodeSocketInt", "Resolution", 64)],
    )

    multiply = nw.new_node(
        Nodes.Math, input_kwargs={0: group_input.outputs["Radius"], 1: 257.0000}, attrs={"operation": "MULTIPLY"}
    )
    ...
    power = nw.new_node(
        Nodes.Math, input_kwargs={0: group_input.outputs["Radius"], 1: 2.0000}, attrs={"operation": "POWER"}
    )
    ...
    transform = nw.new_node(
        Nodes.Transform, input_kwargs={"Geometry": store_named_attribute, "Translation": combine_xyz}
    )
    ...
    less_than = nw.new_node(
        Nodes.Compare, input_kwargs={0: separate_xyz.outputs["Z"]}, attrs={"operation": "LESS_THAN"}
    )

    delete_geometry = nw.new_node(Nodes.DeleteGeometry, input_kwargs={"Geometry": transform, "Selection": less_than})

    group_output = nw.new_node(
        Nodes.GroupOutput, input_kwargs={"Mesh": delete_geometry}, attrs={"is_active_output": True}
    )
...
def nodegroup_arc_top(nw: NodeWrangler):
    ...
    group_input = nw.new_node(
        Nodes.GroupInput,
        expose_input=[("NodeSocketFloatDistance", "Diameter", 1.0000), ("NodeSocketFloat", "Sweep Angle", 180.0000)],
    )

    divide = nw.new_node(
        Nodes.Math, input_kwargs={0: group_input.outputs["Diameter"], 1: 2.0000}, attrs={"operation": "DIVIDE"}
    )
    ...
    radians_1 = nw.new_node(
        Nodes.Math, input_kwargs={0: group_input.outputs["Sweep Angle"]}, attrs={"operation": "RADIANS"}
    )
    ...
    transform_1 = nw.new_node(
        Nodes.Transform, input_kwargs={"Geometry": arc.outputs["Curve"], "Rotation": (1.5708, 0.0000, 0.0000)}
    )
...
def nodegroup_align_bottom_to_floor(nw: NodeWrangler):
    ...
    multiply = nw.new_node(
        Nodes.Math, input_kwargs={0: separate_xyz.outputs["Z"], 1: -1.0000}, attrs={"operation": "MULTIPLY"}
    )

    combine_xyz = nw.new_node(Nodes.CombineXYZ, input_kwargs={"Z": multiply})

    transform_geometry_1 = nw.new_node(
        Nodes.Transform, input_kwargs={"Geometry": group_input.outputs["Geometry"], "Translation": combine_xyz}
    )
...
def nodegroup_bent(nw: NodeWrangler):
    ...
    group_input = nw.new_node(
        Nodes.GroupInput,
        expose_input=[("NodeSocketGeometry", "Geometry", None), ("NodeSocketFloat", "Amount", -0.1000)],
    )
    ...
    multiply_1 = nw.new_node(
        Nodes.Math, input_kwargs={0: multiply, 1: group_input.outputs["Amount"]}, attrs={"operation": "MULTIPLY"}
    )
...
def nodegroup_merge_curve(nw: NodeWrangler):
    ...
    group_output = nw.new_node(
        Nodes.GroupOutput, input_kwargs={"Curve": mesh_to_curve}, attrs={"is_active_output": True}
    )
