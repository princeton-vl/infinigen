...
def shader_apple_shader(nw: NodeWrangler, color1, color2, random_seed):
    ...
    add = nw.new_node(Nodes.VectorMath, input_kwargs={0: texture_coordinate.outputs["Object"], 1: value})

    separate_xyz = nw.new_node(Nodes.SeparateXYZ, input_kwargs={"Vector": add.outputs["Vector"]})

    multiply = nw.new_node(
        Nodes.Math, input_kwargs={0: separate_xyz.outputs["Z"], 1: 0.2}, attrs={"operation": "MULTIPLY"}
    )
    ...
    musgrave_texture = nw.new_node(
        Nodes.MusgraveTexture, input_kwargs={"Vector": add.outputs["Vector"], "Scale": 0.6, "Lacunarity": 1.0}
    )
    ...
    mix = nw.new_node(Nodes.MixRGB, input_kwargs={"Fac": musgrave_texture, "Color1": rgb, "Color2": rgb_1})

    hue_saturation_value = nw.new_node("ShaderNodeHueSaturation", input_kwargs={"Hue": 0.55, "Color": mix})

    mix_3 = nw.new_node(
        Nodes.MixRGB, input_kwargs={"Fac": musgrave_texture_2, "Color1": mix, "Color2": hue_saturation_value}
    )

    principled_bsdf = nw.new_node(Nodes.PrincipledBSDF, input_kwargs={"Base Color": mix_3})
...
def nodegroup_apple_surface(
    ...
    set_material = nw.new_node(
        ...
        input_kwargs={
            "Geometry": adddent_1,
            "Material": surface.shaderfunc_to_material(shader_apple_shader, color1, color2, random_seed),
        },
    )
