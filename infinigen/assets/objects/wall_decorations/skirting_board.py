...
def make_skirting_board(constants, objs, tag, joined=True):
    if joined:
        seqs = list([o for o in objs if room_level(o.name.split('.')[0]) == i] for i in range(constants.n_stories))
    else:
    ...
    for s in seqs:
        ...
        for p in portal_cutters:
            if p.name.startswith('entrance') and int(p.location[-1] / constants.wall_height - 1 / 2) == 0:
...
def linear_ring2curve(ring, constants, reversed=False):
    ...
    invalid = np.sort(np.nonzero((np.abs(lengths - constants.wall_thickness) < .02) | (np.abs(lengths - constants.door_width) < .02))[0])
