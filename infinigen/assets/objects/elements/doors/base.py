...
from infinigen.assets.materials import glass, metal, plastic, wood
...
class BaseDoorFactory(AssetFactory):
    def __init__(
        self, factory_seed, coarse=False, constants=None, use_transparent_mat=True
    ):
        ...
        with FixedSeed(self.factory_seed):
            ...
            if use_transparent_mat:
                self.glass_surface = glass
            else:
                self.glass_surface = self.surface

            self.has_louver = True
            self.louver_surface = np.random.choice([metal, wood], p=[0.3, 0.7])

            self.handle_type = np.random.choice(
                ["knob", "lever", "pull", "bar", "none"]
            )
            # self.handle_type = np.random.choice(["none"])
            ...
            # self.door_frame_style = np.random.choice(["full_frame_double_door"])
            # self.door_frame_style = np.random.choice(["single_column", "full_frame_square", "full_frame_dome"])
            ...
            if self.handle_type == "bar":
                self.handle_info_dict = {
                    ...
                    "color": metal.sample_metal_color(metal_color="natural"),
                }
                # self.bar_length = uniform(0.7, 0.9) * self.width
                # self.bar_thickness = uniform(0.025, 0.045) * self.height
                # self.bar_aspect_ratio = uniform(0.4, 0.6)
                # self.bar_height_ratio = uniform(0.7, 0.9)
                # self.bar_length_ratio = uniform(0.5, 0.8)
                # self.bar_end_length_ratio = uniform(0.1, 0.15)
                # self.bar_end_height_ratio = uniform(1.8, 3.0)
                # self.bar_overall_z_offset = -uniform(0.0, 0.1) * self.height
    ...
    def _create_asset(self, **params):
        ...
        # if export:
        #     export_func = factory.sim_exporter_factory(exporter=exporter, **params)
        #     asset_path, semantic_mapping = export_func(
        #         blend_obj=door_joined,
        #         sim_blueprint=self.sim_blueprint,
        #         seed=self.factory_seed,
        #         **params,
        #     )
        #     return door_joined, asset_path, semantic_mapping
