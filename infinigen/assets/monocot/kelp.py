...
class KelpMonocotFactory(MonocotGrowthFactory):
    ...
    def __init__(self, factory_seed, coarse=False):
        ...
        with FixedSeed(factory_seed):
            ...
            self.align_direction = np.cos(flow_angle), np.sin(flow_angle), uniform(-0.2, 0.2)
    ...
    def make_align_factor(self):
        def align_factor(nw: NodeWrangler):
            ...
            driver.expression = repeated_driver(0.7, 0.85, self.anim_freq, self.anim_offset, self.anim_seed)
    ...
    def make_align_direction(self):
        def align_direction(nw: NodeWrangler):
            ...
            driver.expression = repeated_driver(-0.5, -0.1, self.anim_freq, self.anim_offset, self.anim_seed)
            return direction
