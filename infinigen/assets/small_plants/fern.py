...
def nodegroup_pinnae_level1_yaxis_rotation(nw: NodeWrangler):
    ...
    group_input = nw.new_node(
        Nodes.GroupInput, expose_input=[("NodeSocketFloat", "From Max", 1.0), ("NodeSocketFloat", "Value", 1.0)]
    )
    map_range = nw.new_node(
        Nodes.MapRange, input_kwargs={"Value": group_input.outputs["Value"], 2: group_input.outputs["From Max"]}
    )
...
def nodegroup_pinnae_level1_xaxis_rotation(nw: NodeWrangler):
    ...
    map_range = nw.new_node(
        Nodes.MapRange, input_kwargs={"Value": group_input.outputs["Value2"], 3: -1.5000, 4: 0.0000}
    )
...
def nodegroup_pinnae_level1_stein(nw: NodeWrangler):
    ...
    multiply = nw.new_node(
        Nodes.Math, input_kwargs={0: group_input.outputs["Value2"], 1: 0.01}, attrs={"operation": "MULTIPLY"}
    )
    set_curve_radius = nw.new_node(Nodes.SetCurveRadius, input_kwargs={"Curve": mesh_to_curve, "Radius": multiply})
    multiply_1 = nw.new_node(
        Nodes.Math, input_kwargs={0: group_input.outputs["Value1"], 1: 15.0}, attrs={"operation": "MULTIPLY"}
    )
...
def nodegroup_pinnae_level1_scale(nw: NodeWrangler, pinnae_contour):
    ...
    group_input = nw.new_node(
        Nodes.GroupInput, expose_input=[("NodeSocketFloat", "Value1", 1.0), ("NodeSocketFloat", "Value2", 1.0)]
    )

    pinnae_contour_float_curve = nw.new_node(
        Nodes.FloatCurve, input_kwargs={"Value": group_input.outputs["Value1"]}, label="PinnaeContourFloatCurve"
    )
    node_utils.assign_curve(
...
def nodegroup_pinnae_level1_instance_rotation(nw: NodeWrangler):
    ...
    group_input = nw.new_node(
        Nodes.GroupInput, expose_input=[("NodeSocketFloat", "Value1", 0.5), ("NodeSocketFloat", "Value2", 1.0)]
    )
...
def nodegroup_pinnae_level1_rotation(nw: NodeWrangler, gravity_rotation=1):
    ...
    pinnaelevel1xaxisrotation = nw.new_node(
        ...
        input_kwargs={"From Max": add, 1: pinnae_index, 2: group_input.outputs["Value1"]},
    )
    vector_rotate = nw.new_node(
        Nodes.VectorRotate,
        input_kwargs={"Vector": position, "Center": (0, 0, 0), "Angle": pinnaelevel1xaxisrotation},
        attrs={"rotation_type": "X_AXIS"},
    )
    pinnaelevel1gravityrotation = nw.new_node(
        nodegroup_pinnae_level1_gravity_rotation(gravity_rotation=gravity_rotation).name,
        input_kwargs={"From Max": add, "Value": pinnae_index},
    )
    vector_rotate_1 = nw.new_node(
        Nodes.VectorRotate,
        input_kwargs={"Vector": vector_rotate, "Center": (0, 0, 0), "Angle": pinnaelevel1gravityrotation},
        attrs={"rotation_type": "X_AXIS"},
    )
    pinnaelevel1zaxisrotation = nw.new_node(
        nodegroup_pinnae_level1_zaxis_rotation().name, input_kwargs={"From Max": add, "Value": pinnae_index}
    )
    ...
    pinnaelevel1yaxisrotation = nw.new_node(
        nodegroup_pinnae_level1_yaxis_rotation().name, input_kwargs={"From Max": add, "Value": pinnae_index}
    )
    ...
    group_output = nw.new_node(
        Nodes.GroupOutput, input_kwargs={"Vector": vector_rotate_3, "Value": pinnaelevel1xaxisrotation}
    )
...
def nodegroup_pinnae_level1_instance_position(nw: NodeWrangler, pinnae_contour):
    ...
    map_range_3 = nw.new_node(
        Nodes.MapRange,
        input_kwargs={"Value": group_input.outputs["Value1"], 2: group_input.outputs["From Max"], 3: 1.0, 4: 0.0},
    )
    ...
    group_output = nw.new_node(
        Nodes.GroupOutput, input_kwargs={"Vector": combine_xyz_1, "Result": map_range_3.outputs["Result"]}
    )
...
def nodegroup_pinnae_level2_rotation(nw: NodeWrangler, z_axis_rotate, y_axis_rotate, x_axis_rotate):
    ...
    multiply_1 = nw.new_node(
        Nodes.Math, input_kwargs={0: add_2, 1: map_range_7.outputs["Result"]}, attrs={"operation": "MULTIPLY"}
    )
    vector_rotate_1 = nw.new_node(
        Nodes.VectorRotate,
...
def nodegroup_pinnae_level2_set_point(nw: NodeWrangler, pinna_contour):
    ...
    group_output = nw.new_node(
        Nodes.GroupOutput,
        input_kwargs={"Vector": combine_xyz_2, "Value": float_curve, "Result": map_range_4.outputs["Result"]},
    )
...
def nodegroup_pinnae_level2_instance_on_points(nw: NodeWrangler, leaf, pinna_contour):
    ...
    transform = nw.new_node(
        Nodes.Transform, input_kwargs={"Geometry": object_info_2.outputs["Geometry"], "Scale": (1.2, -1.0, 1.0)}
    )
...
def nodegroup_pinnae_level2_stein(nw: NodeWrangler):
    ...
    multiply = nw.new_node(
        Nodes.Math, input_kwargs={0: group_input.outputs["Value1"], 1: 0.1}, attrs={"operation": "MULTIPLY"}
    )
    set_curve_radius_1 = nw.new_node(Nodes.SetCurveRadius, input_kwargs={"Curve": mesh_to_curve_1, "Radius": multiply})
    multiply_1 = nw.new_node(
        Nodes.Math, input_kwargs={0: group_input.outputs["Value2"], 1: 0.5}, attrs={"operation": "MULTIPLY"}
    )
    curve_circle_1 = nw.new_node(Nodes.CurveCircle, input_kwargs={"Radius": multiply_1, "Resolution": 10})
    curve_to_mesh_1 = nw.new_node(
        Nodes.CurveToMesh, input_kwargs={"Curve": set_curve_radius_1, "Profile Curve": curve_circle_1.outputs["Curve"]}
    )
...
def geometry_pinnae_nodes(
    ...
    for i in range(version_num_param):
        ...
        greater_than = nw.new_node(Nodes.Math, input_kwargs={0: index, 1: 2.0}, attrs={"operation": "GREATER_THAN"})
        modulo = nw.new_node(Nodes.Math, input_kwargs={0: index, 1: 2.0}, attrs={"operation": "MODULO"})
        if random_bit:
    ...
    for i in range(version_num_param):
        ...
        for j in range(6):
            pinna_contour.append(k * np.clip(j * (1.0 + normal(0, 0.1, (1,))[0]) / 5.0 + 0.08, 0, 0.7))
        ...
        set_position_pinna = nw.new_node(
            Nodes.SetPosition,
            input_kwargs={"Geometry": mesh_line_pinna, "Position": pinnaelevel1instanceposition.outputs["Vector"]},
        )
        if rotation:
            pinnaelevel1rotation = nw.new_node(
                nodegroup_pinnae_level1_rotation(gravity_rotation=gravity_rotation).name,
        ...
        set_rotation_pinna = nw.new_node(
            Nodes.SetPosition,
            input_kwargs={"Geometry": set_position_pinna, "Position": pinnaelevel1rotation.outputs["Vector"]},
        )
        ...
        set_position_leaf = nw.new_node(
            Nodes.SetPosition,
            input_kwargs={"Geometry": mesh_line_leaf, "Position": pinnaelevel2setpoint.outputs["Vector"]},
        )
        ...
        pinnaelevel2rotation = nw.new_node(
            nodegroup_pinnae_level2_rotation(
                z_axis_rotate=z_curvature, y_axis_rotate=y_curvature, x_axis_rotate=x_curvature
            ).name,
            input_kwargs={"Geometry": set_position_leaf, 1: age, 2: leaf_index, 3: integer_2},
        )
        set_rotation_leaf = nw.new_node(
            Nodes.SetPosition, input_kwargs={"Geometry": set_position_leaf, "Position": pinnaelevel2rotation}
        )
        ...
        rotate_instances = nw.new_node(
            Nodes.RotateInstances, input_kwargs={"Instances": pinna_on_pinnae, "Rotation": (-0.1571, 0.0, 0.0)}
        )
        scale_instances = nw.new_node(
            Nodes.ScaleInstances, input_kwargs={"Instances": rotate_instances, "Scale": (-1.0, 1.0, 1.0)}
        )
        pinnaelevel2stein = nw.new_node(
            nodegroup_pinnae_level2_stein().name,
            input_kwargs={0: pinnaelevel2setpoint.outputs["Result"], "Mesh": scale_instances},
        )
        ...
        join_geometry = nw.new_node(
            Nodes.JoinGeometry, input_kwargs={"Geometry": [pinnaelevel2stein, pinnaelevel2instanceonpoints]}
        )
        ...
        if i == version_num_param - 1:
            pinnaelevel1stein = nw.new_node(
                ...
                input_kwargs={"Mesh": set_rotation_pinna, 1: age, 2: pinnaelevel1instanceposition.outputs["Result"]},
            )
    ...
    set_positions = nw.new_node(
        Nodes.SetPosition, input_kwargs={"Geometry": realize_instances, "Offset": noise_texture.outputs["Color"]}
    )
...
def geo_fern(nw: NodeWrangler, **kwargs):
    ...
    if fern_mode == "young_and_grownup":
        ...
        for r in rotates:
            ...
            z_transform = nw.new_node(Nodes.Transform, input_kwargs={"Geometry": shape, "Rotation": (0.0, 0.0, r[0])})
            x_transform = nw.new_node(
                Nodes.Transform, input_kwargs={"Geometry": z_transform, "Rotation": (-r[1], 0.0, 0.0)}
            )
            z2_transform = nw.new_node(
                Nodes.Transform, input_kwargs={"Geometry": x_transform, "Rotation": (0.0, 0.0, r[2])}
            )
        ...
        for i in range(young_num):
            ...
            z_transform = nw.new_node(
                Nodes.Transform, input_kwargs={"Geometry": shape, "Rotation": (0.0, 0.0, rotate_z[0])}
            )
            x_transform = nw.new_node(
                Nodes.Transform, input_kwargs={"Geometry": z_transform, "Rotation": (-rotate_x[0], 0.0, 0.0)}
            )
            z2_transform = nw.new_node(
                Nodes.Transform, input_kwargs={"Geometry": x_transform, "Rotation": (0.0, 0.0, rotate_z2[0])}
            )
            pinnaes.append(z2_transform)
        ...
        shape = nw.new_node(
            geometry_pinnae_nodes(
                leaf, leaf_num_param=20, age_param=kwargs["age"], pinna_num_param=60, version_num_param=version_num
            ).name
        )
