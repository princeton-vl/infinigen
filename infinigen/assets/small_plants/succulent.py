...
def nodegroup_pedal_cross_contour(nw: NodeWrangler):
    ...
    pedal_cross_contour_bottom = nw.new_node(
        ...
        input_kwargs={"Y": group_input.outputs["Y_bottom"], "X": group_input.outputs["X"]},
    )
...
def nodegroup_pedal_rotation_on_base_circle(nw: NodeWrangler):
    ...
    group_input = nw.new_node(
        Nodes.GroupInput, expose_input=[("NodeSocketFloat", "Value1", -1.3), ("NodeSocketFloat", "Value2", -1.57)]
    )

    add = nw.new_node(Nodes.Math, input_kwargs={0: random_value_1.outputs[1], 1: group_input.outputs["Value1"]})

    random_value_2 = nw.new_node(Nodes.RandomValue, input_kwargs={2: -0.3, 3: 0.3})

    add_1 = nw.new_node(Nodes.Math, input_kwargs={0: random_value_2.outputs[1], 1: group_input.outputs["Value2"]})

    combine_xyz_2 = nw.new_node(Nodes.CombineXYZ, input_kwargs={"X": add, "Z": add_1})

    group_output = nw.new_node(Nodes.GroupOutput, input_kwargs={"Vector": combine_xyz_2})
...
def nodegroup_base_perturbation(nw: NodeWrangler, R=1.0):
    ...
    random_value_4 = nw.new_node(Nodes.RandomValue, input_kwargs={2: -0.8 * R, 3: 0.8 * R})

    random_value = nw.new_node(Nodes.RandomValue, input_kwargs={2: -0.8 * R, 3: 0.8 * R})

    random_value_1 = nw.new_node(Nodes.RandomValue, input_kwargs={2: -0.2 * R, 3: 0.2 * R})

    group_input = nw.new_node(Nodes.GroupInput, expose_input=[("NodeSocketFloat", "Value", 0.5)])

    add = nw.new_node(Nodes.Math, input_kwargs={0: random_value_1.outputs[1], 1: group_input.outputs["Value"]})

    combine_xyz_1 = nw.new_node(
        Nodes.CombineXYZ, input_kwargs={"X": random_value_4.outputs[1], "Y": random_value.outputs[1], "Z": add}
    )
...
def nodegroup_pedal_geometry(nw: NodeWrangler, curve_param=[]):
    ...
    resample_curve = nw.new_node(Nodes.ResampleCurve, input_kwargs={"Curve": curve_line, "Count": integer})
    ...
    pedal_stem_curvature = nw.new_node(
        nodegroup_pedal_stem_curvature().name, input_kwargs={"Value": group_input.outputs["pedal_stem"]}
    )

    set_position_4 = nw.new_node(
        Nodes.SetPosition, input_kwargs={"Geometry": resample_curve, "Offset": pedal_stem_curvature}
    )

    pedal_z_contour = nw.new_node(
        nodegroup_pedal_z_contour(curve_param=curve_param).name, input_kwargs={"Value": group_input.outputs["pedal_z"]}
    )
    ...
    curve_to_mesh = nw.new_node(
        Nodes.CurveToMesh,
        input_kwargs={"Curve": set_curve_radius, "Profile Curve": pedal_cross_contour, "Fill Caps": True},
    )
...
def nodegroup_pedal_on_base(nw: NodeWrangler, R=1.0):
    ...
    base_perturbation = nw.new_node(
        nodegroup_base_perturbation(R=R).name, input_kwargs={"Value": group_input.outputs["base_z"]}
    )

    set_position = nw.new_node(
        Nodes.SetPosition, input_kwargs={"Geometry": curve_circle_1.outputs["Curve"], "Offset": base_perturbation}
    )

    normal_1 = nw.new_node(Nodes.InputNormal)

    align_euler_to_vector_1 = nw.new_node(
        Nodes.AlignEulerToVector, input_kwargs={"Vector": normal_1}, attrs={"pivot_axis": "Z"}
    )
    ...
    rotate_instances_1 = nw.new_node(
        ...
        input_kwargs={"Instances": realize_instances_1, "Rotation": pedal_rotation_on_base_circle},
    )

    scale_instances = nw.new_node(
        Nodes.ScaleInstances, input_kwargs={"Instances": rotate_instances_1, "Scale": group_input.outputs["Scale"]}
    )
...
def geometry_succulent_nodes(nw: NodeWrangler, **kwargs):
    ...
    pedal_cross_coutour_y_bottom = nw.new_node(Nodes.Value, label="pedal_cross_coutour_y_bottom")
    ...
    pedal_cross_coutour_y_top = nw.new_node(Nodes.Value, label="pedal_cross_coutour_y_top")
    ...
    for i in range(kwargs["num_bases"]):
        ...
        base_pedal_num = nw.new_node(Nodes.Integer, label="base_pedal_num", attrs={"integer": 10})
    ...
    join_geometry = nw.new_node(Nodes.JoinGeometry, input_kwargs={"Geometry": pedal_bases})

    set_shade_smooth_1 = nw.new_node(Nodes.SetShadeSmooth, input_kwargs={"Geometry": join_geometry})

    set_material = nw.new_node(
        Nodes.SetMaterial,
        input_kwargs={"Geometry": set_shade_smooth_1, "Material": surface.shaderfunc_to_material(material)},
    )
