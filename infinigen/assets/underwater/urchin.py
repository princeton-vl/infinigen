...
class UrchinFactory(AssetFactory):
    ...
    def create_asset(self, placeholder, face_size=0.01, **params):
        ...
        surface.add_geomod(obj, self.geo_extrude, apply=True, attributes=["spike", "girdle"], domains=["FACE"] * 2)
        levels = 1
        ...
        butil.modify_mesh(
            obj, "DISPLACE", texture=bpy.data.textures.new(name="urchin", type="STUCCI"), strength=0.005, mid_level=0
        )
    ...
    def animate_stretch(self, obj):
        obj, mod = butil.modify_mesh(
            obj, "SIMPLE_DEFORM", False, return_mod=True, deform_method="STRETCH", deform_axis="Z"
        )
    ...
    def geo_extrude(nw: NodeWrangler):
        ...
        selection = nw.boolean_math("AND", nw.compare("GREATER_EQUAL", face_vertices, 5), nw.bernoulli(face_prob))
        ...
        direction = nw.scale(
            nw.add(nw.new_node(Nodes.InputNormal), nw.uniform([-perturb] * 3, [perturb] * 3)),
