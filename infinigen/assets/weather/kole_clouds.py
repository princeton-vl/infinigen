...
def kole_clouds_shader(
    ...
    drive_param(
        value.outputs[0],
        (coverage_frame_end - coverage_frame_start) / (bpy.context.scene.frame_end - bpy.context.scene.frame_start + 1),
    ...
    vector_transform = nw.new_node("ShaderNodeVectorTransform", input_kwargs={"Vector": geometry.outputs["Position"]})
    ...
    if rg(translation_animation):
        drive_param(
            ...
            0.001,
            offset=-(bpy.context.scene.frame_start + bpy.context.scene.frame_end) / 2 * 0.001 + translation,
            index=0,
        )

    multiply = nw.new_node(
        Nodes.VectorMath, input_kwargs={0: vector_transform, 1: (1.0, 1.0, 1.0)}, attrs={"operation": "MULTIPLY"}
    )

    separate_xyz = nw.new_node(Nodes.SeparateXYZ, input_kwargs={"Vector": multiply.outputs["Vector"]})

    map_range = nw.new_node(Nodes.MapRange, input_kwargs={"Value": separate_xyz.outputs["Z"], 1: 0.1, 2: 0.3, 4: -0.2})
    ...
    add = nw.new_node(
        Nodes.VectorMath, input_kwargs={0: multiply_1.outputs["Vector"], 1: (0.0, 0.0, np.random.uniform(0, 2))}
    )
    ...
    map_range_1 = nw.new_node(Nodes.MapRange, input_kwargs={"Value": musgrave_texture, 1: -1.0}, attrs={"clamp": False})

    add_1 = nw.new_node(Nodes.Math, input_kwargs={0: map_range.outputs["Result"], 1: map_range_1.outputs["Result"]})

    add_2 = nw.new_node(Nodes.Math, input_kwargs={0: value, 1: add_1})

    map_range_2 = nw.new_node(Nodes.MapRange, input_kwargs={"Value": add_2, 1: 0.4, 2: 0.5}, attrs={"clamp": False})
    ...
    map_range_3 = nw.new_node(Nodes.MapRange, input_kwargs={"Value": length.outputs["Value"], 1: 1.0, 2: add_3})

    geometry_1 = nw.new_node("ShaderNodeNewGeometry")

    voronoi_texture = nw.new_node(
        Nodes.VoronoiTexture, input_kwargs={"Vector": geometry_1.outputs["Position"], "Scale": 0.01}
    )

    map_range_4 = nw.new_node(
        Nodes.MapRange, input_kwargs={"Value": voronoi_texture.outputs["Distance"], 3: 0.5, 4: 2.0}
    )
    ...
    node_utils.assign_curve(
        float_curve.mapping.curves[0], [(0.0, 1.0), (0.0273, 0.0063), (0.2455, 0.6), (0.6682, 0.3188), (0.9955, 1.0)]
    )

    map_range_5 = nw.new_node(Nodes.MapRange, input_kwargs={"Value": float_curve, 4: 5.0})
    ...
    multiply_2 = nw.new_node(Nodes.Math, input_kwargs={0: subtract, 1: 0.3}, attrs={"operation": "MULTIPLY"})

    multiply_3 = nw.new_node(Nodes.Math, input_kwargs={0: multiply_2, 1: 0.01}, attrs={"operation": "MULTIPLY"})

    add_4 = nw.new_node(Nodes.Math, input_kwargs={0: greater_than, 1: multiply_3})

    power_1 = nw.new_node(Nodes.Math, input_kwargs={0: map_range_1.outputs["Result"]}, attrs={"operation": "POWER"})

    density_mul = nw.new_node(
        Nodes.Math, input_kwargs={0: power_1, 1: density}, attrs={"operation": "MULTIPLY", "use_clamp": True}
    )

    volume_scatter = nw.new_node(
        "ShaderNodeVolumeScatter", input_kwargs={"Color": add_4, "Density": density_mul, "Anisotropy": anisotropy}
    )

    nw.new_node(Nodes.MaterialOutput, input_kwargs={"Surface": transparent_bsdf, "Volume": volume_scatter})


@gin.configurable("kole_clouds")
def add_kole_clouds(height=0):
    bpy.ops.mesh.primitive_ico_sphere_add(
        subdivisions=6, radius=1, enter_editmode=False, align="WORLD", location=(0, 0, 0), scale=(1, 1, 1)
    )
