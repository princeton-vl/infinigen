...
def nodegroup_stem_branch_leaf_s_r(nw: NodeWrangler):
    ...
    align_euler_to_vector_1 = nw.new_node(
        Nodes.AlignEulerToVector, input_kwargs={"Vector": curve_tangent_1}, attrs={"axis": "Z"}
    )

    group_output = nw.new_node(
        Nodes.GroupOutput, input_kwargs={"Value": random_value.outputs[1], "Rotation": align_euler_to_vector_1}
    )
...
def nodegroup_stem_branch_leaf_selection(nw: NodeWrangler):
    ...
    random_value_3 = nw.new_node(Nodes.RandomValue, input_kwargs={5: integer}, attrs={"data_type": "INT"})

    op_not = nw.new_node(Nodes.BooleanMath, input_kwargs={0: random_value_3.outputs[2]}, attrs={"operation": "NOT"})

    op_and = nw.new_node(Nodes.BooleanMath, input_kwargs={0: colorramp_1.outputs["Color"], 1: op_not})
...
def nodegroup_stem_branch_leaves(nw: NodeWrangler, leaves):
    ...
    random_value_3 = nw.new_node(
        Nodes.RandomValue, input_kwargs={"Max": (0.6, 0.6, 6.28), "Seed": 30}, attrs={"data_type": "FLOAT_VECTOR"}
    )

    rotate_instances_2 = nw.new_node(
        Nodes.RotateInstances,
        input_kwargs={"Instances": instance_on_points_4, "Rotation": random_value_3.outputs["Value"]},
    )

    realize_instances = nw.new_node(Nodes.RealizeInstances, input_kwargs={"Geometry": rotate_instances_2})

    group_output = nw.new_node(Nodes.GroupOutput, input_kwargs={"Instances": realize_instances})
...
def nodegroup_stem_branch_geometry(nw: NodeWrangler):
    ...
    set_curve_radius = nw.new_node(
        Nodes.SetCurveRadius, input_kwargs={"Curve": group_input.outputs["Curve"], "Radius": colorramp.outputs["Color"]}
    )

    r = uniform(0.015, 0.022, size=(1,))[0]
    curve_circle = nw.new_node(Nodes.CurveCircle, input_kwargs={"Resolution": 10, "Radius": r})
...
def nodegroup_stem_branch_rotation(nw: NodeWrangler):
    ...
    multiply_1 = nw.new_node(Nodes.Math, input_kwargs={0: add, 1: 1.0}, attrs={"operation": "MULTIPLY"})

    vector_rotate = nw.new_node(
        Nodes.VectorRotate,
        input_kwargs={"Vector": position, "Center": multiply.outputs["Vector"], "Angle": multiply_1},
...
def nodegroup_stem_branch(nw: NodeWrangler, flowers, leaves):
    ...
    resample_curve_4 = nw.new_node(Nodes.ResampleCurve, input_kwargs={"Curve": curve_line_2, "Count": 20})

    stembranchrotation = nw.new_node(nodegroup_stem_branch_rotation().name)

    set_position_2 = nw.new_node(
        Nodes.SetPosition, input_kwargs={"Geometry": resample_curve_4, "Position": stembranchrotation}
    )
    ...
    scale_instances = nw.new_node(
        Nodes.ScaleInstances, input_kwargs={"Instances": instance_on_points_3, "Scale": random_value.outputs[1]}
    )

    stembranchgeometry = nw.new_node(nodegroup_stem_branch_geometry().name, input_kwargs={"Curve": set_position_2})

    stembranchleaves = nw.new_node(
        nodegroup_stem_branch_leaves(leaves=leaves).name, input_kwargs={"Curve": set_position_2}
    )

    join_geometry_1 = nw.new_node(Nodes.JoinGeometry, input_kwargs={"Geometry": [stembranchgeometry, stembranchleaves]})
    ...
    join_geometry_2 = nw.new_node(Nodes.JoinGeometry, input_kwargs={"Geometry": [realize_instances, join_geometry_1]})
...
def nodegroup_stem_branch_selection(nw: NodeWrangler):
    ...
    random_value = nw.new_node(Nodes.RandomValue, input_kwargs={"Min": 0.0, "Max": 1.0, "Seed": seed})
    less_equal = nw.new_node(
        Nodes.Compare, input_kwargs={0: random_value, 1: threshold}, attrs={"operation": "LESS_EQUAL"}
    )

    op_and = nw.new_node(Nodes.BooleanMath, input_kwargs={0: colorramp_1.outputs["Color"], 1: less_equal})
...
def nodegroup_stem_leaves(nw: NodeWrangler, leaves):
    ...
    random_value_2 = nw.new_node(
        Nodes.RandomValue, input_kwargs={"Max": (0.5, 0.5, 6.28), "Seed": 30}, attrs={"data_type": "FLOAT_VECTOR"}
    )

    rotate_instances = nw.new_node(
        Nodes.RotateInstances,
        input_kwargs={"Instances": instance_on_points_1, "Rotation": random_value_2.outputs["Value"]},
    )
...
def nodegroup_main_flower_setting(nw: NodeWrangler, flowers):
    ...
    transform = nw.new_node(
        Nodes.Transform, input_kwargs={"Geometry": object_info_2.outputs["Geometry"], "Scale": (scale, scale, scale)}
    )
...
def nodegroup_stem_rotation(nw: NodeWrangler):
    ...
    multiply_2 = nw.new_node(Nodes.Math, input_kwargs={0: float_curve, 1: 1.2}, attrs={"operation": "MULTIPLY"})

    vector_rotate = nw.new_node(
        Nodes.VectorRotate,
        input_kwargs={"Vector": position, "Center": multiply.outputs["Vector"], "Angle": multiply_2},
        attrs={"rotation_type": "X_AXIS"},
    )

    noise_texture = nw.new_node(Nodes.NoiseTexture, input_kwargs={"Scale": 0.3})

    add_1 = nw.new_node(Nodes.VectorMath, input_kwargs={0: (-0.5, -0.5, -0.5), 1: noise_texture.outputs["Color"]})

    group_output = nw.new_node(
        Nodes.GroupOutput, input_kwargs={"Rotation": vector_rotate, "Noise": add_1.outputs["Vector"]}
    )
...
def nodegroup_stem_geometry(nw: NodeWrangler):
    ...
    curve_circle = nw.new_node(Nodes.CurveCircle, input_kwargs={"Resolution": 10, "Radius": rad})
...
def geo_flowerplant(nw: NodeWrangler, **kwargs):
    ...
    stemrotation = nw.new_node(nodegroup_stem_rotation().name, input_kwargs={"Geometry": curve_line})
    ...
    resample_curve_1 = nw.new_node(Nodes.ResampleCurve, input_kwargs={"Curve": set_position, "Count": 150})

    stemleaves = nw.new_node(nodegroup_stem_leaves(leaves=leaves).name, input_kwargs={"Curve": resample_curve_1})

    join_geometry = nw.new_node(Nodes.JoinGeometry, input_kwargs={"Geometry": [stemgeometry, stemleaves]})
    ...
    for version in range(num_versions):
        ...
        random_value_1 = nw.new_node(
            Nodes.RandomValue, input_kwargs={"Min": (0.4, 0.4, 0.4)}, attrs={"data_type": "FLOAT_VECTOR"}
        )
        ...
        rotate_instances_1 = nw.new_node(
            Nodes.RotateInstances,
            input_kwargs={"Instances": instance_on_points_2, "Rotation": (random_value_4, "Value")},
        )
        realize_instances_1 = nw.new_node(Nodes.RealizeInstances, input_kwargs={"Geometry": rotate_instances_1})
    ...
    join_geometry_1 = nw.new_node(
        Nodes.JoinGeometry, input_kwargs={"Geometry": [join_geometry, realize_instances] + branches}
    )

    z_rotate = uniform(0, 6.28, size=(1,))[0]
    transform = nw.new_node(
        Nodes.Transform, input_kwargs={"Geometry": join_geometry_1, "Rotation": (0.0, 0.0, z_rotate)}
    )
