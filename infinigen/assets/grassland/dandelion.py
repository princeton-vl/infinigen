...
def nodegroup_pedal_stem_head_geometry(nw: NodeWrangler):
    ...
    uv_sphere_1 = nw.new_node(
        Nodes.MeshUVSphere, input_kwargs={"Segments": 64, "Radius": group_input.outputs["Radius"]}
    )

    transform_1 = nw.new_node(
        Nodes.Transform, input_kwargs={"Geometry": uv_sphere_1, "Translation": group_input.outputs["Translation"]}
    )
...
def nodegroup_pedal_stem_end_geometry(nw: NodeWrangler):
    ...
    uv_sphere = nw.new_node(Nodes.MeshUVSphere, input_kwargs={"Segments": 64, "Radius": 0.04})

    vector = nw.new_node(Nodes.Vector)
    vector.vector = (uniform(0.45, 0.7), uniform(0.45, 0.7), uniform(2, 3))

    transform = nw.new_node(Nodes.Transform, input_kwargs={"Geometry": uv_sphere, "Scale": vector})
    ...
    instance_on_points_1 = nw.new_node(
        ...
        input_kwargs={"Points": transform, "Instance": cone.outputs["Mesh"], "Rotation": align_euler_to_vector_1},
    )
...
def nodegroup_pedal_stem_branch_shape(nw: NodeWrangler):
    ...
    curve_circle_1 = nw.new_node(
        Nodes.CurveCircle, input_kwargs={"Resolution": pedal_stem_branches_num, "Radius": group_input.outputs["Radius"]}
    )
    ...
    multiply = nw.new_node(
        Nodes.Math, input_kwargs={0: float_curve, 1: uniform(0.15, 0.4)}, attrs={"operation": "MULTIPLY"}
    )
    ...
    combine_xyz_2 = nw.new_node(
        Nodes.CombineXYZ,
        input_kwargs={"X": random_value_1.outputs[1], "Y": random_value_2.outputs[1], "Z": random_value.outputs[1]},
    )

    rotate_instances = nw.new_node(
        Nodes.RotateInstances, input_kwargs={"Instances": instance_on_points, "Rotation": combine_xyz_2}
    )

    random_value_3 = nw.new_node(Nodes.RandomValue, input_kwargs={2: 0.8000})

    scale_instances = nw.new_node(
        Nodes.ScaleInstances, input_kwargs={"Instances": rotate_instances, "Scale": random_value_3.outputs[1]}
    )

    group_output = nw.new_node(
        Nodes.GroupOutput, input_kwargs={"Instances": scale_instances}, attrs={"is_active_output": True}
    )
...
def nodegroup_pedal_stem_branch_contour(nw: NodeWrangler):
    ...
    pedal_stem_branch_rsample = nw.new_node(Nodes.Value, label="pedal_stem_branch_rsample")
    pedal_stem_branch_rsample.outputs[0].default_value = 10.0

    resample_curve = nw.new_node(
        Nodes.ResampleCurve, input_kwargs={"Curve": realize_instances, "Count": pedal_stem_branch_rsample}
    )
    ...
    random_value = nw.new_node(Nodes.RandomValue, input_kwargs={2: 0.05, 3: 0.35, "ID": capture_attribute.outputs[5]})

    multiply = nw.new_node(
        Nodes.Math, input_kwargs={0: float_curve, 1: random_value.outputs[1]}, attrs={"operation": "MULTIPLY"}
    )
...
def nodegroup_pedal_stem_branch_geometry(nw: NodeWrangler):
    ...
    set_curve_radius_1 = nw.new_node(
        Nodes.SetCurveRadius, input_kwargs={"Curve": group_input.outputs["Curve"], "Radius": 1.0}
    )

    curve_circle_2 = nw.new_node(Nodes.CurveCircle, input_kwargs={"Radius": uniform(0.001, 0.0025), "Resolution": 4})
    ...
    transform_2 = nw.new_node(
        Nodes.Transform, input_kwargs={"Geometry": curve_to_mesh_1, "Translation": group_input.outputs["Translation"]}
    )
...
def nodegroup_pedal_stem_geometry(nw: NodeWrangler):
    ...
    set_curve_radius = nw.new_node(
        Nodes.SetCurveRadius, input_kwargs={"Curve": quadratic_bezier, "Radius": group_input.outputs["Radius"]}
    )

    curve_circle = nw.new_node(Nodes.CurveCircle, input_kwargs={"Radius": 0.2, "Resolution": 8})
    ...
    group_output = nw.new_node(Nodes.GroupOutput, input_kwargs={"Geometry": set_material_2, "Curve": quadratic_bezier})
...
def nodegroup_pedal_selection(nw: NodeWrangler, params):
    ...
    group_input = nw.new_node(Nodes.GroupInput, expose_input=[("NodeSocketFloat", "num_segments", 0.5)])

    divide = nw.new_node(
        Nodes.Math, input_kwargs={0: index_1, 1: group_input.outputs["num_segments"]}, attrs={"operation": "DIVIDE"}
    )

    less_than = nw.new_node(
        Nodes.Math, input_kwargs={0: divide, 1: params["row_less_than"]}, attrs={"operation": "LESS_THAN"}
    )

    greater_than_1 = nw.new_node(
        Nodes.Math, input_kwargs={0: divide, 1: params["row_great_than"]}, attrs={"operation": "GREATER_THAN"}
    )

    op_and = nw.new_node(Nodes.BooleanMath, input_kwargs={0: less_than, 1: greater_than_1})

    modulo = nw.new_node(
        Nodes.Math, input_kwargs={0: index_1, 1: group_input.outputs["num_segments"]}, attrs={"operation": "MODULO"}
    )

    less_than_1 = nw.new_node(
        Nodes.Math, input_kwargs={0: modulo, 1: params["col_less_than"]}, attrs={"operation": "LESS_THAN"}
    )

    greater_than_2 = nw.new_node(
        Nodes.Math, input_kwargs={0: modulo, 1: params["col_great_than"]}, attrs={"operation": "GREATER_THAN"}
    )

    op_and_1 = nw.new_node(Nodes.BooleanMath, input_kwargs={0: less_than_1, 1: greater_than_2})

    nand = nw.new_node(Nodes.BooleanMath, input_kwargs={0: op_and, 1: op_and_1}, attrs={"operation": "NAND"})
...
def nodegroup_stem_geometry(nw: NodeWrangler):
    ...
    map_range = nw.new_node(
        Nodes.MapRange, input_kwargs={"Value": spline_parameter.outputs["Factor"], 3: 0.4, 4: value}
    )
    ...
    curve_circle_3 = nw.new_node(Nodes.CurveCircle, input_kwargs={"Radius": stem_radius})

    curve_to_mesh_2 = nw.new_node(
        Nodes.CurveToMesh,
        input_kwargs={"Curve": set_curve_radius_2, "Profile Curve": curve_circle_3.outputs["Curve"], "Fill Caps": True},
    )
    ...
    group_output = nw.new_node(Nodes.GroupOutput, input_kwargs={"Mesh": tag_nodegroup(nw, set_material, "stem")})
...
def nodegroup_pedal_stem(nw: NodeWrangler):
    ...
    pedal_stem_geometry = nw.new_node(
        ...
        input_kwargs={"End": pedal_stem_top_point, "Middle": pedal_stem_mid_point, "Radius": pedal_stem_radius},
    )
    ...
    pedal_stem_branch_shape = nw.new_node(
        nodegroup_pedal_stem_branch_shape().name, input_kwargs={"Radius": pedal_stem_top_radius}
    )

    pedal_stem_branch_geometry = nw.new_node(
        nodegroup_pedal_stem_branch_geometry().name,
        input_kwargs={"Curve": pedal_stem_branch_shape, "Translation": pedal_stem_top_point},
    )
    ...
    resample_curve = nw.new_node(Nodes.ResampleCurve, input_kwargs={"Curve": pedal_stem_geometry.outputs["Curve"]})

    pedal_stem_end_geometry = nw.new_node(
        nodegroup_pedal_stem_end_geometry().name, input_kwargs={"Points": resample_curve}
    )

    pedal_stem_head_geometry = nw.new_node(
        nodegroup_pedal_stem_head_geometry().name,
        input_kwargs={"Translation": pedal_stem_top_point, "Radius": pedal_stem_top_radius},
    )
...
def nodegroup_flower_geometry(nw: NodeWrangler, params):
    ...
    num_core_segments = nw.new_node(Nodes.Integer, label="num_core_segments", attrs={"integer": 10})
    num_core_segments.integer = randint(8, 25)

    num_core_rings = nw.new_node(Nodes.Integer, label="num_core_rings", attrs={"integer": 10})
    num_core_rings.integer = randint(8, 20)

    uv_sphere_2 = nw.new_node(
        Nodes.MeshUVSphere,
        input_kwargs={"Segments": num_core_segments, "Rings": num_core_rings, "Radius": uniform(0.02, 0.05)},
    )
    ...
    transform = nw.new_node(Nodes.Transform, input_kwargs={"Geometry": uv_sphere_2, "Scale": flower_core_shape})
    ...
    pedal_selection = nw.new_node(
        nodegroup_pedal_selection(params=selection_params).name, input_kwargs={"num_segments": num_core_segments}
    )

    group_input = nw.new_node(Nodes.GroupInput, expose_input=[("NodeSocketGeometry", "Instance", None)])
    ...
    multiply = nw.new_node(Nodes.Math, input_kwargs={0: random_value_1.outputs[1]}, attrs={"operation": "MULTIPLY"})
    ...
    group_output = nw.new_node(
        Nodes.GroupOutput, input_kwargs={"Geometry": tag_nodegroup(nw, join_geometry_1, "flower")}
    )
...
def nodegroup_flower_on_stem(nw: NodeWrangler):
    ...
    group_input = nw.new_node(
        Nodes.GroupInput,
        expose_input=[("NodeSocketGeometry", "Points", None), ("NodeSocketGeometry", "Instance", None)],
    )
    ...
    group_output = nw.new_node(Nodes.GroupOutput, input_kwargs={"Instances": realize_instances_2})
...
def geometry_dandelion_nodes(nw: NodeWrangler, **kwargs):
    ...
    resample_curve = nw.new_node(Nodes.ResampleCurve, input_kwargs={"Curve": quadratic_bezier_1})

    pedal_stem = nw.new_node(nodegroup_pedal_stem().name)

    geometry_to_instance = nw.new_node("GeometryNodeGeometryToInstance", input_kwargs={"Geometry": pedal_stem})

    flower_geometry = nw.new_node(
        nodegroup_flower_geometry(kwargs).name, input_kwargs={"Instance": geometry_to_instance}
    )

    geometry_to_instance_1 = nw.new_node("GeometryNodeGeometryToInstance", input_kwargs={"Geometry": flower_geometry})

    value_2 = nw.new_node(Nodes.Value)
    value_2.outputs[0].default_value = uniform(-0.15, -0.5)

    transform_3 = nw.new_node(Nodes.Transform, input_kwargs={"Geometry": geometry_to_instance_1, "Scale": value_2})

    flower_on_stem = nw.new_node(
        nodegroup_flower_on_stem().name, input_kwargs={"Points": resample_curve, "Instance": transform_3}
    )

    stem_geometry = nw.new_node(nodegroup_stem_geometry().name, input_kwargs={"Curve": quadratic_bezier_1})

    join_geometry_2 = nw.new_node(Nodes.JoinGeometry, input_kwargs={"Geometry": [flower_on_stem, stem_geometry]})
...
def geometry_dandelion_seed_nodes(nw: NodeWrangler, **kwargs):
    ...
    group_output = nw.new_node(Nodes.GroupOutput, input_kwargs={"Geometry": geometry_to_instance})
