...
def geometry_nodes_add_cabinet_top(nw: NodeWrangler):
    ...
    transform_geometry = nw.new_node(
        Nodes.Transform, input_kwargs={"Geometry": set_material, "Translation": combine_xyz}
    )

    join_geometry = nw.new_node(
        Nodes.JoinGeometry, input_kwargs={"Geometry": [group_input.outputs["Geometry"], transform_geometry]}
    )
...
def geometry_node_to_tagged_bbox(nw: NodeWrangler):
    ...
    transform_geometry = nw.new_node(
        Nodes.Transform, input_kwargs={"Geometry": bounding_box, "Scale": (0.9700, 0.9700, 1.000)}
    )
...
class KitchenSpaceFactory(AssetFactory):
    def __init__(self, factory_seed, coarse=False, dimensions=None, island=False):
        ...
        with FixedSeed(factory_seed):
            if dimensions is None:
                dimensions = Vector((uniform(0.7, 1), uniform(1.7, 5), uniform(2.3, WALL_HEIGHT - WALL_THICKNESS)))
    ...
    def create_asset(self, **params):
        ...
        cabinet_bottom_factory = KitchenCabinetFactory(
            self.factory_seed, dimensions=(x, y - 0.15, cabinet_bottom_height), drawer_only=True
        )
        ...
        if not self.island:
            ...
            cabinet_top_factory = KitchenCabinetFactory(
                self.factory_seed, dimensions=(x / 2.0, cabinet_top_width, cabinet_top_height), drawer_only=False
            )
            ...
            if mid_style == "range_hood":
                range_hood_factory = RangeHoodFactory(
                    self.factory_seed, dimensions=(x * 0.66, top_mid_width + 0.15, cabinet_top_height)
                )
                ...
                cabinet_top_mid_factory = KitchenCabinetFactory(
                    self.factory_seed, dimensions=(x * 0.66, top_mid_width, cabinet_top_height * 0.8), drawer_only=False
                )
