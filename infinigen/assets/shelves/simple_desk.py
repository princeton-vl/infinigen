...
def nodegroup_table_legs(nw: NodeWrangler):
    ...
    multiply_1 = nw.new_node(
        Nodes.Math, input_kwargs={1: group_input.outputs["depth"]}, attrs={"operation": "MULTIPLY"}
    )
    ...
    transform_3 = nw.new_node(
        Nodes.Transform, input_kwargs={"Geometry": cylinder.outputs["Mesh"], "Translation": combine_xyz_4}
    )

    combine_xyz_5 = nw.new_node(Nodes.CombineXYZ, input_kwargs={"X": multiply_3, "Y": multiply_4, "Z": multiply_2})

    transform_4 = nw.new_node(
        Nodes.Transform, input_kwargs={"Geometry": cylinder.outputs["Mesh"], "Translation": combine_xyz_5}
    )

    join_geometry_1 = nw.new_node(
        Nodes.JoinGeometry, input_kwargs={"Geometry": [transform, transform_2, transform_3, transform_4]}
    )
...
def nodegroup_table_top(nw: NodeWrangler, tag_support=True):
    ...
    combine_xyz = nw.new_node(
        Nodes.CombineXYZ, input_kwargs={"X": group_input.outputs["width"], "Y": group_input.outputs["depth"], "Z": add}
    )
...
def geometry_nodes(nw: NodeWrangler, **kwargs):
    ...
    table_top = nw.new_node(
        ...
        input_kwargs={"depth": table_depth, "width": table_width, "height": table_height, "thickness": top_thickness},
    )

    set_material = nw.new_node(
        Nodes.SetMaterial,
        input_kwargs={"Geometry": table_top, "Material": surface.shaderfunc_to_material(kwargs["top_material"])},
    )
    ...
    set_material_1 = nw.new_node(
        Nodes.SetMaterial,
        input_kwargs={"Geometry": table_legs, "Material": surface.shaderfunc_to_material(kwargs["leg_material"])},
    )
