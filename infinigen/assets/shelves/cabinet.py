...
def nodegroup_mid_board(nw: NodeWrangler, **kwargs):
    ...
    set_material = nw.new_node(
        Nodes.SetMaterial, input_kwargs={"Geometry": transform_4, "Material": kwargs["material"][0]}
    )
    ...
    set_material_1 = nw.new_node(
        Nodes.SetMaterial, input_kwargs={"Geometry": transform_7, "Material": kwargs["material"][1]}
    )
...
def geometry_door_nodes(nw: NodeWrangler, **kwargs):
    ...
    set_material_2 = nw.new_node(
        Nodes.SetMaterial, input_kwargs={"Geometry": join_geometry_1, "Material": kwargs["frame_material"]}
    )
    ...
    set_material_3 = nw.new_node(
        Nodes.SetMaterial, input_kwargs={"Geometry": knob_handle, "Material": kwargs["frame_material"]}
    )
    ...
    for h in kwargs["attach_height"]:
        ...
        attach = nw.new_node(
            nodegroup_node_group().name, input_kwargs={"attach_height": attach_height, "door_width": door_width}
        )

        set_material_1 = nw.new_node(
            Nodes.SetMaterial, input_kwargs={"Geometry": attach, "Material": get_shelf_material("metal")}
        )
...
def geometry_cabinet_nodes(nw: NodeWrangler, **kwargs):
    ...
    set_material = nw.new_node(
        Nodes.SetMaterial, input_kwargs={"Geometry": join_geometry_a, "Material": get_shelf_material("metal")}
    )

    join_geometry = nw.new_node(
        Nodes.JoinGeometry, input_kwargs={"Geometry": [shelf_info.outputs["Geometry"]] + doors + [set_material]}
    )
