...
def nodegroup_screw_head(nw: NodeWrangler):
    ...
    group_output = nw.new_node(
        Nodes.GroupOutput, input_kwargs={"Geometry": realize_instances}, attrs={"is_active_output": True}
    )
...
def nodegroup_attachment(nw: NodeWrangler):
    ...
    combine_xyz = nw.new_node(
        Nodes.CombineXYZ, input_kwargs={"X": add, "Y": add_1, "Z": group_input.outputs["attach_thickness"]}
    )
    ...
    subtract = nw.new_node(
        Nodes.Math, input_kwargs={0: multiply, 1: group_input.outputs["attach_gap"]}, attrs={"operation": "SUBTRACT"}
    )
    ...
    multiply_2 = nw.new_node(
        Nodes.Math, input_kwargs={0: group_input.outputs["depth"], 1: -0.5000}, attrs={"operation": "MULTIPLY"}
    )

    add_2 = nw.new_node(Nodes.Math, input_kwargs={0: multiply_1, 1: multiply_2})

    combine_xyz_1 = nw.new_node(
        Nodes.CombineXYZ, input_kwargs={"X": subtract_1, "Y": add_2, "Z": group_input.outputs["attach_z_translation"]}
    )
...
def nodegroup_division_board(nw: NodeWrangler, material, tag_support=False):
    ...
    combine_xyz_1 = nw.new_node(
        Nodes.CombineXYZ,
        input_kwargs={"X": group_input.outputs["x_translation"], "Z": group_input.outputs["z_translation"]},
    )
...
def nodegroup_bottom_board(nw: NodeWrangler):
    ...
    combine_xyz = nw.new_node(
        Nodes.CombineXYZ,
        input_kwargs={"X": group_input.outputs["width"], "Y": group_input.outputs["thickness"], "Z": add},
    )
    ...
    subtract = nw.new_node(
        Nodes.Math, input_kwargs={0: multiply, 1: group_input.outputs["y_gap"]}, attrs={"operation": "SUBTRACT"}
    )

    multiply_1 = nw.new_node(Nodes.Math, input_kwargs={0: add}, attrs={"operation": "MULTIPLY"})

    combine_xyz_1 = nw.new_node(
        Nodes.CombineXYZ, input_kwargs={"X": group_input.outputs["x_translation"], "Y": subtract, "Z": multiply_1}
    )
...
def nodegroup_side_board(nw: NodeWrangler):
    ...
    combine_xyz_1 = nw.new_node(
        Nodes.CombineXYZ, input_kwargs={"X": group_input.outputs["x_translation"], "Z": multiply}
    )
...
def geometry_nodes(nw: NodeWrangler, **kwargs):
    ...
    back_board = nw.new_node(
        ...
        input_kwargs={"width": add_side, "thickness": backboard_thickness, "height": add_2, "depth": shelf_depth},
    )
    ...
    join_geometry = nw.new_node(
        Nodes.JoinGeometry, input_kwargs={"Geometry": [back_board] + side_boards + bottom_boards}
    )
        ...
        for j in range(len(kwargs["division_board_z_translation"])):
            ...
            division_board = nw.new_node(
                nodegroup_division_board(
                    material=kwargs["board_material"], tag_support=kwargs.get("tag_support", False)
                ).name,
                input_kwargs={
...
class LargeShelfBaseFactory(AssetFactory):
    ...
    def get_asset_params(self, i=0):
        ...
        if params.get("bottom_board_height", None) is None:
            params["bottom_board_height"] = np.clip(normal(0.083, 0.01), 0.05, 0.11) * np.random.choice(
                [1.0, 0.0], p=[0.8, 0.2]
            )
...
class LargeShelfFactory(LargeShelfBaseFactory):
    def sample_params(self):
        params = dict()
        params["Dimensions"] = (uniform(0.25, 0.35), uniform(0.3, 2.0), uniform(0.9, 2.0))
