...
def nodegroup_truncated_leaf_selection(nw: NodeWrangler):
    ...
    multiply_1 = nw.new_node(
        Nodes.Math, input_kwargs={0: multiply, 1: uniform(0.98, 0.99)}, attrs={"operation": "MULTIPLY"}
    )
    ...
    multiply_2 = nw.new_node(
        Nodes.Math, input_kwargs={0: multiply, 1: np.clip(normal(0.65, 0.2), 0.7, 0.5)}, attrs={"operation": "MULTIPLY"}
    )
...
def nodegroup_leaf_rotate_downward(nw: NodeWrangler):
    ...
    add2 = nw.new_node(Nodes.Math, input_kwargs={0: multiply.outputs["Value"], 1: -1.57})
...
def nodegroup_truncated_stem_geometry(nw: NodeWrangler):
    ...
    modulo = nw.new_node(Nodes.Math, input_kwargs={0: index_2, 1: randint(6, 10)}, attrs={"operation": "MODULO"})
...
def nodegroup_tree_trunk_geometry(nw: NodeWrangler, radius):
    ...
    subdivide_mesh = nw.new_node(Nodes.SubdivideMesh, input_kwargs={"Mesh": curve_to_mesh, "Level": 2})
...
def geometry_palm_tree_nodes(nw: NodeWrangler, truncatedstem_chance=0.4, **kwargs):
    ...
    resample_curve = nw.new_node(
        Nodes.ResampleCurve, input_kwargs={"Curve": quadratic_bezier, "Length": 0.02}, attrs={"mode": "LENGTH"}
    )
    ...
    set_material_1 = nw.new_node(
        Nodes.SetMaterial,
        input_kwargs={"Geometry": transform, "Material": surface.shaderfunc_to_material(shader_trunk)},
    )
    ...
    join_geometry_1 = nw.new_node(Nodes.JoinGeometry, input_kwargs={"Geometry": [set_material_1, leafontop]})
