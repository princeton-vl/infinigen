...
def nodegroup_nodegroup_shape_with_jigsaw(nw: NodeWrangler):
    ...
    colorramp = nw.new_node(Nodes.ColorRamp, input_kwargs={"Fac": jigsaw.outputs["Distance"]})
    ...
    multiply = nw.new_node(
        Nodes.Math, input_kwargs={0: group_input.outputs["Jigsaw Depth"], 1: 0.0}, attrs={"operation": "MULTIPLY"}
    )

    multiply_add = nw.new_node(
        Nodes.Math,
        input_kwargs={0: colorramp.outputs["Color"], 1: multiply, 2: group_input.outputs["Leaf Shape"]},
...
def nodegroup_nodegroup_vein_coord_003(nw: NodeWrangler):
    ...
    node_utils.assign_curve(
        ...
        [(0.0, 0.0), (0.0182, 0.05), (0.2909, 0.2199), (0.4182, 0.3063), (0.7045, 0.3), (1.0, 0.8562)],
...
def nodegroup_nodegroup_vein_coord(nw: NodeWrangler):
    ...
    node_utils.assign_curve(
        vein_shape.mapping.curves[0],
        [(0.0, 0.0), (0.0182, 0.05), (0.3364, 0.2386), (0.6045, 0.4812), (0.7, 0.725), (0.8273, 0.8437), (1.0, 1.0)],
...
def nodegroup_nodegroup_vein_coord_002(nw: NodeWrangler):
    ...
    vein_shape = nw.new_node(Nodes.FloatCurve, input_kwargs={"Value": divide}, label="Vein Shape")
    node_utils.assign_curve(
        vein_shape.mapping.curves[0], [(0.0, 0.0), (0.0182, 0.05), (0.3364, 0.2386), (0.8091, 0.7312), (1.0, 0.9937)]
    )
...
def nodegroup_nodegroup_shape(nw: NodeWrangler, leaf_contour_control_points=None):
    ...
    if leaf_contour_control_points is not None:
        ...
        node_utils.assign_curve(
            leaf_shape.mapping.curves[0],
            [(0.0, 0.0), (0.15, 0.25), (0.3818, 0.35), (0.6273, 0.3625), (0.7802, 0.2957), (0.8955, 0.2), (1.0, 0.0)],
        )

    multiply = nw.new_node(
        Nodes.Math, input_kwargs={0: leaf_shape, 1: group_input.outputs["scale"]}, attrs={"operation": "MULTIPLY"}
    )

    subtract_1 = nw.new_node(
        Nodes.Math, input_kwargs={0: length.outputs["Value"], 1: multiply}, attrs={"operation": "SUBTRACT"}
    )

    group_output = nw.new_node(Nodes.GroupOutput, input_kwargs={"Leaf Shape": subtract_1, "Value": multiply})
...
def nodegroup_nodegroup_leaf_gen(nw: NodeWrangler, leaf_contour_control_points=None):
    ...
    delete_geometry = nw.new_node(Nodes.DeleteGeometry, input_kwargs={"Geometry": set_position, "Selection": less_than})
...
def nodegroup_nodegroup_leaf_rotate_x(nw: NodeWrangler):
    ...
    group_input = nw.new_node(
        Nodes.GroupInput, expose_input=[("NodeSocketGeometry", "Geometry", None), ("NodeSocketFloat", "To Max", -0.4)]
    )
    ...
    set_position_1 = nw.new_node(
        Nodes.SetPosition, input_kwargs={"Geometry": group_input.outputs["Geometry"], "Position": vector_rotate}
    )
...
def nodegroup_nodegroup_leaf_shader(nw: NodeWrangler):
    ...
    diffuse_bsdf = nw.new_node(Nodes.DiffuseBSDF, input_kwargs={"Color": group_input.outputs["Color"]})
...
def shader_stem_material(nw: NodeWrangler, stem_color_hsv=None):
    ...
    principled_bsdf = nw.new_node(Nodes.PrincipledBSDF, input_kwargs={"Base Color": hsv2rgba(stem_color_hsv)})
