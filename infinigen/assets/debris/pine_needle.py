...
def nodegroup_pine_needle(nw: NodeWrangler):
    ...
    curve_circle = nw.new_node(
        Nodes.CurveCircle, input_kwargs={"Resolution": 6, "Radius": group_input.outputs["Radius"]}
    )

    curve_to_mesh = nw.new_node(
        Nodes.CurveToMesh, input_kwargs={"Curve": quadratic_bezier, "Profile Curve": curve_circle.outputs["Curve"]}
    )
...
class PineNeedleFactory(AssetFactory):
    def sample_params(self):
        s = N(1, 0.2)
        return {"Scale": 0.04 * s, "Bend": 0.03 * s * N(1, 0.2), "Radius": 0.001 * s * N(1, 0.2)}

    def create_asset(self, **_):
        obj = butil.spawn_vert("pine_needle")
        butil.modify_mesh(obj, "NODES", apply=True, node_group=nodegroup_pine_needle(), ng_inputs=self.sample_params())
