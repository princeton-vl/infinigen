...
class BoulderFactory(AssetFactory):
    ...
    def create_placeholder(self, boulder_scale=1, **kwargs) -> bpy.types.Object:
        ...
        if self.do_voronoi:
            ...
            butil.modify_mesh(obj, "DISPLACE", texture=voronoi_texture, strength=0.01, mid_level=0)
    ...
    def geo_extrusion(nw: NodeWrangler, extrude_scale=1):
        ...
        for prob, extrude, scale in extrude_configs:
            ...
            face_area_stats = nw.new_node(
                Nodes.AttributeStatistic, [geometry, None, face_area], attrs={"domain": "FACE"}
            ).outputs
            selection = reduce(
                lambda *xs: nw.boolean_math("AND", *xs),
                [top_facing, nw.bernoulli(prob), nw.compare("GREATER_THAN", face_area, face_area_stats["Mean"])],
            )
            geometry, top, side = nw.new_node(
                Nodes.ExtrudeMesh, [geometry, selection, None, nw.uniform(extrude * 0.5, extrude)]
            ).outputs
            geometry = nw.new_node(Nodes.ScaleElements, [geometry, top, nw.uniform(scale * 0.5, scale)])
            tops.append(top)
