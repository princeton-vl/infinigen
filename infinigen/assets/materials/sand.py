...
def shader_SAND(
    nw, color=("palette", "desert"), random_seed=0, wet=False, wet_part=("uniform", 0.2, 0.25), *args, **kwargs
):
        ...
        offsets.append(
            nw.multiply(
                nw.add(
                    ...
                    nw.new_node(
                        Nodes.WaveTexture,
                        [nw.add(position, position_shift1), nw.scalar_multiply(wave_scale_node, 0.98), wave_distortion],
                    ),
                    nw.multiply(
                        nw.new_node(
                            Nodes.NoiseTexture,
                            [nw.add(position, position_shift2), None, noise_scale, noise_detail, noise_roughness],
                        ),
                        Vector([1] * 3),
                    ),
                ),
...
def apply(objs, selection=None, **kwargs):
    ...
    surface.add_material(
        objs, shader_SAND, selection=selection, input_kwargs={"obj": objs[0] if isinstance(objs, list) else objs}
    )
