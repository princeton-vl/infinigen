...
def shader_mud(nw: NodeWrangler):
    ...
    material_output = nw.new_node(
        Nodes.MaterialOutput, input_kwargs={"Surface": principled_bsdf_2}, attrs={"is_active_output": True}
    )
...
def geo_mud(nw: NodeWrangler, random_seed=0, selection=None):
    ...
    with FixedSeed(random_seed):
        ...
        noise_texture_4 = nw.new_node(
            Nodes.NoiseTexture, input_kwargs={"Vector": mix_2, "Scale": nw.new_value(N(50, 5), "noise_texture_4_scale")}
        )

        voronoi_texture_2 = nw.new_node(
            Nodes.VoronoiTexture,
            input_kwargs={"Vector": mix_2, "Scale": nw.new_value(N(3.0000, 0.5), "voronoi_texture_2_scale")},
        )
        ...
        float_curve_1 = nw.new_node(Nodes.FloatCurve, input_kwargs={"Value": colorramp_1.outputs["Color"]})

        node_utils.assign_curve(
            float_curve_1.mapping.curves[0], [(0.0000, 0.0000), (0.3386, 0.0844), (0.8114, 0.6312), (1.0000, 0.7656)]
        )
        # node_utils.assign_curve(
        ...
        multiply = nw.new_node(
            Nodes.VectorMath, input_kwargs={0: float_curve_1, 1: value_6}, attrs={"operation": "MULTIPLY"}
        )

        add = nw.new_node(
            Nodes.VectorMath, input_kwargs={0: noise_texture_4.outputs["Fac"], 1: multiply.outputs["Vector"]}
        )

        normal = nw.new_node(Nodes.InputNormal)

        multiply_1 = nw.new_node(
            Nodes.VectorMath, input_kwargs={0: add.outputs["Vector"], 1: normal}, attrs={"operation": "MULTIPLY"}
        )
