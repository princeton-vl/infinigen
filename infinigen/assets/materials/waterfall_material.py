...
def waterfall_shader(nw: NodeWrangler):
    ...
    principled_bsdf = nw.new_node(
        Nodes.PrincipledBSDF, input_kwargs={"Base Color": rgb, "Roughness": 0.0, "IOR": 1.33, "Transmission": 1.0}
    )
    ...
    musgrave_texture_1 = nw.new_node(
        Nodes.MusgraveTexture, input_kwargs={"Vector": mapping, "Scale": 3.3, "Detail": 13.0, "Dimension": 0.3}
    )
    ...
    mix_shader_1 = nw.new_node(Nodes.MixShader, input_kwargs={1: mix_shader, 2: principled_bsdf_1})

    volume_scatter = nw.new_node("ShaderNodeVolumeScatter", input_kwargs={"Color": (0.5841, 0.7339, 0.8, 1.0)})

    material_output = nw.new_node(
        Nodes.MaterialOutput, input_kwargs={"Surface": mix_shader_1, "Volume": volume_scatter}
    )
...
def geometry_geo_water(nw: NodeWrangler):
    ...
    add = nw.new_node(Nodes.VectorMath, input_kwargs={0: position, 1: (630.0, 564.0, 374.0)})
    ...
    multiply = nw.new_node(
        Nodes.VectorMath, input_kwargs={0: musgrave_texture, 1: (0.0, 0.0, 0.0128)}, attrs={"operation": "MULTIPLY"}
    )
