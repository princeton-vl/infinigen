...
def nodegroup_rotate2_d_002(nw: NodeWrangler, rand=True, **input_kwargs):
    ...
    multiply = nw.new_node(
        Nodes.Math, input_kwargs={0: group_input.outputs["Value3"], 1: 0.0175}, attrs={"operation": "MULTIPLY"}
    ...
    group_output = nw.new_node(
        Nodes.GroupOutput, input_kwargs={"Value": subtract, "Value1": add}, attrs={"is_active_output": True}
    )
...
def shader_eyeball_fish(nw: NodeWrangler):
    ...
    mix = nw.new_node(
        Nodes.MixRGB, input_kwargs={"Fac": 0.0200, "Color1": mapping, "Color2": noise_texture.outputs["Color"]}
    )
    ...
    multiply = nw.new_node(
        Nodes.Math, input_kwargs={0: separate_xyz.outputs["Z"], 1: 0.3000}, attrs={"operation": "MULTIPLY"}
    )

    multiply_1 = nw.new_node(Nodes.Math, input_kwargs={0: multiply, 1: multiply}, attrs={"operation": "MULTIPLY"})

    multiply_2 = nw.new_node(
        Nodes.Math, input_kwargs={0: separate_xyz.outputs["Y"], 1: 0.8000}, attrs={"operation": "MULTIPLY"}
    )
    ...
    mapping_1 = nw.new_node(
        Nodes.Mapping, input_kwargs={"Vector": attribute_1.outputs["Color"], "Scale": (1.0000, 100.0000, 1.0000)}
    )

    mix_1 = nw.new_node(
        Nodes.MixRGB, input_kwargs={"Fac": 0.3000, "Color1": mapping_1, "Color2": attribute_1.outputs["Color"]}
    )

    noise_texture_1 = nw.new_node(Nodes.NoiseTexture, input_kwargs={"Vector": mix_1, "Scale": 10.0000})

    mix_2 = nw.new_node(
        Nodes.MixRGB, input_kwargs={"Fac": 0.7000, "Color1": noise_texture_1.outputs["Fac"], "Color2": mix_1}
    )
    ...
    mapping_2 = nw.new_node(
        Nodes.Mapping, input_kwargs={"Vector": attribute_1.outputs["Color"], "Scale": (20.0000, 1.0000, 1.0000)}
    )

    mix_3 = nw.new_node(
        Nodes.MixRGB, input_kwargs={"Fac": 0.3000, "Color1": mapping_2, "Color2": attribute_1.outputs["Color"]}
    )

    noise_texture_2 = nw.new_node(Nodes.NoiseTexture, input_kwargs={"Vector": mix_3, "Scale": 10.0000})

    mix_4 = nw.new_node(
        Nodes.MixRGB, input_kwargs={"Fac": 0.7000, "Color1": noise_texture_2.outputs["Fac"], "Color2": mix_3}
    )
    ...
    bright_contrast = nw.new_node(
        "ShaderNodeBrightContrast", input_kwargs={"Color": mix_5, "Bright": 0.6000, "Contrast": 1.5000}
    )

    scale1 = U(0.65, 1.2)

    multiply_6 = nw.new_node(
        Nodes.Math, input_kwargs={0: separate_xyz.outputs["Z"], 1: scale1}, attrs={"operation": "MULTIPLY"}
    )

    multiply_7 = nw.new_node(Nodes.Math, input_kwargs={0: multiply_6, 1: multiply_6}, attrs={"operation": "MULTIPLY"})

    multiply_8 = nw.new_node(
        Nodes.Math, input_kwargs={0: separate_xyz.outputs["Y"], 1: scale1}, attrs={"operation": "MULTIPLY"}
    )
    ...
    add_4 = nw.new_node(
        Nodes.Math, input_kwargs={0: bright_contrast, 1: colorramp_2.outputs["Color"]}, attrs={"use_clamp": True}
    )

    scale2 = U(0.6, 0.8)
    multiply_10 = nw.new_node(
        Nodes.Math, input_kwargs={0: separate_xyz.outputs["Z"], 1: scale2}, attrs={"operation": "MULTIPLY"}
    )
    ...
    multiply_12 = nw.new_node(
        Nodes.Math, input_kwargs={0: separate_xyz.outputs["Y"], 1: scale2}, attrs={"operation": "MULTIPLY"}
    )
    ...
    mix_8 = nw.new_node(
        Nodes.MixRGB, input_kwargs={"Fac": add_4, "Color1": (0.0000, 0.0000, 0.0000, 1.0000), "Color2": mix_7}
    )
    ...
    principled_bsdf = nw.new_node(
        Nodes.PrincipledBSDF, input_kwargs={"Base Color": mix_8, "Specular": 0.0000, "Roughness": 0.0000}
    )
    ...
    material_output = nw.new_node(
        Nodes.MaterialOutput, input_kwargs={"Surface": mix_shader_1}, attrs={"is_active_output": True}
    )
