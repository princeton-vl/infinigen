...
def shader_aluminumdisp2tut(nw: NodeWrangler, rand=False, **input_kwargs):
    ...
    multiply = nw.new_node(
        Nodes.Math, input_kwargs={0: separate_xyz.outputs["X"], 1: 0.1}, attrs={"operation": "MULTIPLY"}
    )
    if rand:
        multiply.inputs[1].default_value = sample_range(-1, 1)
    multiply_1 = nw.new_node(Nodes.Math, input_kwargs={0: separate_xyz.outputs["Y"]}, attrs={"operation": "MULTIPLY"})
    if rand:
    ...
    mapping_1 = nw.new_node(
        Nodes.Mapping, input_kwargs={"Vector": texture_coordinate.outputs["Object"], "Rotation": combine_xyz}
    )

    mapping = nw.new_node(
        Nodes.VectorMath, input_kwargs={0: mapping_1.outputs["Vector"], 1: (1, 75, 1)}, attrs={"operation": "MULTIPLY"}
    )
    ...
    colorramp_1 = nw.new_node(Nodes.ColorRamp, input_kwargs={"Fac": colorramp_4.outputs["Color"]})
    ...
    colorramp = nw.new_node(Nodes.ColorRamp, input_kwargs={"Fac": colorramp_4.outputs["Color"]})
...
def geo_aluminumdisp2tut(nw: NodeWrangler, rand=False, **input_kwargs):
    ...
    multiply = nw.new_node(Nodes.VectorMath, input_kwargs={0: position, 1: scale}, attrs={"operation": "MULTIPLY"})
    ...
    subtract = nw.new_node(Nodes.VectorMath, input_kwargs={0: mix, 1: value}, attrs={"operation": "SUBTRACT"})
    ...
    capture_attribute = nw.new_node(
        Nodes.CaptureAttribute, input_kwargs={"Geometry": set_position, 1: mix}, attrs={"data_type": "FLOAT_VECTOR"}
    )
