...
def nodegroup_hammered_metal(nw: NodeWrangler):
    ...
    multiply = nw.new_node(
        Nodes.Math, input_kwargs={0: group_input.outputs["Scale"], 1: 20.0000}, attrs={"operation": "MULTIPLY"}
    )
    ...
    multiply_1 = nw.new_node(
        Nodes.Math, input_kwargs={0: group_input.outputs["Scale"], 1: 300.0000}, attrs={"operation": "MULTIPLY"}
    )
    ...
    multiply_3 = nw.new_node(
        Nodes.Math, input_kwargs={0: power, 1: log_uniform(0.001, 0.003)}, attrs={"operation": "MULTIPLY"}
    )
...
def shader_hammered_metal(nw: NodeWrangler, scale=None, base_color=None, seed=None, **kwargs):
    ...
    group = nw.new_node(
        nodegroup_hammered_metal().name, input_kwargs={"Base Color": base_color, "Scale": scale, "Seed": seed}
    )
