...
from .bark_random import nodegroup_apply_geo_matv2, nodegroup_canonical_coord, nodegroup_shader_canonical_coord
...
def nodegroup_birch_mat_helper(nw):
    ...
    noise_texture = nw.new_node(
        Nodes.NoiseTexture, input_kwargs={"Vector": group_input.outputs["Vector"], "Scale": 50.0, "Detail": 10.0}
    )

    map_range = nw.new_node(
        Nodes.MapRange, input_kwargs={"Value": noise_texture.outputs["Fac"], 1: 0.3, 2: 0.4, 3: 1.0, 4: 0.0}
    )
    ...
    map_range_1 = nw.new_node(Nodes.MapRange, input_kwargs={"Value": musgrave_texture, 1: 0.3, 2: 0.5})
    ...
    multiply = nw.new_node(
        Nodes.Math, input_kwargs={0: separate_xyz.outputs["Z"], 1: 10.0}, attrs={"operation": "MULTIPLY"}
    )
    ...
    noise_texture_1 = nw.new_node(Nodes.NoiseTexture, input_kwargs={"Vector": combine_xyz, "Scale": 2.0})

    separate_rgb = nw.new_node(Nodes.SeparateRGB, input_kwargs={"Image": noise_texture_1.outputs["Color"]})

    map_range_2 = nw.new_node(Nodes.MapRange, input_kwargs={"Value": separate_rgb.outputs["B"], 1: 0.341, 2: 0.377})

    map_range_3 = nw.new_node(Nodes.MapRange, input_kwargs={"Value": separate_rgb.outputs["R"], 1: 0.341, 2: 0.377})

    map_range_4 = nw.new_node(Nodes.MapRange, input_kwargs={"Value": separate_rgb.outputs["G"], 1: 0.341, 2: 0.377})
    ...
    map_range_5 = nw.new_node(Nodes.MapRange, input_kwargs={"Value": multiply_add, 2: 2.0})

    colorramp = nw.new_node(Nodes.ColorRamp, input_kwargs={"Fac": map_range_5.outputs["Result"]})
    ...
    mix_1 = nw.new_node(
        Nodes.MixRGB, input_kwargs={"Fac": power, "Color1": mix, "Color2": (0.0232, 0.0144, 0.0021, 1.0)}
    )
    ...
    group_output = nw.new_node(
        Nodes.GroupOutput, input_kwargs={"Color": mix_2, "Result": map_range_6.outputs["Result"]}
    )


@node_utils.to_nodegroup("nodegroup_birch_geo", singleton=False, type="GeometryNodeTree")
def nodegroup_birch_geo(nw):
    group_input = nw.new_node(Nodes.GroupInput, expose_input=[("NodeSocketVector", "Position", (0.0, 0.0, 0.0))])

    separate_xyz = nw.new_node(Nodes.SeparateXYZ, input_kwargs={"Vector": group_input.outputs["Position"]})
    ...
    noise_texture = nw.new_node(Nodes.NoiseTexture, input_kwargs={"Vector": combine_xyz, "Scale": 2.0})

    separate_rgb = nw.new_node(Nodes.SeparateRGB, input_kwargs={"Image": noise_texture.outputs["Color"]})

    map_range = nw.new_node(
        Nodes.MapRange, input_kwargs={"Value": separate_rgb.outputs["G"], 1: 0.341, 2: 0.377, 3: 1.0, 4: 0.0}
    )

    map_range_1 = nw.new_node(
        Nodes.MapRange, input_kwargs={"Value": separate_rgb.outputs["R"], 1: 0.341, 2: 0.377, 3: 1.0, 4: 0.0}
    )
    ...
    multiply_1 = nw.new_node(Nodes.Math, input_kwargs={0: multiply_add, 1: 0.01}, attrs={"operation": "MULTIPLY"})

    noise_texture_1 = nw.new_node(
        Nodes.NoiseTexture, input_kwargs={"Vector": group_input.outputs["Position"], "Scale": 10.0, "Detail": 15.0}
    )

    multiply_2 = nw.new_node(
        Nodes.Math, input_kwargs={0: noise_texture_1.outputs["Fac"], 1: 0.08}, attrs={"operation": "MULTIPLY"}
    )
    ...
    map_range_2 = nw.new_node(Nodes.MapRange, input_kwargs={"Value": musgrave_texture, 1: 0.3, 2: 0.5})
    ...
    multiply_3 = nw.new_node(Nodes.Math, input_kwargs={0: power, 1: 0.03}, attrs={"operation": "MULTIPLY"})
    ...
    multiply_4 = nw.new_node(Nodes.Math, input_kwargs={0: add_1, 1: 5.0}, attrs={"operation": "MULTIPLY"})

    group_output = nw.new_node(Nodes.GroupOutput, input_kwargs={"Factor": multiply_4})


def shader_birch_mat(nw, selection=None):
    attribute = nw.new_node(Nodes.Attribute, attrs={"attribute_name": "initial_position"})

    group = nw.new_node(nodegroup_shader_canonical_coord().name, input_kwargs={"Vector": attribute.outputs["Vector"]})

    group_1 = nw.new_node(nodegroup_birch_mat_helper().name, input_kwargs={"Vector": group})
...
def geo_bark_birch(nw, selection=None):
    ...
    birchgeo = nw.new_node(nodegroup_birch_geo().name, input_kwargs={"Position": canonicalcoord})
