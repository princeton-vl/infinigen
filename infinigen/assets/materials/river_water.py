...
def shader_river_water(nw: NodeWrangler):
    ...
    multiply = nw.new_node(
        Nodes.Math, input_kwargs={1: light_path.outputs["Is Camera Ray"]}, attrs={"operation": "MULTIPLY"}
    )

    transparent_bsdf = nw.new_node(Nodes.TransparentBSDF)

    principled_bsdf = nw.new_node(
        Nodes.PrincipledBSDF, input_kwargs={"Roughness": 0.0000, "IOR": 1.3300, "Transmission": 1.0000}
    )
    ...
    map_range_2 = nw.new_node(
        Nodes.MapRange, input_kwargs={"Value": separate_xyz.outputs["Y"], 2: 20.0000, 3: 1.0000, 4: 6.0000}
    )
    ...
    musgrave_texture = nw.new_node(
        Nodes.MusgraveTexture, input_kwargs={"Vector": geometry.outputs["Position"], "Scale": 11.6400}
    )
    ...
    volume_absorption = nw.new_node(
        "ShaderNodeVolumeAbsorption", input_kwargs={"Color": rgb, "Density": 5.9000 + 0.1 * N()}
    )
...
def geometry_river_water(nw: NodeWrangler):
    ...
    multiply_1 = nw.new_node(
        Nodes.Math, input_kwargs={0: musgrave_texture_1, 1: multiply}, attrs={"operation": "MULTIPLY"}
    )
    ...
    voronoi_texture = nw.new_node(
        Nodes.VoronoiTexture, input_kwargs={"Vector": add_3.outputs["Vector"], "Scale": 0.1000}
    )
    ...
    voronoi_texture_2 = nw.new_node(
        Nodes.VoronoiTexture, input_kwargs={"Vector": add_4.outputs["Vector"], "Scale": 0.1000}
    )
    ...
    add_5 = nw.new_node(
        Nodes.VectorMath, input_kwargs={0: multiply_2.outputs["Vector"], 1: multiply_3.outputs["Vector"]}
    )
    ...
    multiply_4 = nw.new_node(
        Nodes.Math, input_kwargs={0: add_5.outputs["Vector"], 1: ripple_height}, attrs={"operation": "MULTIPLY"}
    )

    add_6 = nw.new_node(Nodes.Math, input_kwargs={0: multiply_1, 1: multiply_4})

    multiply_5 = nw.new_node(
        Nodes.VectorMath, input_kwargs={0: add_6, 1: (0.0000, 0.0000, 1.0000)}, attrs={"operation": "MULTIPLY"}
    )
