...
def shader_slimy(nw, rand=False, **input_kwargs):
    ...
    mapping = nw.new_node(
        Nodes.Mapping, input_kwargs={"Vector": texture_coordinate.outputs["Generated"], "Scale": value}
    )

    musgrave_texture = nw.new_node(
        Nodes.MusgraveTexture, input_kwargs={"Vector": mapping, "Scale": 6.4}, attrs={"musgrave_dimensions": "4D"}
    )
    if rand:
    ...
    mix_1 = nw.new_node(
        Nodes.MixRGB, input_kwargs={"Color1": colorramp_1.outputs["Color"], "Color2": colorramp_4.outputs["Color"]}
    )
...
def geo_slimy(nw, rand=False, **input_kwargs):
    ...
    wave_texture = nw.new_node(Nodes.WaveTexture, input_kwargs={"Vector": noise_texture.outputs["Fac"]})

    colorramp = nw.new_node(Nodes.ColorRamp, input_kwargs={"Fac": wave_texture.outputs["Fac"]})
    ...
    math = nw.new_node(
        Nodes.Math, input_kwargs={0: 1.0, 1: colorramp.outputs["Color"]}, attrs={"operation": "SUBTRACT"}
    )

    normal = nw.new_node(Nodes.InputNormal)

    vector_math_1 = nw.new_node(Nodes.VectorMath, input_kwargs={0: math, 1: normal}, attrs={"operation": "MULTIPLY"})

    value_1 = nw.new_node(Nodes.Value)
    value_1.outputs["Value"].default_value = uniform(0.005, 0.02) if rand else 0.015

    vector_math_2 = nw.new_node(
        Nodes.VectorMath, input_kwargs={0: vector_math_1.outputs["Vector"], 1: value_1}, attrs={"operation": "MULTIPLY"}
    )

    set_position = nw.new_node(
        Nodes.SetPosition, input_kwargs={"Geometry": group_input, "Offset": vector_math_2.outputs["Vector"]}
    )

    capture_attribute = nw.new_node(
        Nodes.CaptureAttribute, input_kwargs={"Geometry": set_position, 1: math}, attrs={"data_type": "FLOAT_VECTOR"}
    )
