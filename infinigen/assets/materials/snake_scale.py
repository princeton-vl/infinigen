...
def nodegroup_scale_shape(nw: NodeWrangler):
    ...
    group_input = nw.new_node(
        Nodes.GroupInput, expose_input=[("NodeSocketFloat", "thickness", 0.0), ("NodeSocketFloat", "length", 1.0)]
    )

    combine_xyz = nw.new_node(Nodes.CombineXYZ, input_kwargs={"X": group_input.outputs["length"]})
    ...
    separate_xyz = nw.new_node(Nodes.SeparateXYZ, input_kwargs={"Vector": spline_parameter.outputs["Factor"]})

    float_curve = nw.new_node(Nodes.FloatCurve, input_kwargs={"Value": separate_xyz.outputs["X"]})
    ...
    multiply = nw.new_node(
        Nodes.Math, input_kwargs={0: float_curve, 1: group_input.outputs["length"]}, attrs={"operation": "MULTIPLY"}
    )

    combine_xyz_1 = nw.new_node(Nodes.CombineXYZ, input_kwargs={"Y": multiply})

    set_position = nw.new_node(Nodes.SetPosition, input_kwargs={"Geometry": resample_curve, "Offset": combine_xyz_1})
    ...
    multiply_1 = nw.new_node(
        Nodes.Math, input_kwargs={0: separate_xyz_1.outputs["Y"], 1: -1.0}, attrs={"operation": "MULTIPLY"}
    )
    ...
    set_position_1 = nw.new_node(Nodes.SetPosition, input_kwargs={"Geometry": set_position, "Position": combine_xyz_2})

    join_geometry = nw.new_node(Nodes.JoinGeometry, input_kwargs={"Geometry": [set_position_1, set_position]})

    convex_hull = nw.new_node(Nodes.ConvexHull, input_kwargs={"Geometry": join_geometry})
    ...
    separate_xyz_2 = nw.new_node(Nodes.SeparateXYZ, input_kwargs={"Vector": (transfer_attribute, "Value")})
    ...
    node_utils.assign_curve(
        ...
        [(0.0114, 0.46110000000000001), (0.51139999999999997, 0.30940000000000001), (1.0, 0.058099999999999999)],
    )

    set_curve_radius = nw.new_node(Nodes.SetCurveRadius, input_kwargs={"Curve": mesh_to_curve, "Radius": float_curve_1})

    combine_xyz_3 = nw.new_node(Nodes.CombineXYZ, input_kwargs={"Y": group_input.outputs["thickness"]})
    ...
    convex_hull_1 = nw.new_node(Nodes.ConvexHull, input_kwargs={"Geometry": curve_to_mesh})

    join_geometry_1 = nw.new_node(Nodes.JoinGeometry, input_kwargs={"Geometry": [join_geometry, resample_curve]})

    group_output = nw.new_node(Nodes.GroupOutput, input_kwargs={"Convex Hull": convex_hull_1, "Curve": join_geometry_1})
...
def nodegroup_scale(nw: NodeWrangler):
    ...
    node_utils.assign_curve(
        float_curve.mapping.curves[0],
        [(0.0, 0.0), (0.45229999999999998, 0.13439999999999999), (0.92949999999999999, 0.1875), (1.0, 0.0)],
    )
    ...
    curve_circle = nw.new_node(Nodes.CurveCircle, input_kwargs={"Resolution": 5, "Radius": 0.02})

    curve_to_mesh = nw.new_node(
        Nodes.CurveToMesh, input_kwargs={"Curve": set_curve_radius, "Profile Curve": curve_circle.outputs["Curve"]}
    )

    join_geometry = nw.new_node(Nodes.JoinGeometry, input_kwargs={"Geometry": curve_to_mesh})

    join_geometry_1 = nw.new_node(
        Nodes.JoinGeometry,
        input_kwargs={
            "Geometry": [join_geometry, nodegroup_nodegroup_nodegroup_scale_shape_011.outputs["Convex Hull"]]
        },
    )

    transform = nw.new_node(
        Nodes.Transform, input_kwargs={"Geometry": join_geometry_1, "Rotation": group_input.outputs["Rotation"]}
    )
...
def geometry_snake_scale(nw: NodeWrangler):
    ...
    named_attribute_1 = nw.new_node(Nodes.NamedAttribute, input_kwargs={"Name": "inside_mouth"})

    add = nw.new_node(Nodes.Math, input_kwargs={0: named_attribute.outputs[1], 1: named_attribute_1.outputs[1]})

    separate_geometry_1 = nw.new_node(
        Nodes.SeparateGeometry, input_kwargs={"Geometry": group_input.outputs["Geometry"], "Selection": add}
    )

    geometry_proximity = nw.new_node(Nodes.Proximity, input_kwargs={"Target": separate_geometry_1.outputs["Selection"]})

    map_range = nw.new_node(
        Nodes.MapRange, input_kwargs={"Value": geometry_proximity.outputs["Distance"], 2: 0.050000000000000003}
    )
    ...
    set_shade_smooth = nw.new_node(Nodes.SetShadeSmooth, input_kwargs={"Geometry": nodegroup_scale_1})
    ...
    align_euler_to_vector = nw.new_node(
        Nodes.AlignEulerToVector, input_kwargs={"Vector": (transfer_attribute, "Value")}, attrs={"axis": "Z"}
    )
    ...
    set_shade_smooth_1 = nw.new_node(Nodes.SetShadeSmooth, input_kwargs={"Geometry": nodegroup_scale_2})
    ...
    align_euler_to_vector_1 = nw.new_node(
        Nodes.AlignEulerToVector, input_kwargs={"Vector": (transfer_attribute_1, "Value")}, attrs={"axis": "Z"}
    )
