...
def shader_rug(nw: NodeWrangler, strength=1.0, **kwargs):
    ...
    vec = nw.new_node(Nodes.MixRGB, [uniform(0.8, 0.9), nw.new_node(Nodes.NoiseTexture, [coord]), coord])
    height = 0, 0, 0, 1
    base_scale = log_uniform(250, 500)
    for scale, thresh in zip([1, 0.75, 0.5], [1, 0.5, 0.33]):
        voronoi = nw.new_node(Nodes.VoronoiTexture, [vec], input_kwargs={"Scale": scale * base_scale}).outputs[0]
        height = nw.new_node(Nodes.MixRGB, [nw.math("GREATER_THAN", voronoi, thresh), voronoi, height])

    displacement = nw.new_node(Nodes.Displacement, input_kwargs={"Scale": strength, "Height": height})
    ...
    if uniform() < 0.2:
        ...
        back_color = hsv2rgba(
            base_hue + uniform(-0.01, 0.01), base_saturation * uniform(0.9, 1.1), base_value * uniform(0.9, 1.1)
        )
    else:
    ...
    color = nw.new_node(
        Nodes.MixRGB,
        [
            nw.build_float_curve(nw.musgrave(uniform(20, 50)), [(0, 1), (uniform(0.3, 0.4), 0), (1, 0)]),
            front_color,
            back_color,
        ],
    )
    roughness = nw.build_float_curve(nw.musgrave(uniform(20, 50)), [(0.5, 0.9), (1, 0.8)])
