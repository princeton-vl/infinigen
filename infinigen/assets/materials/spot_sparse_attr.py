...
def shader_spots_sparse_attr(nw: NodeWrangler, rand=True, **input_kwargs):
    ...
    principled_bsdf = nw.new_node(
        Nodes.PrincipledBSDF, input_kwargs={"Base Color": mix, "Specular": 0.0, "Roughness": colorramp.outputs["Color"]}
    )
...
def geometry_spots_sparse(nw: NodeWrangler, rand=True, **input_kwargs):
    ...
    add = nw.new_node(Nodes.VectorMath, input_kwargs={0: position, 1: value}, attrs={"operation": "MULTIPLY"})

    noise_texture = nw.new_node(Nodes.NoiseTexture, input_kwargs={"Vector": add.outputs["Vector"]})

    mix = nw.new_node(
        Nodes.MixRGB, input_kwargs={"Fac": 0.1, "Color1": add.outputs["Vector"], "Color2": noise_texture.outputs["Fac"]}
    )

    voronoi_texture = nw.new_node(
        Nodes.VoronoiTexture, input_kwargs={"Vector": mix, "Scale": sample_ratio(8, 0.5, 2) if rand else 8.0}
    )

    noise_texture_1 = nw.new_node(
        Nodes.NoiseTexture, input_kwargs={"Vector": add.outputs["Vector"], "Scale": 15.0, "Roughness": 1.0}
    )
    ...
    multiply = nw.new_node(Nodes.VectorMath, input_kwargs={0: subtract, 1: normal}, attrs={"operation": "MULTIPLY"})

    value_1 = nw.new_node(Nodes.Value)
    value_1.outputs[0].default_value = 0

    multiply_1 = nw.new_node(
        Nodes.VectorMath, input_kwargs={0: multiply.outputs["Vector"], 1: value_1}, attrs={"operation": "MULTIPLY"}
    )
