    ...
    if rand:
        for i in range(3):
            mix_1.inputs[7].default_value[i] = (
                colorramp.color_ramp.elements[0].color[i] + colorramp.color_ramp.elements[1].color[i]
            ) / 2
...
def geo_bark(nw: NodeWrangler):
    ...
    multiply_6 = nw.new_node(
        Nodes.VectorMath, input_kwargs={0: multiply_5.outputs["Vector"], 1: value_1}, attrs={"operation": "MULTIPLY"}
    )
