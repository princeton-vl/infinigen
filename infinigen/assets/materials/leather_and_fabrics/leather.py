...
def nodegroup_leather(nw: NodeWrangler):
    ...
    multiply = nw.new_node(
        Nodes.Math, input_kwargs={0: group_input.outputs["Scale"], 1: 10.0000}, attrs={"operation": "MULTIPLY"}
    )
    ...
    multiply_1 = nw.new_node(
        Nodes.Math, input_kwargs={0: group_input.outputs["Scale"], 1: 800.0000}, attrs={"operation": "MULTIPLY"}
    )
    ...
    hue_saturation_value = nw.new_node(
        "ShaderNodeHueSaturation", input_kwargs={"Value": 0.6000, "Color": group_input.outputs["Base Color"]}
    )
    ...
    hue_saturation_value_1 = nw.new_node(
        "ShaderNodeHueSaturation", input_kwargs={"Value": 0.4000, "Color": group_input.outputs["Base Color"]}
    )
    ...
    map_range = nw.new_node(
        Nodes.MapRange, input_kwargs={"Value": mix_2.outputs[2], 3: uniform(0.3, 0.5), 4: uniform(0.5, 0.7)}
    )

    principled_bsdf = nw.new_node(
        Nodes.PrincipledBSDF, input_kwargs={"Base Color": mix_2.outputs[2], "Roughness": map_range.outputs["Result"]}
    )

    multiply_3 = nw.new_node(
        Nodes.Math, input_kwargs={0: mix_1.outputs[2], 1: -0.2000}, attrs={"operation": "MULTIPLY"}
    )

    multiply_4 = nw.new_node(
        Nodes.Math, input_kwargs={0: color_ramp.outputs["Color"], 1: 0.0500}, attrs={"operation": "MULTIPLY"}
    )
...
def shader_leather(nw: NodeWrangler, scale=1.0, base_color=None, seed=None, **kwargs):
    ...
    group = nw.new_node(nodegroup_leather().name, input_kwargs={"Seed": seed, "Scale": scale, "Base Color": base_color})
