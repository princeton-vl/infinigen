...
def nodegroup_node_group(nw: NodeWrangler):
    ...
    map_range_6 = nw.new_node(
        Nodes.MapRange, input_kwargs={"Value": noise_texture_9.outputs["Fac"], 3: 0.6000, 4: 1.4000}
    )
    ...
    map_range = nw.new_node(
        Nodes.MapRange, input_kwargs={"Value": voronoi_texture.outputs["Distance"], 2: 0.0300, 3: 1.0000, 4: 0.0000}
    )
    ...
    map_range_1 = nw.new_node(
        Nodes.MapRange, input_kwargs={"Value": noise_texture_1.outputs["Fac"], 1: 0.5500, 2: 0.5700}
    )
    ...
    map_range_2 = nw.new_node(
        Nodes.MapRange, input_kwargs={"Value": noise_texture_2.outputs["Fac"], 1: 0.6300, 2: 0.6800}
    )

    multiply_2 = nw.new_node(
        Nodes.Math, input_kwargs={0: map_range_2.outputs["Result"], 1: -1.0000}, attrs={"operation": "MULTIPLY"}
    )
    ...
    mix = nw.new_node(
        Nodes.Mix, input_kwargs={0: 0.4000, 2: noise_texture_3.outputs["Fac"], 3: voronoi_texture_1.outputs["Distance"]}
    )

    multiply_3 = nw.new_node(
        Nodes.Math, input_kwargs={0: mix.outputs["Result"], 1: 0.1000}, attrs={"operation": "MULTIPLY"}
    )
    ...
    subtract = nw.new_node(
        Nodes.Math, input_kwargs={0: noise_texture_4.outputs["Fac"]}, attrs={"operation": "SUBTRACT"}
    )
    ...
    map_range_4 = nw.new_node(
        Nodes.MapRange, input_kwargs={"Value": noise_texture_7.outputs["Fac"], 1: 0.5500, 2: 0.5700}
    )
    ...
    subtract_2 = nw.new_node(
        Nodes.Math, input_kwargs={0: noise_texture_6.outputs["Fac"]}, attrs={"operation": "SUBTRACT"}
    )
    ...
    multiply_8 = nw.new_node(
        Nodes.Math, input_kwargs={0: map_range_5.outputs["Result"], 1: 0.0500}, attrs={"operation": "MULTIPLY"}
    )
...
def shader_bumpy_rubber(nw: NodeWrangler, scale=2.0, base_color=None, seed=None):
    ...
    displacement = nw.new_node(
        Nodes.Displacement, input_kwargs={"Height": group.outputs["Displacement"], "Midlevel": 0.0000, "Scale": 0.0010}
    )
