...
def shader_green_transition_succulent(nw: NodeWrangler):
    ...
    less_than = nw.new_node(
        Nodes.Math, input_kwargs={0: separate_xyz.outputs["Z"], 1: 0.85}, attrs={"operation": "LESS_THAN"}
    )

    multiply = nw.new_node(
        Nodes.Math, input_kwargs={0: colorramp_3.outputs["Color"], 1: less_than}, attrs={"operation": "MULTIPLY"}
    )

    multiply_1 = nw.new_node(
        Nodes.Math, input_kwargs={0: multiply, 1: uniform(2.0, 10.0)}, attrs={"operation": "MULTIPLY"}
    )
    ...
    colorramp_1 = nw.new_node(Nodes.ColorRamp, input_kwargs={"Fac": separate_xyz.outputs["Z"]})
    ...
    diffuse_bsdf_1 = nw.new_node(
        Nodes.DiffuseBSDF, input_kwargs={"Color": diffuse_color, "Roughness": uniform(0.3, 0.8)}
    )

    mix_shader_2 = nw.new_node(Nodes.MixShader, input_kwargs={"Fac": multiply_1, 1: principled_bsdf, 2: diffuse_bsdf_1})

    material_output = nw.new_node(Nodes.MaterialOutput, input_kwargs={"Surface": mix_shader_2})
...
def shader_pink_transition_succulent(nw: NodeWrangler):
    ...
    multiply_1 = nw.new_node(
        Nodes.Math, input_kwargs={0: multiply, 1: uniform(2.0, 8.0)}, attrs={"operation": "MULTIPLY"}
    )
    ...
    diffuse_bsdf_1 = nw.new_node(
        Nodes.DiffuseBSDF, input_kwargs={"Color": diffuse_color, "Roughness": uniform(0.0, 0.5)}
    )
...
def shader_green_succulent(nw: NodeWrangler):
    musgrave_texture_1 = nw.new_node(Nodes.MusgraveTexture, input_kwargs={"Scale": uniform(4.0, 15.0)})
    ...
    less_than = nw.new_node(
        Nodes.Math, input_kwargs={0: separate_xyz.outputs["Z"], 1: 1.0}, attrs={"operation": "LESS_THAN"}
    )
...
def shader_yellow_succulent(nw: NodeWrangler):
    musgrave_texture_1 = nw.new_node(Nodes.MusgraveTexture, input_kwargs={"Scale": uniform(5.0, 8.0)})
    ...
    multiply_1 = nw.new_node(
        Nodes.Math, input_kwargs={0: multiply, 1: uniform(1.0, 3.0)}, attrs={"operation": "MULTIPLY"}
    )
    ...
    mix_shader_1 = nw.new_node(Nodes.MixShader, input_kwargs={"Fac": 0.4, 1: principled_bsdf, 2: translucent_bsdf})

    diffuse_bsdf_1 = nw.new_node(Nodes.DiffuseBSDF, input_kwargs={"Color": main_color, "Roughness": uniform(0.2, 1.0)})

    mix_shader_2 = nw.new_node(Nodes.MixShader, input_kwargs={"Fac": multiply_1, 1: mix_shader_1, 2: diffuse_bsdf_1})
...
def shader_whitish_green_succulent(nw: NodeWrangler):
    ...
    l = uniform(0.88, 0.98)
    less_than = nw.new_node(
        Nodes.Math, input_kwargs={0: separate_xyz.outputs["Z"], 1: l - 0.05}, attrs={"operation": "LESS_THAN"}
    )
    ...
    multiply_1 = nw.new_node(
        Nodes.Math, input_kwargs={0: multiply, 1: uniform(1.0, 4.0)}, attrs={"operation": "MULTIPLY"}
    )
    ...
    mix_shader_1 = nw.new_node(Nodes.MixShader, input_kwargs={"Fac": 0.7, 1: principled_bsdf, 2: translucent_bsdf})
    diffuse = hsv2rgba((uniform(0.23, 0.25), uniform(0.40, 0.60), uniform(0.10, 0.15)))
    diffuse_bsdf_1 = nw.new_node(Nodes.DiffuseBSDF, input_kwargs={"Color": diffuse, "Roughness": 0.5})
