...
def nodegroup_apply_value_to_normal(nw: NodeWrangler):
    ...
    group_input = nw.new_node(
        Nodes.GroupInput,
        expose_input=[("NodeSocketFloat", "value", 0.0000), ("NodeSocketFloat", "displacement", 1.0000)],
    )

    scale = nw.new_node(
        Nodes.VectorMath, input_kwargs={0: normal, "Scale": group_input.outputs["value"]}, attrs={"operation": "SCALE"}
    )
    ...
    group_output = nw.new_node(
        Nodes.GroupOutput, input_kwargs={"Vector": scale_1.outputs["Vector"]}, attrs={"is_active_output": True}
    )
...
def shader_cracked_ground(nw: NodeWrangler, random_seed=0):
    ...
    mix = nw.new_node(
        Nodes.MixRGB, input_kwargs={"Fac": attribute.outputs["Fac"], "Color1": hue_saturation_value, "Color2": col_crac}
    )

    mix_2 = nw.new_node(Nodes.MixRGB, input_kwargs={"Fac": attribute_2.outputs["Fac"], "Color1": mix, "Color2": col_2})

    principled_bsdf = nw.new_node(
        Nodes.PrincipledBSDF, input_kwargs={"Base Color": mix_2, "Specular": 0.2000, "Roughness": 0.9000}
    )
...
def geo_cracked_ground(nw: NodeWrangler, selection=None, random_seed=0):
    ...
    map_range = nw.new_node(
        Nodes.MapRange, input_kwargs={"Value": voronoi_texture.outputs["Distance"], 2: wid_crac, 3: 1.0000, 4: 0.0000}
    )
    ...
    map_range_1 = nw.new_node(
        Nodes.MapRange, input_kwargs={"Value": noise_texture_2.outputs["Fac"], 1: subtract_1, 2: add}
    )
    ...
    store_named_attribute_1 = nw.new_node(
        Nodes.StoreNamedAttribute, input_kwargs={"Geometry": store_named_attribute, "Name": "crack", "Value": multiply}
    )
    ...
    applyvaluetonormal_1 = nw.new_node(
        nodegroup_apply_value_to_normal().name, input_kwargs={"value": multiply, "displacement": dep_crac}
    )
    ...
    set_position = nw.new_node(Nodes.SetPosition, input_kwargs={"Geometry": store_named_attribute_2, "Offset": offset})
