...
class DishwasherFactory(AssetFactory):
    ...
    def create_asset(self, **params):
        ...
        butil.modify_mesh(
            obj, "NODES", node_group=nodegroup_dishwasher_geometry(preprocess=True), ng_inputs=self.params, apply=True
        )
...
def nodegroup_dish_rack(nw: NodeWrangler):
    ...
    curve_line = nw.new_node(
        Nodes.CurveLine, input_kwargs={"Start": (0.0000, -1.0000, 0.0000), "End": (0.0000, 1.0000, 0.0000)}
    )
    ...
    combine_xyz_4 = nw.new_node(Nodes.CombineXYZ, input_kwargs={"Y": -1.0000, "Z": group_input.outputs["Height"]})

    curve_line_1 = nw.new_node(Nodes.CurveLine, input_kwargs={"Start": (0.0000, -1.0000, 0.0000), "End": combine_xyz_4})

    geometry_to_instance_1 = nw.new_node("GeometryNodeGeometryToInstance", input_kwargs={"Geometry": curve_line_1})

    multiply = nw.new_node(
        Nodes.Math, input_kwargs={0: group_input.outputs["Amount"], 1: 2.0000}, attrs={"operation": "MULTIPLY"}
    )
    ...
    divide = nw.new_node(
        Nodes.Math, input_kwargs={0: 1.0000, 1: group_input.outputs["Amount"]}, attrs={"operation": "DIVIDE"}
    )
    ...
    set_position_2 = nw.new_node(
        Nodes.SetPosition, input_kwargs={"Geometry": duplicate_elements_2.outputs["Geometry"], "Offset": combine_xyz_3}
    )
    ...
    subtract = nw.new_node(
        Nodes.Math,
        input_kwargs={0: duplicate_elements.outputs["Duplicate Index"], 1: group_input.outputs["Amount"]},
    ...
    transform_1 = nw.new_node(
        Nodes.Transform, input_kwargs={"Geometry": set_position, "Rotation": (0.0000, 0.0000, 1.5708)}
    )
    ...
    subtract_1 = nw.new_node(
        Nodes.Math,
        input_kwargs={0: duplicate_elements_1.outputs["Duplicate Index"], 1: group_input.outputs["Amount"]},
        attrs={"operation": "SUBTRACT"},
    )

    multiply_3 = nw.new_node(Nodes.Math, input_kwargs={0: subtract_1, 1: divide}, attrs={"operation": "MULTIPLY"})
    ...
    multiply_4 = nw.new_node(
        Nodes.Math, input_kwargs={0: group_input.outputs["Height"], 1: 0.8000}, attrs={"operation": "MULTIPLY"}
    )

    combine_xyz_5 = nw.new_node(Nodes.CombineXYZ, input_kwargs={"Z": multiply_4})

    transform_2 = nw.new_node(Nodes.Transform, input_kwargs={"Geometry": quadrilateral_1, "Translation": combine_xyz_5})

    join_geometry = nw.new_node(
        Nodes.JoinGeometry, input_kwargs={"Geometry": [quadrilateral, transform_1, set_position_1, transform_2]}
    )
    ...
    transform = nw.new_node(
        Nodes.Transform,
        input_kwargs={"Geometry": curve_to_mesh, "Rotation": (0.0000, 0.0000, 1.5708), "Scale": combine_xyz_2},
    )

    group_output = nw.new_node(Nodes.GroupOutput, input_kwargs={"Mesh": transform}, attrs={"is_active_output": True})
...
def nodegroup_dishwasher_geometry(nw: NodeWrangler, preprocess: bool = False):
    ...
    multiply_3 = nw.new_node(
        Nodes.Math, input_kwargs={0: group_input.outputs["Width"], 1: 0.1000}, attrs={"operation": "MULTIPLY"}
    )

    multiply_4 = nw.new_node(
        Nodes.Math, input_kwargs={0: group_input.outputs["Height"], 1: 0.9500}, attrs={"operation": "MULTIPLY"}
    )
    ...
    text = nw.new_node(
        nodegroup_text().name,
        input_kwargs={"Translation": combine_xyz_12, "String": group_input.outputs["BrandName"], "Size": multiply_6},
    )

    text = complete_no_bevel(nw, text, preprocess)

    set_material_9 = nw.new_node(
        Nodes.SetMaterial, input_kwargs={"Geometry": text, "Material": group_input.outputs["NameMaterial"]}
    )
    ...
    subtract = nw.new_node(
        Nodes.Math, input_kwargs={0: group_input.outputs["Depth"], 1: multiply_7}, attrs={"operation": "SUBTRACT"}
    )
    ...
    subtract_1 = nw.new_node(
        Nodes.Math, input_kwargs={0: group_input.outputs["Width"], 1: multiply_8}, attrs={"operation": "SUBTRACT"}
    )
    ...
    duplicate_elements = nw.new_node(
        Nodes.DuplicateElements,
        input_kwargs={"Geometry": geometry_to_instance_1, "Amount": group_input.outputs["RackAmount"]},
        attrs={"domain": "INSTANCE"},
    )
    ...
    subtract_2 = nw.new_node(
        Nodes.Math, input_kwargs={0: group_input.outputs["Height"], 1: multiply_11}, attrs={"operation": "SUBTRACT"}
    )
    ...
    set_material_5 = nw.new_node(
        Nodes.SetMaterial, input_kwargs={"Geometry": cube_1, "Material": group_input.outputs["Top"]}
    )
