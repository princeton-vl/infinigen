...
class CubeSphericalMesher:
    def __init__(
        ...
        register_func(
            ...
            dll,
            "stitch_update",
            [POINTER(c_double), POINTER(c_int32), POINTER(c_double), c_int32, POINTER(c_int32), c_int32],
        )
    ...
    def __call__(self, kernels):
        ...
        if self.upscale != -1 and self.upscale != 1:
            ...
            while True:
                ...
                if cnt == 0 and self.complete_depth_test:
                    with Timer("complete_depth_test"):
                        ...
                        if self.verbose:
                            batch_range = tqdm(range(0, W * self.upscale, self.complete_depth_test_relax))
                        else:
                            batch_range = range(0, W * self.upscale, self.complete_depth_test_relax)
                        for b in batch_range:
                            positions = AC(
                                np.zeros(
                                    (
                                        ((H * self.upscale - 1) // self.complete_depth_test_relax + 1)
                                        * ((R * self.upscale - 1) // self.complete_depth_test_relax + 1),
                                        3,
                                    ),
                                    dtype=np.float64,
                                )
            ...
            with Timer(
                ...
                positions = AC(
                    np.zeros(((2 * (W + H) * R * (U * U + 2 * U + 1) + 4 * R * (U + 2)) * 8, 3), dtype=np.float64)
                )
