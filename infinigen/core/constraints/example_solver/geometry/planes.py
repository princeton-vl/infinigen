...
from infinigen.core.constraints.constraint_language.util import blender_objs_from_names, meshes_from_names
...
class Planes:
    ...
    def get_all_planes_deprecated(self, obj, face_mask, tolerance=1e-4) -> tuple[str, int]:
        ...
        for polygon in obj.data.polygons:
            ...
            for name, polygon2_index in unique_planes:
                ...
                if np.allclose(np.cross(normal, plane_normal), 0, rtol=tolerance) and np.allclose(
    ...
    def get_tagged_planes(self, obj: bpy.types.Object, tags: set, fast=True):
        ...
        if not mask.any():
            ...
            logger.warning(f"Attempted to get_tagged_planes {obj.name=} {tags=} but mask was empty, {obj_tags=}")
            return []
    ...
    def extract_tagged_plane(self, obj: bpy.types.Object, tags: set, plane: int):
        ...
        if not mask.any():
            obj_tags = tagging.union_object_tags(obj)
            logger.warning(f"Attempted to extract_tagged_plane {obj.name=} {tags=} but mask was empty, {obj_tags=}")
    ...
    def tagged_plane_mask(
        ...
        mesh_or_face_mask_changed = (
            cache_key not in self._cached_plane_masks or self._mesh_hashes.get(obj_id) != current_hash
        )
    ...
    def _compute_tagged_plane_mask(self, obj, face_mask, plane, tolerance):
        ...
        for candidate_polygon in obj.data.polygons:
            ...
            in_plane = np.allclose(ndot, 1, atol=tolerance) and np.allclose(pdot, 0, atol=tolerance)
