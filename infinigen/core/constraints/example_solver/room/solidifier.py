...
panoramic_rooms = defaultdict(
    float, {
        Semantics.Hallway: ('bool', .1), Semantics.Balcony: ('bool', .8), Semantics.OpenOffice: ('bool', .2),
        Semantics.Office: ('bool', .2),
        Semantics.MeetingRoom: ('bool', .2), Semantics.BreakRoom: ('bool', .2), Semantics.Garage: ('bool', 1)
    }
)

combined_rooms = [({Semantics.Bedroom}, {'non-adjacent': 'none', 'adjacent': 'none'}),
    ({Semantics.Hallway, Semantics.OpenOffice}, {'non-adjacent': 'open', 'adjacent': 'open'}),
    ({Semantics.Hallway, Semantics.LivingRoom, Semantics.DiningRoom, Semantics.StaircaseRoom},
    {
        'non-adjacent': ('weighted_choice', (.3, 'open'), (.3, 'panoramic'), (.4, 'door')),
        'adjacent': ('weighted_choice', (.5, 'open'), (.5, 'door'))
    }),
    ({Semantics.Garage, Semantics.Warehouse}, {'non-adjacent': 'open', 'adjacent': 'open'}),
    ({Semantics.DiningRoom, Semantics.Kitchen},
    {
        'non-adjacent': ('weighted_choice', (.3, 'open'), (.3, 'panoramic'), (.4, 'door')), 'adjacent': 'open'
    }),
    ...
    {'non-adjacent': ('weighted_choice', (.3, 'open'), (.3, 'panoramic'), (.4, 'door')), 'adjacent': 'door'}),
    ({Semantics.OpenOffice, Semantics.Hallway}, {
        'non-adjacent': 'open',
        'adjacent': 'open'
    }),
    ({Semantics.MeetingRoom, Semantics.BreakRoom, Semantics.Hallway, Semantics.OpenOffice}, {
        'non-adjacent': ('weighted_choice', (.5, 'window'), (.5, 'door')),
        'adjacent': 'door'
    })
]
...
window_rooms = defaultdict(
    lambda: 1., {
        Semantics.Utility: ('bool', .3),
        Semantics.Closet: 0.,
        Semantics.Bathroom: ('bool', .5),
        Semantics.Garage: 0.,
        Semantics.Warehouse: 0.
    }
)

wall_cut_prob = 'bool', .5
...
class BlueprintSolidifier:
    ...
    def solidify(self, state):
        ...
        shared_edges = {k: {l.target_name: canonicalize_mls(l.value) for l in obj_st.relations} for k, obj_st in
            valid_rooms(state)}
        exterior = next(k for k in state.objs if room_type(k) == Semantics.Exterior)
        exterior_edges = {r.target_name: canonicalize_mls(r.value) for r in state[exterior].relations}
        ...
        open_cutters, door_cutters, interior_cutters = self.make_interior_cutters(
            self.graph.valid_neighbours, shared_edges, segments,
            exterior_buffer
        )
        window_cutters, entrance_cutters = self.make_exterior_cutters(exterior_edges, exterior_shape)
        all_cutter_lists = [open_cutters, door_cutters, window_cutters, entrance_cutters]
    ...
    def convert_solver_state(
        self, rooms, segments, shared_edges, open_cutters, door_cutters, window_cutters, interior_cutters,
        entrance_cutters
    ):
        obj_states = {}
    ...
    def make_room(self, state, name):
        ...
        exterior = next(k for k in state.objs if room_type(k) == Semantics.Exterior)
        exterior_edge = next(r.value for r in state.objs[exterior].relations if r.target_name == name)
        ...
        if len(exterior_centers) > 0:
            exterior = (np.abs(center[:, np.newaxis, :2] - np.array(exterior_centers)[np.newaxis]).sum(
                -1
            ) < self.constants.wall_thickness * 4).any(-1)
        else:
            exterior = np.zeros(len(obj.data.polygons), dtype=bool)
        write_attr_data(obj, f'{PREFIX}{t.Subpart.Interior.value}', ~exterior, 'BOOLEAN', 'FACE')
        assert len(obj.data.vertices) > 0

        obj.vertex_groups.new(name='visible_')
        butil.modify_mesh(
            obj, 'SOLIDIFY', thickness=self.constants.wall_thickness / 2, offset=-1,
            use_even_offset=True, shell_vertex_group='visible_', use_quality_normals=True
        )
        write_attribute(obj, 'visible_', f'{PREFIX}{t.Subpart.Visible.value}', 'FACE', 'BOOLEAN')
    ...
    def make_interior_cutters(self, neighbours, shared_edges, segments, exterior):
        ...
        for k in shared_edges:
            name_groups[k] = set(i for i, (rt, _) in enumerate(combined_rooms) if room_type(k) in rt)
        dist2entrance = self.compute_dist2entrance(neighbours)
        centroids = {k: np.array(s.centroid.coords[0]) for k, s in segments.items()}
        open_cutters, door_cutters, interior_cutters = defaultdict(dict), defaultdict(dict), defaultdict(dict)
        for k, ses in shared_edges.items():
            for l, se in ses.items():
                ...
                if len(i) > 0 and self.enable_open:
                    ...
                    fn = rg(
                        group['adjacent'] if k in neighbours[l] else
                        group['non-adjacent']
                    )
                else:
                ...
                match fn:
                    case 'open':
                        open_cutters[k][l] = open_cutters[l][k] = self.make_open_cutter(se, exterior)
                    case 'door':
                        door_cutters[k][l] = door_cutters[l][k] = self.make_door_cutter(se, direction)
                    case 'window':
                        interior_cutters[k][l] = interior_cutters[l][k] = self.make_window_cutter(se, False)
                    case 'panoramic':
                        interior_cutters[k][l] = interior_cutters[l][k] = self.make_window_cutter(se, self.level == 0)
    ...
    def make_exterior_cutters(self, exterior_edges, exterior):
        ...
        for k, mls in exterior_edges.items():
            ...
            for ls in mls.geoms:
                ...
                buffered = ls.buffer(.1, single_sided=True)
            ...
            for k, name in names.items():
                if room_type(name) == Semantics.StaircaseRoom:
                    with np.errstate(invalid="ignore"):
                        cutter = polygon2obj(buffer(staircase, -self.constants.wall_thickness / 2))
                    butil.modify_mesh(
                        cutter, 'SOLIDIFY', thickness=self.constants.wall_thickness * 1.2,
                        offset=0
                    )
                    self.tag(cutter)
    ...
    def make_open_cutter(self, es, exterior):
        ...
        es = shapely.remove_repeated_points(linemerge(es) if not isinstance(es, LineString) else es, .01)
        ...
        for ls in es:
            ...
            if len(coords) < 2:
                continue
            coords[0] = line_interpolate_point(LineString(coords[0: 2]), wt / 2 + _eps).coords[0]
            coords[-1] = line_interpolate_point(LineString(coords[-1:-3:-1]), wt / 2 + _eps).coords[0]
            lines.append(coords)
        line = shapely.simplify(shapely.remove_repeated_points(shapely.MultiLineString(lines), .01), .01)
    ...
    def tag(self, obj, visible=True):
        ...
        if visible:
            write_attr_data(obj, f'{PREFIX}{t.Subpart.Visible.value}', full, 'BOOLEAN', 'FACE')
        else:
            write_attr_data(obj, f'{PREFIX}{t.Subpart.Visible.value}', ~full, 'BOOLEAN', 'FACE')
        write_attr_data(obj, f'{PREFIX}{t.Subpart.Interior.value}', full, 'BOOLEAN', 'FACE')
