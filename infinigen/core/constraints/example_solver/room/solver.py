...
from .utils import update_shared, update_exterior, update_contour
from .base import room_type, room_name, valid_rooms, room_level
from .. import state_def
from ...constraint_language.constants import RoomConstants
from ...constraint_language.util import plot_geometry
...
class FloorPlanMoves:
    ...
    def perturb_state(self, state: state_def.State):
        while True:
            k = np.random.choice([k for k in state.objs if room_type(k) != Semantics.Exterior])
    ...
    def extrude_room(self, state, k, out=True):
        ...
        u, v = coords[m:m + 2]
        r = uniform()
        mid = self.constants.unit_cast(u + uniform(.35, .65) * (v - u))
        if max(np.linalg.norm(u - mid), np.linalg.norm(v - mid)) < self.constants.segment_margin:
            if r < .2:
                u = mid
            elif r < .4:
                v = mid
        ...
        if out:
            ...
            if self.constants.fixed_contour:
                c = self.constants.canonicalize(c.intersection(state[room_name(Semantics.Exterior, room_level(k))].polygon))
    ...
    def extrude_room_in(self, state, k):
        ...
        for r in state[k].relations:
            ...
            p = state[l].polygon
            if shapely.distance(p, c) <= stride:
                if np.abs(np.abs(theta) - np.pi / 2) < .1:
                    q = shapely.affinity.translate(p, -stride, 0).union(shapely.affinity.translate(p, stride, 0))
                else:
                    q = shapely.affinity.translate(p, 0, -stride).union(shapely.affinity.translate(p, 0, stride))
    ...
    def move_staircase(self, state):
        ...
        if uniform() < .2:
            ...
            if p.coords[0][0] != self.constants.unit_cast(p.coords[0][0]):
                p = shapely.affinity.translate(p, self.constants.unit / 2, self.constants.unit / 2)
        else:
            p = shapely.affinity.translate(
                p, self.constants.unit * np.random.randint(1 - self.max_stride, self.max_stride),
                self.constants.unit * np.random.randint(1 - self.max_stride, self.max_stride)
            )
        if state[room_name(Semantics.Exterior, len(state.graphs) - 1)].polygon.contains(p.buffer(-.1)):
            names = set()
