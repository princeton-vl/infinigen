...
from infinigen.core.constraints.example_solver.room.base import room_type, room_name, room_level
from infinigen.core.constraints.example_solver.room.solidifier import max_mls
from infinigen.core.constraints.example_solver.room.utils import update_shared, update_exterior
...
class ContourFactory:
    ...
    def add_staircase(self, contour):
        ...
        for _ in range(self.n_trials):
            ...
            skewness = log_uniform(.5, .8)
            if uniform() < .5:
                skewness = 1 / skewness
            width, height = self.constants.unit_cast(np.sqrt(area * skewness).item()), self.constants.unit_cast(
    ...
    def get_length(polygon, point):
        ...
        indices = np.nonzero(np.abs(coords - point[np.newaxis]).sum(-1) < .1)[0]
        if len(indices) > 0:
            i = indices[0]
    ...
    def decorate(self, state):
        if self.constants.fixed_contour:
            if rg(self.long_prob):
                ...
                for k, obj_st in state.objs.items():
                    if room_type(k) not in [Semantics.StaircaseRoom]:
                        ...
                        y -= np.where(np.abs(x) < .1, slope * x, 0)
            ...
            for i in reversed(range(len(state.graphs))):
                ...
                for p in coords:
                    if shapely.Point(p).buffer(.1, cap_style='square').intersection(exterior).area > 2 * .1 ** 2:
                        continue
                    inside_point = p[0] + (c[0] - p[0]) * 1e-2, p[1] + (c[1] - p[1]) * 1e-2
                    ...
                    k, l = None, None
                    for k, obj_st in state.objs.items():
                        if room_type(k) not in [Semantics.Staircase, Semantics.Exterior] and room_level(k) == i:
                    ...
                    directions = [
                        (x_, y_) for x_ in [-1, 1] for y_ in [-1, 1] if
                        exterior.contains(shapely.Point(p[0] + x_, p[1] + y_))
                    ]
                    if len(directions) == 0:
                        continue
                    ...
                    while length > 0:
                        ...
                        cutter = shapely.box(min(p[0], q[0]), min(p[1], q[1]), max(p[0], q[0]), max(p[1], q[1]))
                        quad_segs = 1 if corner_func == 'sharp' else np.random.randint(4, 7)
                        exterior_ = exterior.difference(cutter).union(
                            shapely.Point(q).buffer(length, quad_segs=quad_segs)
                        ).buffer(0)
                        new = state[k].polygon.intersection(exterior_).buffer(0)
                        if all(new.buffer(-m + 1e-2).geom_type == 'Polygon' for m in np.linspace(0, .75, 4)):
                    ...
                    if i < len(state.graphs) - 1:
                        if not exterior_.contains(state[room_name(Semantics.Exterior, i + 1)].polygon):
                            continue
                    coords = np.array(new.exterior.coords)
                    diff = normalize(coords[1:] - coords[:-1])
                    diff_ = diff[list(range(1, len(diff))) + [0]]
                    if np.any((diff * diff_).sum(-1) < -.01):
                        continue
                    state[k].polygon = new
