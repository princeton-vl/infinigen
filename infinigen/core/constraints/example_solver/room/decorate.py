...
room_floor_fns = defaultdict(
    lambda: ('weighted_choice', (3, 'wood_tile'), (2, 'non_wood_tile'), (1, 'rug')), {
        t.Semantics.Garage: 'concrete',
        t.Semantics.Utility: ('weighted_choice', (1, 'concrete'), (1, 'concrete'), (1, 'tile')),
...
room_wall_fns = defaultdict(
    lambda: ('weighted_choice', (5, 'plaster'), (2, 'wood_tile'), (2, 'non_wood_tile')), {
        ...
        t.Semantics.Bathroom: 'non_wood_tile',
        t.Semantics.Restroom: 'non_wood_tile',
        t.Semantics.Warehouse: ('weighted_choice', (5, 'concrete'), (1, 'brick'), (3, 'plaster')),
    }
)
...
room_wall_alternative_fns = defaultdict(
    lambda: ('weighted_choice', (2, 'none'), (1, 'half')), {
        t.Semantics.LivingRoom: (
            'weighted_choice', (2, 'none'), (2, 'art'), (2, 'plaster'), (2, 'half')),
        t.Semantics.Bedroom: (
            'weighted_choice', (2, 'none'), (2, 'art'), (2, 'plaster'), (2, 'half')),
        t.Semantics.Office: (
            'weighted_choice', (2, 'none'), (2, 'art'), (2, 'plaster'), (2, 'half')),
...
pillar_rooms = {t.Semantics.LivingRoom, t.Semantics.Balcony, t.Semantics.DiningRoom, t.Semantics.Bedroom}


def room_walls(walls: list[bpy.types.Object], constants: RoomConstants, n_walls=3):
    wall_fns = list(import_material(rg(room_wall_fns[room_type(r.name)])) for r in walls)
    ...
    for w in walls:
        ...
        match fn:
            case 'none':
                continue
            case 'half':
                z_thresh = np.min(read_co(w)[:, -1]) + constants.wall_height * uniform(.3, .6)
                with butil.ViewportMode(w, 'EDIT'):
                    bpy.ops.mesh.select_all(action='SELECT')
                    bpy.ops.mesh.bisect(plane_co=(0, 0, z_thresh + w.location[-1]), plane_no=(0, 0, 1))
                write_attr_data(w, 'alternative', read_center(w)[:, -1] > z_thresh, type='INT', domain='FACE')
                plaster.apply(w, **kwargs, selection='alternative')
            case _:
                co = read_co(w)
                u, v = read_edges(w).T
                i = np.argmax(read_edge_length(w) - 100 * (np.abs(co[u, -1] - co[v, -1]) > .1))
                center = read_center(w)[:, :2]
                u_ = co[np.newaxis, u[i], :2]
                v_ = co[np.newaxis, v[i], :2]
                alternative = np.linalg.norm(center - u_, axis=-1) + np.linalg.norm(
                    center - v_, axis=-1
                ) - np.linalg.norm(u_ - v_, axis=-1) < .1
                write_attr_data(w, 'alternative', alternative, type='INT', domain='FACE')
                import_material(fn).apply(w, **kwargs, selection='alternative', scale=log_uniform(.5, 2.))


def room_ceilings(ceilings):
    logger.debug(f'{room_ceilings.__name__} adding materials to {len(ceilings)=}')

    ceiling_fns = list(import_material(rg(room_ceiling_fns[room_type(r.name)])) for r in ceilings)
...
def room_floors(floors, n_floors=3):
    floor_fns = list(import_material(rg(room_floor_fns[room_type(r.name)])) for r in floors)
...
def populate_windows(
    ...
    for j, cutter in enumerate(placeholders):
        ...
        curtain = None if cutter_dims[-1] < 4 and room_type(parent) not in room_no_curtain else False
        if abs(cutter_dims[-1] - constants.wall_height + constants.wall_thickness) < .1:
            window = factory(int(j), dimensions=dims, open=False, curtain=curtain, shutter=False)
        else:
...
def room_stairs(constants, state, rooms_meshed):
    ...
    for k, s in state.objs.items():
        if k.startswith(t.Semantics.StaircaseRoom.value):
            doors_ = [bpy.data.objects[l] for l, o in state.objs.items() if any(
                r.relation == cl.CutFrom() and r.target_name == k for r in
                    o.relations
            ) and l.startswith('door')]
            p = shapely.Polygon(s.polygon)
            contour = shapely.simplify(p.buffer(-constants.wall_thickness / 2, join_style='mitre'), .1)
            for door in doors_:
    ...
    for _ in trange(200, desc='Generating staircases: '):
        ...
        fns = [random_staircase_factory()(np.random.randint(1e7), False, constants) for _ in geoms]
        ...
        for j, fn in enumerate(fns):
            ...
            mlss.append(
                polygon.exterior if polygon.geom_type == 'Polygon' else shapely.MultiLineString(
                    [p.exterior for p in polygon.geoms]
                )
            )
            x, y, z = read_co(ph).T
            ...
            for j, mls in enumerate(mlss):
                ...
                for _ in range(50):
                    ...
                    if max(np.abs(p.x - projected.x), np.abs(p.y - projected.y)) < constants.staircase_snap:
                        ...
                        ) if mls.geom_type == 'MultiLineString' else mls.coords
                        projected = nearest_points(shapely.MultiPoint(coords), Point(x, y))[0]
                        if max(np.abs(p.x - projected.x), np.abs(p.y - projected.y)) <= constants.staircase_snap:
                            p = projected
                    x, y = p.x, p.y
                    placeholders[j].location = x, y, j * constants.wall_height + constants.wall_thickness / 2
                    ...
                    if contains_lower and contains_upper and lower_valid and upper_valid:
                        offsets.append((x, y))
                        break
            if len(offsets) == len(geoms):
                ts = list(
                    trimesh.convex.convex_hull(
                        obj2trimesh(ph).apply_transform(translation_matrix([*o, constants.wall_height * j]))
                    ) for
                        j, (ph, o) in enumerate(zip(placeholders, offsets))
                )
    ...
    for j, fn in enumerate(tqdm(fns)):
        ...
        s.location = *offsets[j], j * constants.wall_height + constants.wall_thickness / 2
        ...
        cutter.location = *offsets[j], j * constants.wall_height + constants.wall_thickness / 2
        butil.modify_mesh(
            mesh, 'BOOLEAN', object=cutter, operation='DIFFERENCE', use_self=True,
            use_hole_tolerant=True
        )
        butil.modify_mesh(
            mesh_, 'BOOLEAN', object=cutter, operation='DIFFERENCE', use_self=True,
            use_hole_tolerant=True
        )
        butil.delete(cutter)
