...
def domain_contains(dom: r.Domain, state: state_def.State, obj: state_def.ObjectState):
    ...
    for rel, dom in dom.relations:
        if isinstance(rel, cl.NegatedRelation):
            if any(
                relstate.relation.intersects(rel.rel) and domain_contains(dom, state, state.objs[relstate.target_name])
            ...
            if not any(
                relstate.relation.intersects(rel) and domain_contains(dom, state, state.objs[relstate.target_name])
                for relstate in obj.relations
            ):
...
def objkeys_in_dom(dom: r.Domain, curr: state_def.State):
    return [k for k, o in curr.objs.items() if domain_contains(dom, curr, o) and o.active]
