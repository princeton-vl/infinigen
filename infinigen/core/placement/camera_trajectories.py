...
def compute_trajectories(
    cam, n_trajectories,
    terrain, terrain_bvh, terrain_bbox, 
    ...
    base_view_selections = cam_util.compute_base_views(cam, n_views=n_min_candidates * min_base_views_ratio, 
        terrain=terrain, terrain_bvh=terrain_bvh, terrain_bbox=terrain_bbox, placeholders_kd=placeholders_kd,
        vertexwise_min_dist=vertexwise_min_dist, min_candidates_ratio=1,
        camera_selection_ratio=camera_selection_ratio)
    with tqdm(total=n_min_candidates, desc='Searching for potential trajectories') as pbar:
        for view in base_view_selections:
            ...
            try:
                animation_policy.animate_trajectory(obj=cam, bvh=terrain_bvh, policy_func=policy, 
                                                    validate_pose_func=anim_valid_pose_func, fatal=True, verbose=False)
            ...
            for frame in range(start, end+ 1, 2):
                ...
                dists, _, _ = terrain_camera_query(
                                cam, terrain_bvh, camera_selection_answers)
            ...
            trajectory_proposals.append((avg_score, deepcopy(keyframe_list), deepcopy(original_locations), 
                                         deepcopy(original_rotations), focus_dist))
            print(avg_score, len(original_locations))
            pbar.update()
...
@gin.configurable
def animate_trajectories(cam_rigs, 
    bounding_box,
    scene_preprocessed,
    policy_registry=None,
    validate_pose_func=None
    ...
    trajectories = compute_trajectories(dummy_camera, n_trajectories=len(cam_rigs),
                                        terrain=scene_preprocessed['terrain'], terrain_bvh=scene_preprocessed['terrain_bvh'],
                                        terrain_bbox=bounding_box, placeholders_kd=scene_preprocessed['placeholders_kd'],
                                        vertexwise_min_dist=scene_preprocessed['vertexwise_min_dist'],
                                        policy_registry=policy_registry, validate_pose_func=validate_pose_func)
    
    # keyframe in trajectory for each camera rig
    for cam_rig, trajectory in zip(cam_rigs, trajectories):
        print("number of keyframes", len(trajectory[1]))
        for keyframe, location, rotation in zip(trajectory[1], trajectory[2], trajectory[3]):
