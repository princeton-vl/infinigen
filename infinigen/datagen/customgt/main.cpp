...
std::vector<T> read_buffer(GLenum color_attachment, const int width, const int height)
{
    ...
    const std::string message =
        "type=" + std::to_string(type) + " format=" + std::to_string(format);
...
bool sortBySecond(const std::pair<fs::path, std::string> &a,
                  const std::pair<fs::path, std::string> &b)
{
...
int main(int argc, char *argv[])
{
    ...
    {
        std::cerr << "Error: The customgt executable is out-of-date, you need to "
                     "re-compile it."
                  << std::endl;
        exit(1);
    }
    ...
    program.add_argument("--dst_frame")
        .required()
        .help("destination frame for flow / point trajectory")
        .scan<'i', int>();
    ...
    program.add_argument("-dst_in", "--dst_input_dir")
        .required()
        .help("The destination input/output dir");
    program.add_argument("-out", "--output_dir").required().help("The input/output dir");
    program.add_argument("-lw", "--line_width")
        .default_value(2)
        .help("The width of the occlusion boundaries")
        .scan<'i', int>();
    program.add_argument("-s", "--subdivide")
        .default_value(2)
        .help("How many times to subdivide")
        .scan<'i', int>();
    program.add_argument("-nrf", "--normal_receptive_field")
        .default_value(1)
        .help("Receptive field of normal calculation (in px)")
        .scan<'i', int>();
    ...
    {
        const auto matches =
            match_regex("camview_([0-9]+_[0-9]+_[0-9]+_[0-9]+)", entry.path().stem().string());
    ...
    {
        cout << "The image size [" << buffer_width << " x " << buffer_height << "] is too large."
             << endl;
        return 1;
    }
...
// Linux (works headless)
    ...
    static const EGLint pbufferAttribs[] = {
        EGL_WIDTH, buffer_width, EGL_HEIGHT, buffer_height, EGL_NONE,
    };
    ...
    const EGLint GiveMeGLES2[] = {EGL_CONTEXT_CLIENT_VERSION,
                                  2,
...
// MacOS, Windows?
    ...
    for (int i = 0; i < camview_files.size(); i++)
    {
        ...
        camera_views.push_back(
            {entry.second, dst_entry.second, camera_dir, dst_camera_dir, output_w, output_h});
    }

    const auto glsl = source_directory / "glsl";
    Shader spineShader((glsl / "wings.vert").c_str(), (glsl / "spine.frag").c_str(),
                       (glsl / "spine.geom").c_str());
    Shader wingsShader((glsl / "wings.vert").c_str(), (glsl / "wings.frag").c_str(),
                       (glsl / "wings.geom").c_str());
    Shader hairShader(
        (glsl / "hair.vert").c_str(), (glsl / "hair.frag").c_str(), (glsl / "hair.geom").c_str());
    Shader nextShader((glsl / "next_wings.vert").c_str(), (glsl / "wings.frag").c_str(),
                      (glsl / "wings.geom").c_str());
    ...
    /*
        Drawing the 3D meshes. Instances are small-ish so we draw them all at
       once. Non-instances are larger so we draw them in chunks. Smaller stride
       -> smaller maxmimum memory.
    */
    std::vector<json> all_bboxes;
    while (true)
    {
        const auto some_object_model =
            load_blender_mesh(input_dir / frame_str / "mesh" / "saved_mesh.json",
                              dst_input_dir / dst_frame_str / "mesh" / "saved_mesh.json");
    ...
    for (const auto &cd : camera_views)
    {
        ...
        */
        {
            ...
            if (!depth_only)
            {
                ...
                */
                ...
                for (const loop_obj &o :
                     image_iterator(buffer_width, buffer_height, "Copying Optical Flow"))
                {
                    if (pixels[o.j + 2] > 0)
                    {
                        for (int k = 0; k < 3; k++)
        ...
        */
        ...
        {
            const auto pixels =
                read_buffer<float>(GL_COLOR_ATTACHMENT7, buffer_width, buffer_height);
            Eigen::Tensor<double, 3> geo_normals(output_h, output_w, 3);
            geo_normals.setZero();
            for (const loop_obj &o :
                 image_iterator(buffer_width, buffer_height, "Geometry Surface Normals"))
            {
                for (int k = 0; k < 3; k++)
            ...
            {
                const double norm = std::pow(std::pow(geo_normals(o.y, o.x, 0), 2) +
                                                 std::pow(geo_normals(o.y, o.x, 1), 2) +
                                                 std::pow(geo_normals(o.y, o.x, 2), 2),
                                             0.5);
                for (int k = 0; k < 3; k++)
                {
        ...
        */
        if (!flow_only && !depth_only)
        {
            ...
            for (const loop_obj &o :
                 image_iterator(buffer_width, buffer_height, "Copying instance segmentation masks"))
            {
                for (int k = 0; k < 3; k++)
                    instance_seg(o.y, o.x, k) = pixels[o.j + k];
            }
            save_npy(
                output_dir / ("InstanceSegmentation_" + cd.frame_string + ".npy"), instance_seg);
        }
        ...
        */
        if (!flow_only && !depth_only)
        {
            ...
            object_seg.setZero();
            for (const loop_obj &o :
                 image_iterator(buffer_width, buffer_height, "Copying object segmentation masks"))
            {
        ...
        if (!flow_only && !depth_only)
        {
            ...
            tag_seg.setZero();
            for (const loop_obj &o :
                 image_iterator(buffer_width, buffer_height, "Copying tag segmentation mask"))
            {
        ...
        // faceids.setZero();
        // {
        //     const auto pixels = read_buffer<int>(GL_COLOR_ATTACHMENT3,
        //     buffer_width, buffer_height); for (const loop_obj &o :
        //     image_iterator(buffer_width, buffer_height, "Copying face ids from
        //     first frame")){
        //         for (int k=0; k<3; k++)
        ...
        //     }
        // }
        // Eigen::Array<unsigned char, -1, -1> flow_occlusion(buffer_height,
        // buffer_width); for (const loop_obj &o : image_iterator(buffer_width,
        // buffer_height, "Calculating flow occlusion")){
        ...
        //             if ((0 <= next_y) && (next_y < buffer_height) && (0 <=
        //             next_x) && (next_x < buffer_width)){
        //                 const bool face_ids_match = (
        //                     (faceids(0, o.y, o.x, 0) == faceids(1, next_y,
        //                     next_x, 0)) && (faceids(0, o.y, o.x, 1) ==
        //                     faceids(1, next_y, next_x, 1)) && (faceids(0, o.y,
        //                     o.x, 2) == faceids(1, next_y, next_x, 2))
        //                 );
        ...
        // }
        // imwrite(output_dir / ("Flow3DMask_" + cd.frame_string + ".png"),
        // flow_occlusion);
        // }

        /*
        //  Reading/Writing the face size in m^2
        read_buffer<float>(GL_COLOR_ATTACHMENT3, pixels, buffer_width,
        buffer_height); auto face_size = Eigen::Tensor<double,
        2>(output_h,output_w); face_size.setZero(); for (const loop_obj &o :
        image_iterator(buffer_width, buffer_height, "Copying Face-Sizes (in
        m^2)")){ face_size(o.y/2, o.x/2) = pixels[o.j]/4;
        }

        //    Reading/Writing the face size in pixels
        read_buffer<float>(GL_COLOR_ATTACHMENT4, pixels, buffer_width,
        buffer_height); auto pixel_size = Eigen::Tensor<double,
        2>(output_h,output_w); pixel_size.setZero(); for (const loop_obj &o :
        image_iterator(buffer_width, buffer_height, "Copying Face-Sizes (in
        pixels)")){ pixel_size(o.y/2, o.x/2) = pixels[o.j]/4;
        }
        ...
        */
        if (!flow_only && !depth_only)
        {
            ...
            for (const loop_obj &o :
                 image_iterator(buffer_width, buffer_height, "Copying Occlusion Boundaries"))
            {
                occlusion_boundaries(o.y, o.x) = pixels[o.j + 1] * 255;
            }
            imwrite(output_dir / ("OcclusionBoundaries_" + cd.frame_string + ".png"),
