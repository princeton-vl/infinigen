...
unsigned int CameraView::create_framebuffer()
{
    ...
    constexpr GLenum color_attachments[8] = {
    ...
    for (int ai = 0; ai < num_color_attachments; ai++)
    {
        ...
        if (is_float[ai])
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F, buffer_width, buffer_height, 0, GL_RGBA,
                         GL_FLOAT, NULL);
        else
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32I, buffer_width, buffer_height, 0,
                         GL_RGBA_INTEGER, GL_INT, NULL);
        glCheckError();
        glFramebufferTexture2D(GL_FRAMEBUFFER, col_attach, GL_TEXTURE_2D, textureColorbuffer,
                               0); // attach it to currently bound framebuffer object
    }
...
CameraView::CameraView(const std::string fstr, const std::string dst_fstr, const fs::path input_dir,
                       const fs::path dst_input_dir, const int width, const int height)
    : frame_string(fstr), dst_frame_string(dst_fstr), image_height(height), image_width(width),
      buffer_height(height * 2), buffer_width(width * 2)
{
    // Current Frame
    ...
    current_frame_view_matrix = glm::make_mat4(
        Matrix4f(opengl_camera_pose.inverse()).data()); // Y up Z back (aka blender/opengl)
...
void CameraView::activateShader(Shader &shader) const
{
    ...
    shader.setMat4("view",
                   current_frame_view_matrix); // Y up Z back (aka blender/opengl)
...
Tensor<double, 3> CameraView::project(const Tensor<double, 3> &cam_coords) const
{
    ...
    for (int x = 0; x < width; x++)
    {
        for (int y = 0; y < height; y++)
        {
            ...
            output(y, x, 0) = u / buffer_over_image;
            output(y, x, 1) = v / buffer_over_image;
            output(y, x, 2) = z;
        }
    }
    return output;
