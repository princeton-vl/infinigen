...
template <typename T> std::vector<T> triangulate_face(std::vector<T> &polygon)
{
    std::vector<T> output;
...
void curve_quartet(const float *verts, const float *radii, std::vector<float> &point_lookup,
                   std::vector<unsigned int> &indices, int offs)
{
...
std::vector<unsigned int> generate_buffer(const std::vector<unsigned int> &indices)
{
    ...
    for (const auto &keyval : edge_neighbors)
    {
        ...
        for (const auto &v2_v3s : mapping)
        {
            ...
            if (v3s.size() == 1)
                v3s.push_back(v3s[0]); // i.e. treat dangling edges as two
                                       // triangles folded together
            RASSERT(v3s.size() >= 2);
...
{
    {
        ...
        for (int i = 0; i < (mwd.size() / 16); i++)
            model_mats[{instance_ids[i * 3], instance_ids[i * 3 + 1], instance_ids[i * 3 + 2]}] =
                Eigen::Matrix<float, 4, 4, Eigen::RowMajor>(&mwd[i * 16]);
    }

    if (obj_type == "MESH")
    {
        lookup = my_npz.read_data<float>(mesh_id + "_vertices");
        // const auto face_tag_lookup = my_npz.read_data<int>(mesh_id +
        // "_masktag"); tag_lookup.resize(lookup.size());
        ...
        // MRASSERT(loop_totals.size() == face_tag_lookup.size(),
        // "loop_totals.size() ["+std::to_string(loop_totals.size())+"] !=
        // tag_lookup.size() ["+std::to_string(face_tag_lookup.size())+"]");
        ...
        for (const int &polygon_size : loop_totals)
        {
            ...
            //     tag_lookup[t] = std::max(face_id, tag_lookup[t]); // tag is
            //     per-vertex, ideally should be per-face
    ...
    else if (obj_type == "CURVES")
    {
        ...
        for (int hair_idx = 0; hair_idx < num_hairs; hair_idx++)
            curve_quartet(vert_data.data() + (hair_idx * 5 * 3), radii_data.data() + (hair_idx * 5),
                          lookup, indices, hair_idx * 7);
    }
    else
    {
