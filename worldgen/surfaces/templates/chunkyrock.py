...
def shader_rocks(nw, rand=False, **input_kwargs):
    ...
    colorramp_2.color_ramp.elements[1].color = (0.01, 0.024, 0.0283, 1.0)
    if rand:
        sample_color(colorramp_2.color_ramp.elements[0].color)
        sample_color(colorramp_2.color_ramp.elements[1].color)

    colorramp_1 = nw.new_node(Nodes.ColorRamp,
        input_kwargs={'Fac': noise_texture_1.outputs["Fac"]})
    colorramp_1.color_ramp.elements[0].position = 0.0026
    colorramp_1.color_ramp.elements[0].color = (0.0, 0.0, 0.0, 1.0)
    colorramp_1.color_ramp.elements[1].position = 1.0
    colorramp_1.color_ramp.elements[1].color = (0.0694, 0.1221, 0.0693, 1.0)
    if rand:
        sample_color(colorramp_1.color_ramp.elements[0].color)
        sample_color(colorramp_1.color_ramp.elements[1].color)
...
def geo_rocks(nw: NodeWrangler, rand=False, selection=None, random_seed=0, geometry=True, **input_kwargs):
    ...
    with FixedSeed(random_seed):
        ...
        if rand:
            sample_max = 2
            sample_min = 1/2
            voronoi_texture.inputs["Scale"].default_value = sample_ratio(voronoi_texture.inputs["Scale"].default_value, sample_min, sample_max)
            voronoi_texture.inputs["W"].default_value = sample_range(0, 5)
