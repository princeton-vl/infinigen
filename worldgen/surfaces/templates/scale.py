...
def geo_scale(nw, rand=True, **input_kwargs):
    ...
    #separate_xyz_2 = nw.new_node(Nodes.SeparateXYZ, input_kwargs={'Vector': group_input.outputs["UV"]})
    ...
    separate_xyz_1 = nw.new_node(Nodes.SeparateXYZ, input_kwargs={'Vector': multiply_add})
    ...
    subtract_3 = nw.new_node(Nodes.Math, input_kwargs={0: colorramp_1.outputs["Color"]}, attrs={'operation': 'SUBTRACT'})
    
    sine_1 = nw.new_node(Nodes.Math, input_kwargs={0: subtract_3}, attrs={'operation': 'SINE'})
    
    multiply_15 = nw.new_node(Nodes.Math, input_kwargs={0: separate_xyz_1.outputs["X"], 1: sine_1}, attrs={'operation': 'MULTIPLY'})
    
    cosine_1 = nw.new_node(Nodes.Math, input_kwargs={0: subtract_3}, attrs={'operation': 'COSINE'})
    
    multiply_16 = nw.new_node(Nodes.Math, input_kwargs={0: separate_xyz_1.outputs["Y"], 1: cosine_1}, attrs={'operation': 'MULTIPLY'})
    
    add_6 = nw.new_node(Nodes.Math, input_kwargs={0: multiply_15, 1: multiply_16})
    ...
    colorramp = nw.new_node(Nodes.ColorRamp, input_kwargs={'Fac': multiply_18})
    colorramp.color_ramp.elements[0].position = 0.9156
    ...
    rgb_curves = nw.new_node(Nodes.RGBCurve, input_kwargs={'Color': colorramp.outputs["Color"]})
    node_utils.assign_curve(rgb_curves.mapping.curves[0], [(0.0000, 0.0000), (1.0000, 1.0000)])
    node_utils.assign_curve(rgb_curves.mapping.curves[1], [(0.0000, 0.0000), (1.0000, 1.0000)])
    node_utils.assign_curve(rgb_curves.mapping.curves[2], [(0.0000, 0.0000), (1.0000, 1.0000)])
    node_utils.assign_curve(rgb_curves.mapping.curves[3], [(0.0000, 0.0000), (0.5046, 0.4844), (1.0000, 1.0000)])
    
    multiply_19 = nw.new_node(Nodes.Math, input_kwargs={0: rgb_curves}, attrs={'operation': 'MULTIPLY'})
    ...
    capture_attribute_2 = nw.new_node(Nodes.CaptureAttribute,
        input_kwargs={'Geometry': set_position, 1: multiply_18},
        attrs={'data_type': 'FLOAT_VECTOR'})
    
    capture_attribute_1 = nw.new_node(Nodes.CaptureAttribute,
        input_kwargs={'Geometry': capture_attribute_2.outputs["Geometry"], 1: multiply_add_1},
        attrs={'data_type': 'FLOAT_VECTOR'})
    
    capture_attribute = nw.new_node(Nodes.CaptureAttribute,
        input_kwargs={'Geometry': capture_attribute_1.outputs["Geometry"], 1: multiply_add},
        attrs={'data_type': 'FLOAT_VECTOR'})
    
    capture_attribute_3 = nw.new_node(Nodes.CaptureAttribute,
        input_kwargs={'Geometry': capture_attribute.outputs["Geometry"], 1: subtract_4},
        attrs={'data_type': 'FLOAT_VECTOR'})
    
    capture_attribute_4 = nw.new_node(Nodes.CaptureAttribute,
        input_kwargs={'Geometry': capture_attribute_3.outputs["Geometry"], 1: multiply_19},
        attrs={'data_type': 'FLOAT_VECTOR'})
    
    group_output = nw.new_node(Nodes.GroupOutput,
        input_kwargs={
            'Geometry': capture_attribute_4.outputs["Geometry"], 
            #1: capture_attribute.outputs["Attribute"], 
            'attr2': capture_attribute_1.outputs["Attribute"], 
            'attr3': capture_attribute_2.outputs["Attribute"], 
            #4: capture_attribute_3.outputs["Attribute"], 
            'attr5': capture_attribute_4.outputs["Attribute"]
            },
        attrs={'is_active_output': True})

    #group_output = nw.new_node(Nodes.GroupOutput,
    #    input_kwargs={'Geometry': capture_attribute_4.outputs["Geometry"], 1: capture_attribute.outputs["Attribute"], 2: capture_attribute_1.outputs["Attribute"], 3: capture_attribute_2.outputs["Attribute"], 4: capture_attribute_3.outputs["Attribute"], 5: capture_attribute_4.outputs["Attribute"]})
    group_output = nw.new_node(Nodes.GroupOutput,
