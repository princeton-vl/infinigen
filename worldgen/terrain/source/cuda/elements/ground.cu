...
__global__ void ground_kernel(
    ...
    if (idx < size) {
        ground(position[idx], sdfs + idx, auxs + idx * n_auxiliaries, is_caved, i_params, f_params, caves_i_params, caves_f_params);
    }
}
...
extern "C" {
    ...
    void call(
        ...
        ground_kernel<<<ceil(size / 256.0), 256>>>(
