...
This section will allow you to use our own `--pipeline_configs opengl_gt` ground truth extraction config, which provides additional labels such as occlusion boundaries, sub-object segmentation, 3D flow and easy 3D bounding boxes. If you do not need these features, skip this section and use `--pipeline_configs blender_gt` as shown in [Generate image(s) in one command](#generate-images-in-one-command). This section is intended for computer vision researchers and power-users. 
...
Here, we will continue the [Hello-World](/README.md#generate-a-scene-step-by-step) example, but produce the full set of annotations that Infinigen supports.

To produce annotations for the "hello world" example:
...
```
3. Summarize file structure into a single JSON
```
...
`<rig>` and `<sub-cam>` are typically both "00" in the monocular setting. `<file-ext>` is typically "npy" or "png" for the the actual data and the visualization, `<frame>` is a 0-padded number, e.g. "0013". <type> can be "SurfaceNormal", "Depth", etc. For example
...
*path:* `d["Depth"]["npy"][<rig>][<sub-cam>][<frame>]` -> `frames/Depth_0001_00_00.npy`
...
*path:* `d["SurfaceNormal"]["npy"][<rig>][<sub-cam>][<frame>]` -> `frames/SurfaceNormal_0001_00_00.npy`
...
*path:* `d["Flow3D"]["npy"][<rig>][<sub-cam>][<frame>]` -> `frames/Flow3D_0001_00_00.npy`

*visualization:* `d["Flow3D"]["png"][<rig>][<sub-cam>][<frame>]` -> `frames/ObjectSegmentation_0001_00_00.png`
...
*path/visualization:* `d["Flow3DMask"]["png"][<rig>][<sub-cam>][<frame>]` -> `frames/Flow3DMask_0001_00_00.png`
...
*path*: `d["Camera Intrinsics"]["npy"][<rig>][<sub-cam>][<frame>]` -> `saved_mesh/frame_0002/cameras/K_0002_00_00.npy`
...
*path*: `d["Camera Pose"]["npy"][<rig>][<sub-cam>][<frame>]` -> `saved_mesh/frame_0002/cameras/T_0002_00_00.npy`
...
`d["ObjectSegmentation"]["npy"][<rig>][<sub-cam>][<frame>]` -> `frames/ObjectSegmentation_0001_00_00.npy`

`d["TagSegmentation"]["npy"][<rig>][<sub-cam>][<frame>]` -> `frames/TagSegmentation_0001_00_00.npy`

`d["InstanceSegmentation"]["npy"][<rig>][<sub-cam>][<frame>]` -> `frames/InstanceSegmentation_0001_00_00.npy`

`d["Objects"]["json"][<rig>][<sub-cam>][<frame>]` -> `frames/Objects_0001_00_00.json`

`d["Mask Tags"][<frame>]` -> `fine/MaskTag.json`

*visualization:*

`d["ObjectSegmentation"]["png"][<rig>][<sub-cam>][<frame>]` -> `frames/ObjectSegmentation_0001_00_00.png`

`d["TagSegmentation"]["png"][<rig>][<sub-cam>][<frame>]` -> `frames/TagSegmentation_0001_00_00.png`

`d["InstanceSegmentation"]["png"][<rig>][<sub-cam>][<frame>]` -> `frames/InstanceSegmentation_0001_00_00.png`
