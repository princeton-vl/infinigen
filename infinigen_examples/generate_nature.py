...
from infinigen.assets import cactus, creatures, fluid, lighting, monocot, rocks, trees, underwater, weather
...
def compose_nature(output_folder, scene_seed, **params):
    ...
    def add_bushes(terrain_mesh):
        ...
        for i in range(n_bush_species):
            ...
            selection = density.placement_mask(
                uniform(0.015, 0.2), normal_thresh=0.3, select_thresh=uniform(0.5, 0.6), tag=land_domain
            )
            placement.scatter_placeholders_mesh(
                terrain_mesh,
                fac,
    ...
    def add_boulders(terrain_mesh):
        ...
        for i in range(n_boulder_species):
            ...
            placement.scatter_placeholders_mesh(
                terrain_mesh,
                fac,
                overall_density=params.get("boulder_density", uniform(0.02, 0.05)) / n_boulder_species,
                selection=selection,
                altitude=-0.25,
            )
    ...
    def add_glowing_rocks(terrain_mesh):
        ...
        placement.scatter_placeholders_mesh(
            terrain_mesh, fac, overall_density=params.get("glow_rock_density", 0.025), selection=selection
        )
    ...
    def add_cactus(terrain_mesh):
        ...
        for i in range(n_cactus_species):
            ...
            selection = density.placement_mask(scale=0.05, tag=land_domain, select_thresh=0.57)
            placement.scatter_placeholders_mesh(
                terrain_mesh,
                fac,
                altitude=-0.05,
                overall_density=params.get("cactus_density", uniform(0.02, 0.1) / n_cactus_species),
                selection=selection,
                distance_min=1,
            )
    ...
    def add_ground_creatures(target):
        ...
        selection = (
            density.placement_mask(select_thresh=0, tag="beach", altitude_range=(-0.5, 0.5))
            if fac_class is creatures.CrabFactory
            else 1
        )
        col = placement.scatter_placeholders_mesh(
            target, fac, num_placeholders=n, overall_density=1, selection=selection, altitude=0.2
        )
    ...
    p.run_stage(
        "animate_cameras",
        lambda: cam_util.animate_cameras(camera_rigs, bbox, scene_preprocessed, pois=pois),
        use_chance=False,
    )
    ...
    def add_fish_school():
        ...
        for i in range(n):
            ...
            col = placement.scatter_placeholders_mesh(
                terrain_near, fac, selection=selection, overall_density=1, num_placeholders=1, altitude=2
            )
    ...
    def add_bug_swarm():
        ...
        col = placement.scatter_placeholders_mesh(
            terrain_inview, fac, selection=selection, overall_density=1, num_placeholders=n, altitude=2
        )
        placement.populate_collection(fac, col)
    ...
    def add_rocks(target):
        selection = density.placement_mask(
            scale=0.15, select_thresh=0.5, normal_thresh=0.7, return_scalar=True, tag=nonliving_domain
        )
    ...
    def add_ground_leaves(target):
        selection = density.placement_mask(
            scale=0.1, select_thresh=0.52, normal_thresh=0.7, return_scalar=True, tag=land_domain
        )
    ...
    def add_ground_twigs(target):
        ...
        selection = density.placement_mask(
            scale=0.15, select_thresh=0.55, normal_thresh=0.7, return_scalar=True, tag=nonliving_domain
        )
    ...
    def add_chopped_trees(target):
        selection = density.placement_mask(
            scale=0.15, select_thresh=uniform(0.55, 0.6), normal_thresh=0.7, return_scalar=True, tag=nonliving_domain
        )
    ...
    def add_monocots(target):
        selection = density.placement_mask(normal_dir=(0, 0, 1), scale=0.2, tag=land_domain)
        monocots.apply(terrain_inview, grass=True, selection=selection)
        selection = density.placement_mask(
            normal_dir=(0, 0, 1), scale=0.2, select_thresh=0.55, tag=params.get("grass_habitats", None)
        )
    ...
    def add_ferns(target):
        selection = density.placement_mask(
            normal_dir=(0, 0, 1), scale=0.1, select_thresh=0.6, return_scalar=True, tag=land_domain
        )
    ...
    def add_flowers(target):
        selection = density.placement_mask(
            normal_dir=(0, 0, 1), scale=0.01, select_thresh=0.6, return_scalar=True, tag=land_domain
        )
    ...
    def add_corals(target):
        vertical_faces = density.placement_mask(scale=0.15, select_thresh=uniform(0.44, 0.48))
        coral_reef.apply(
            target, selection=vertical_faces, tag=underwater_domain, density=params.get("coral_density", 2.5)
        )
        horizontal_faces = density.placement_mask(scale=0.15, normal_thresh=-0.4, normal_thresh_high=0.4)
        coral_reef.apply(
            target,
    ...
    p.run_stage(
        ...
        lambda: ground_mushroom.Mushrooms().apply(
            terrain_near,
            selection=density.placement_mask(scale=0.1, select_thresh=0.65, return_scalar=True, tag=land_domain),
    ...
    p.run_stage(
        ...
        lambda: seaweed.apply(
            terrain_inview,
            selection=density.placement_mask(scale=0.05, select_thresh=0.5, normal_thresh=0.4, tag=underwater_domain),
        ),
    )
    p.run_stage(
        "urchin",
        lambda: urchin.apply(
            terrain_inview, selection=density.placement_mask(scale=0.05, select_thresh=0.5, tag=underwater_domain)
        ),
    )
    p.run_stage(
        "jellyfish",
    ...
    p.run_stage(
        ...
        lambda: seashells.apply(
            terrain_near,
            selection=density.placement_mask(scale=0.05, select_thresh=0.5, tag="landscape,", return_scalar=True),
        ),
    )
    p.run_stage(
        "pinecone",
        lambda: pinecone.apply(
            terrain_near, selection=density.placement_mask(scale=0.1, select_thresh=0.63, tag=land_domain)
        ),
    )
    p.run_stage(
        "pine_needle",
        lambda: pine_needle.apply(
            terrain_near,
            selection=density.placement_mask(
                scale=uniform(0.05, 0.2), select_thresh=uniform(0.4, 0.55), tag=land_domain, return_scalar=True
            ),
        ),
    )
    p.run_stage(
        ...
        lambda: decorative_plants.apply(
            terrain_near,
            selection=density.placement_mask(
                scale=uniform(0.05, 0.2), select_thresh=uniform(0.5, 0.65), tag=land_domain, return_scalar=True
            ),
        ),
    )
    ...
    placeholders = list(
        itertools.chain.from_iterable(c.all_objects for c in bpy.data.collections if c.name.startswith("placeholders:"))
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=[],
        help="Parameter settings that override config defaults " "e.g. --gin_param module_1.a=2 module_2.b=3",
    )
