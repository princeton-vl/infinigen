...
from infinigen.core.constraints import example_solver
from infinigen.core.constraints import reasoning as r
from infinigen.core.constraints import usage_lookup
from infinigen.core.constraints.example_solver import Solver, greedy, populate, state_def
...
def default_greedy_stages():
    ...
    greedy_stages["on_wall"] = (
        primary.with_relation(-on_floor, all_room).with_relation(-on_ceiling, all_room).with_relation(on_wall, all_room)
    )
...
def compose_indoors(output_folder: Path, scene_seed: int, **overrides):
    ...
    def solve_large():
        assignments = greedy.iterate_assignments(stages["on_floor"], state, all_vars, limits, nonempty=True)
    ...
    solved_rooms = [
        ...
        for assignment in greedy.iterate_assignments(stages["on_floor"], state, [cu.variable_room], limits)
    ]
    ...
    def pose_cameras():
        nonroom_objs = [o.obj for o in state.objs.values() if t.Semantics.Room not in o.tags]
        scene_objs = solved_rooms + nonroom_objs

        scene_preprocessed = placement.camera.camera_selection_preprocessing(terrain=None, scene_objs=scene_objs)

        solved_floor_surface = butil.join_objects(
            [tagging.extract_tagged_faces(o, {t.Subpart.SupportSurface}) for o in solved_rooms]
        )

        placement.camera.configure_cameras(
            camera_rigs, scene_preprocessed=scene_preprocessed, init_surfaces=solved_floor_surface
        )
    ...
    p.run_stage(
        "room_windows", lambda: room_dec.populate_windows(solver.get_bpy_objects(window_filter)), use_chance=False
    )
    ...
    p.run_stage("overhead_cam", place_cam_overhead, cam=camera_rigs[0], bbox=solved_bbox, use_chance=False)
                ...
                for mat in wall.data.materials:
                    for n in mat.node_tree.nodes:
                        if n.type == "BSDF_PRINCIPLED":
                            n.inputs["Alpha"].default_value = overrides.get("alpha_walls", 1.0)
...
def main(args):
    ...
    init.apply_gin_configs(
        configs=args.configs, overrides=args.overrides, configs_folder="infinigen_examples/configs_indoor"
    )
