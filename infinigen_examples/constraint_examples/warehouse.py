...
def warehouse_constraints(weights=None, fast=False):
    ...
    node_constraint = (
        rooms[Semantics.Root].all(
            lambda r: rooms[Semantics.Warehouse]
        ...
        * rooms[Semantics.Warehouse].all(
            lambda r: rooms[Semantics.FactoryOffice]
            .related_to(r, cl.Traverse())
            .count()
            .in_range(0, 1, mean=0.7)
        )
        * rooms[Semantics.Warehouse].all(
    ...
    room_term = (
        ...
        .minimize(weight=5.0)
        + (
            rooms[Semantics.Warehouse].sum(
                lambda r: (r.area() / 300).log().hinge(0, 0.4).pow(2)
            )
            + rooms[Semantics.Garage].sum(
                lambda r: (r.area() / 80).log().hinge(0, 0.4).pow(2)
            )
            + rooms[Semantics.FactoryOffice].sum(
        ...
        + rooms.union({Semantics.FactoryOffice, Semantics.Garage, Semantics.Warehouse})
        .sum(lambda r: r.aspect_ratio().log())
        .minimize(weight=50.0)
        + rooms.union({Semantics.Utility, Semantics.Restroom})
        .sum(lambda r: r.aspect_ratio().log())
        ...
        .minimize(weight=1.0)
        + rooms.union({Semantics.FactoryOffice, Semantics.Warehouse, Semantics.Garage})
        ...
        .minimize(weight=5.0)
        + rooms.union({Semantics.Garage, Semantics.Warehouse})
        .sum(
            lambda r: rooms.union({Semantics.Garage, Semantics.Warehouse})
            .related_to(r, cl.Traverse())
            .shared_length(r)
            .hinge(4, np.inf)
        )
...
all_constraint_funcs = [warehouse_constraints]
