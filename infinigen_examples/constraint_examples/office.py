...
def office_constraints(weights=None, fast=False):
    ...
    constants = RoomConstants(
        room_type={Semantics.Hallway,
            ...
            Semantics.Utility
        }, aspect_ratio_range=(.2, .3), fixed_contour=True
    )
    ...
    node_constraint = (
            rooms[Semantics.Root].all(
                lambda r: rooms[Semantics.Hallway].related_to(r, cl.Traverse()).count().in_range(1, 2, mean=1.4)
            ) *
            rooms[Semantics.Hallway].all(
                lambda r: rooms[Semantics.Hallway].related_to(r, cl.Traverse()).count().in_range(0, 2, mean=.7)
            ) *
            rooms[Semantics.Hallway].all(
                lambda r: rooms[Semantics.MeetingRoom].related_to(r, cl.Traverse()).count().in_range(0, 1, mean=.6)
            ) *
            rooms[Semantics.Hallway].all(
                lambda r: rooms[Semantics.Office].related_to(r, cl.Traverse()).count().in_range(0, 2, mean=.6)
            ) *
            rooms[Semantics.Hallway].all(
                lambda r: rooms[Semantics.OpenOffice].related_to(r, cl.Traverse()).count().in_range(0, 1, mean=.6)
            ) *
            rooms[Semantics.StaircaseRoom].related_to(rooms[Semantics.Hallway], cl.Traverse()).count().in_range(
                n_staircase, n_staircase, mean=n_staircase
            ) *
            rooms[Semantics.Hallway].all(
                lambda r: rooms[Semantics.BreakRoom].related_to(r, cl.Traverse()).count().in_range(0, 1, mean=.5)
            ) *
            rooms[Semantics.Hallway].all(
                lambda r: rooms[Semantics.Restroom].related_to(r, cl.Traverse()).count().in_range(0, 1, mean=.3)
            ) *
            rooms[Semantics.Hallway].all(
                lambda r: rooms[Semantics.Utility].related_to(r, cl.Traverse()).count().in_range(0, 1, mean=.4)
            ) *
            ...
            rooms[Semantics.OpenOffice].all(
                lambda r: rooms[Semantics.Office].related_to(r, cl.Traverse()).count().in_range(0, 1, mean=.4)
            ) *
            rooms[Semantics.OpenOffice].all(
                lambda r: rooms[Semantics.OpenOffice].related_to(r, cl.Traverse()).count().in_range(0, 1, mean=.4)
            ) *
            rooms[Semantics.OpenOffice].all(
                lambda r: rooms[Semantics.MeetingRoom].related_to(r, cl.Traverse()).count().in_range(0, 1, mean=.3)
            ) *
            rooms[Semantics.OpenOffice].all(
                lambda r: rooms[Semantics.Utility].related_to(r, cl.Traverse()).count().in_range(0, 1, mean=.3)
            )
    )
    ...
    node_constraint = (
            ...
            rooms.all(
                lambda r: rooms[-Semantics.Exterior][-Semantics.Entrance].related_to(r, cl.Traverse()).count() <= 5
            ) *
    ...
    room_term = (
            ...
            (rooms[Semantics.MeetingRoom].sum(lambda r: (r.area() / 25).log().hinge(0, .4).pow(2)) +
             ...
             rooms[Semantics.BreakRoom].sum(lambda r: (r.area() / 15).log().hinge(0, .4).pow(2)) +
